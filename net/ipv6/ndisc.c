...
#ifdef CONFIG_LGP_DATA_TCPIP_IPV6_MTU_MTK
#include <linux/netdevice.h>
#endif
/* 2016-05-26 shsh.kim@lge.com LGP_DATA_TCPIP_IPV6_MTU_MTK [END] */
...
#ifdef CONFIG_LGP_DATA_TCPIP_IPV6_MTU_MTK
    printk(KERN_INFO "[LGE_DATA][ipv6] sysctl_lge_carrier = %d\n", sysctl_lge_carrier);
    if ( sysctl_lge_carrier == LGU_CARRIER || sysctl_lge_carrier == KT_CARRIER ) {
        if (ndopts.nd_opts_mtu) {
            ...
            mtu = ntohl(n);
            printk(KERN_INFO "[LGE_DATA][ipv6] mtu = %d / dev->mtu = %d, device %s,\n", mtu, skb->dev->mtu,skb->dev->name);
