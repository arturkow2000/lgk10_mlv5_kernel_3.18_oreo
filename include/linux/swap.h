...
enum {
	...
	SWP_SYNCHRONOUS_IO = (1 << 10),	/* synchronous IO is efficient */
...
extern struct swap_info_struct *swap_info_get(swp_entry_t entry);
extern void swap_info_lock(struct swap_info_struct *si);
extern void swap_info_unlock(struct swap_info_struct *si);
#else /* CONFIG_SWAP */

static inline int swap_readpage(struct page *page)
{
	return 0;
}
