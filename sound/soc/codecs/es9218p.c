...
//extern int ioexp_gpio_get_value(uint8_t gpio);
extern int ioexp_gpio_set_value(uint8_t gpio, uint8_t value);
//extern int ioexp_gpio_set_cfg(uint8_t gpio, uint8_t direction, uint8_t pulldown_disable, uint8_t level);
#endif
...
#ifdef CONFIG_MACH_MT6750S_CV5A
static struct regulator *reg_VCN33_HIFI;
#endif
...
enum hifi_gpio_type {
        ...
        PINCTRL_POWER_LDO_OFF,
        PINCTRL_POWER_LDO_ON,
        PINCTRL_HIFI_NUM
};
...
struct hifi_gpio_attr hifi_gpios[PINCTRL_HIFI_NUM] = {
        ...
        [PINCTRL_POWER_LDO_OFF] = {"hifi_ldo_off", NULL},
        [PINCTRL_POWER_LDO_ON] = {"hifi_ldo_on", NULL},
};
...
static int g_headset_type = 1; //Temporary
...
#endif
#ifdef CONFIG_MACH_MT6750S_CV5A
	if((board_rev == HW_REV_0)||(board_rev == HW_REV_A)){
		if (regulator_enable(reg_VCN33_HIFI)){
			pr_err("VCN33_HIFI on fail!\n");
		}
	}else{
        pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_POWER_LDO_ON].gpioctrl);
	}
#else
	pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_POWER_LDO_ON].gpioctrl);
#endif
...
#ifdef CONFIG_MACH_MT6750S_CV5A
	if((board_rev == HW_REV_0)||(board_rev == HW_REV_A)){
		if (regulator_disable(reg_VCN33_HIFI)){
			pr_err("VCN33_HIFI off fail!\n");
		}
	}else{
        pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_POWER_LDO_OFF].gpioctrl);
	}
#else
	pinctrl_select_state(pinctrl_hifi, hifi_gpios[PINCTRL_POWER_LDO_OFF].gpioctrl);
#endif
...
static void es9218_reset_gpio_H(void)
{
    ...
    //gpio_set_value(g_es9218_priv->es9218_data->reset_gpio, 1);
    //mt_set_gpio_dir(g_es9218_priv->es9218_data->reset_gpio, GPIO_DIR_OUT);
    //mt_set_gpio_out(g_es9218_priv->es9218_data->reset_gpio, GPIO_OUT_ONE);
    pr_info("%s()\n", __func__);
#endif
}
...
static void es9218_reset_gpio_L(void)
{
    ...
    //gpio_set_value(g_es9218_priv->es9218_data->reset_gpio, 0);
    //mt_set_gpio_dir(g_es9218_priv->es9218_data->reset_gpio, GPIO_DIR_OUT);
    //mt_set_gpio_out(g_es9218_priv->es9218_data->reset_gpio, GPIO_OUT_ZERO);
    pr_info("%s()\n", __func__);
#endif
}
...
#else
    /*int ret,value;
    value = tca6418a_get_gpio(0x23);
    pr_err("%s() - dir %x", __func__, value);
    value |= 0x10;
    ret = tca6418a_set_gpio(0x23, value);
    pr_err("%s() - dir %x, ret %d", __func__, value, ret);

    value = tca6418a_get_gpio(0x17);
    pr_err("%s() - on %x", __func__, value);
    value |= 0x10;
    ret = tca6418a_set_gpio(0x17, value);
    pr_err("%s() - on %x, ret %d", __func__, value, ret);*/
#ifdef CONFIG_MACH_MT6750S_CV7A
        nxp_gpio_set_value(2, 1);
#else
#ifdef CONFIG_MACH_MT6750S_CV5A
    if (lge_get_board_revno() == 0) {
        #ifdef CONFIG_TI_TCA6418
        ioexp_gpio_set_value(3, 1);
        #endif
    } else {
        #ifdef CONFIG_GPIO_PCAL6416A
        nxp_gpio_set_value(2, 1);
...
#endif // CONFIG_MACH_MT6750S_CV7A
#endif
}
...
static void es9218_hph_switch_gpio_L(void)
{
   ...
   gpio_set_value(g_es9218_priv->es9218_data->hph_switch, 0);
    ...
    value = tca6418a_get_gpio(0x17);
    pr_err("%s() - off %x", __func__, value);
    value &= (~0x10);
    ret = tca6418a_set_gpio(0x17, value);
    pr_err("%s() - off %x, ret %d", __func__, value, ret);*/
#ifdef CONFIG_MACH_MT6750S_CV7A
        nxp_gpio_set_value(2, 0);
#else
#ifdef CONFIG_MACH_MT6750S_CV5A
    if (board_rev == HW_REV_0) {
        #ifdef CONFIG_TI_TCA6418
        ioexp_gpio_set_value(3, 0);
        #endif
    } else {
        #ifdef CONFIG_GPIO_PCAL6416A
        nxp_gpio_set_value(2, 0);
...
static int es9218p_sabre_bypass2hifi(void)
{
    ...
    if( call_common_init_registers == 1 ) {
        ...
        for (i = 0 ; i < sizeof(es9218_common_init_registers)/sizeof(es9218_common_init_registers[0]) ; i++) {
			pr_info("%s(): call es9218_common_init_registers - register num = %d.\n", __func__, es9218_common_init_registers[i].num);
        ...
        struct snd_ctl_elem_value *ucontrol)
{
    ...
    switch(ucontrol->value.enumerated.item[0]) {
        ...
        case SET_STATE_ACTIVE:
            ...
            break;
        case SET_STATE_RESET_HIGH:
            es9218_reset_gpio_H();
            break;
        case SET_STATE_RESET_LOW:
            es9218_reset_gpio_L();
            break;
        case SET_STATE_HPH_HIGH:
            es9218_hph_switch_gpio_H();
            break;
        case SET_STATE_HPH_LOW:
            es9218_hph_switch_gpio_L();
            break;
        case SET_STATE_START_UP:
            es9218_startup_dl1();
            break;
        case SET_STATE_SHUT_DOWN:
            es9218_shutdown_dl1();
            break;
        default:
            break;
        ...
        struct es9218_data *pdata)
{
    ...
    //pdata->reset_gpio = of_get_named_gpio(dev->of_node, "dac,reset-gpio", 0);
    of_property_read_u32_index(dev->of_node, "dac,reset-gpio", 0, &pdata->reset_gpio);
    ...
    if (pdata->reset_gpio < 0) {
        ...
        //pdata->reset_gpio |= 0x80000000;
        mt_set_gpio_mode(pdata->reset_gpio, GPIO_MODE_00);
    }
...
int es9218_pcm_hw_params_dl1(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
    ...
    if(es9218_power_state == ESS_PS_CLOSE || es9218_power_state == ESS_PS_IDLE || es9218_power_state == ESS_PS_BYPASS) {
        return ret;
    }
...
EXPORT_SYMBOL(es9218_startup_dl1);

void es9218_shutdown_dl1(void)
{
    mutex_lock(&g_es9218_priv->power_lock);
...
EXPORT_SYMBOL(es9218_shutdown_dl1);
...
static int es9218_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	board_rev = lge_get_board_revno();
	pr_info("%s(), lge board revision : %d\n", __func__,board_rev);
    ...
    if((board_rev == HW_REV_0)||(board_rev == HW_REV_A)){
        ...
        reg_VCN33_HIFI = regulator_get(&client->dev, "vcn33_bt");
        if (!reg_VCN33_HIFI){
            pr_err("Regulator_get VCN33_HIFI fail\n");
        }else{
            regulator_set_voltage(reg_VCN33_HIFI, 3300 * 1000, 3300 * 1000);
        }
    }
#endif
...
static int __init es9218_init(void)
{
    ...
    if (i2c_add_driver(&es9218_i2c_driver) != 0) {
        ...
        pr_notice("[es9218_init] success to register es9218_i2c_driver.\n");
    }
    return 0;
}
