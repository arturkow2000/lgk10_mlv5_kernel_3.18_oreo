...
static int max98925_post_probe(struct snd_soc_codec *codec);
static void max98925_amp_reinit_put_work(struct work_struct *work);
static int max98925_enable(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	int i = 0;
	msg_maxim("Reinit in\n");
	for (i = 0; i < platform->component.card->num_links; i++) {

		runtime = snd_soc_get_pcm_runtime(platform->component.card, platform->component.card->rtd[i].dai_link->name);
		if(runtime != NULL){
			if(!runtime->pcm)
				continue;
			substream = snd_soc_dpcm_get_substream(runtime, SNDRV_PCM_STREAM_PLAYBACK);
			if(substream != NULL){
				if(substream->runtime != NULL){
					max98925->ch_size = substream->runtime->format;
					msg_maxim("platform->component.card->rtd[i].dai_link->name=%s format=%d\n",
						platform->component.card->rtd[i].dai_link->name, substream->runtime->format);
				}
			}
		}
	}
	if(!max98925->ch_size){
		dev_err(codec->dev, "%s: can't find stream info\n", __func__);
		max98925->ch_size = SNDRV_PCM_FORMAT_S32_LE;
	}

	max98925->sel = (int)ucontrol->value.integer.value[0];

	ret = schedule_delayed_work(&max98925->amp_reinit_work, msecs_to_jiffies(0));
...
static void max98925_amp_reinit_put_work(struct work_struct *work)
{
	struct max98925_priv *max98925 = NULL;
	int i;
	...
	max98925 = container_of(to_delayed_work(work), struct max98925_priv, amp_reinit_work);

	mutex_lock(&max98925->mutex);

	sel = max98925->sel;

	max98925_post_probe(max98925->codec);
	msg_maxim("Reinit work in, sel : %d\n", sel);
	...
	max98925_handle_pdata(max98925->codec);
#ifndef CONFIG_LGE_EXTERNAL_SPEAKER
	max98925_regmap_update_bits(max98925,
			MAX98925_R02D_GAIN,
			MAX98925_SPK_GAIN_MASK,
...
/* DAI format configuration */
	if (max98925->ch_size == SNDRV_PCM_FORMAT_S32_LE){
		dai_bclk = MAX98925_DAI_BSEL_64;
		ch_size = MAX98925_DAI_CHANSZ_32;
	} else if (max98925->ch_size == SNDRV_PCM_FORMAT_S24_LE){
		dai_bclk = MAX98925_DAI_BSEL_64;
		ch_size = MAX98925_DAI_CHANSZ_32;
	} else {
...
static const char * const Amp_Reinit_Status[] = {"Off", "On"};
static const struct soc_enum Amp_Eum[] = {
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Amp_Reinit_Status), Amp_Reinit_Status),
};
...
static const struct snd_kcontrol_new max98925_snd_controls[] = {
	...
	SOC_ENUM_EXT("Amp Reinit", Amp_Eum[0],
		max98925_amp_reinit_get, max98925_amp_reinit_put),

	SOC_ENUM_EXT("Amp enable", Amp_Eum[0],
		max98925_amp_reinit_get, max98925_enable),
#ifdef CONFIG_SND_SOC_MAXIM_DSM
#ifdef USE_DSM_LOG
...
static int max98925_enable(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max98925_priv *max98925 = snd_soc_codec_get_drvdata(codec);
	int enable = (int)ucontrol->value.integer.value[0];
	msg_maxim("max98925_enable %d\n", enable);

	if (enable) {
		__max98925_dai_digital_mute(max98925, !enable);
	} else {
		__max98925_dai_digital_mute(max98925, !enable);
	}

	msg_maxim("max98925_enable done\n");
	return 0;
}
...
static int max98925_post_probe(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "%s build number %s\n", __FUNCTION__, MAX98925_REVISION);
...
static int max98925_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&max98925->amp_reinit_work, max98925_amp_reinit_put_work);
