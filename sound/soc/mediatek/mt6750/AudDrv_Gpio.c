...
struct audio_gpio_attr hifi_bypass_gpios[HIFI_BYPASS_NUM] = {
        [POWER_LDO_OFF] = {"hifi_ldo_off", false, NULL},
        [POWER_LDO_ON] = {"hifi_ldo_on", false, NULL},
        [RESET_OFF] = {"hifi_reset_off", false, NULL},
        [RESET_ON] = {"hifi_reset_on", false, NULL},
};

void AudDrv_GPIO_HIFI_DAC_BYPASS_Select(int bEnable)
{
	int retval = 0;
	pr_warn("%s\n", __func__);

	if (bEnable == 1) {
		if (hifi_bypass_gpios[POWER_LDO_ON].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, hifi_bypass_gpios[POWER_LDO_ON].gpioctrl);
			if (retval)
				pr_err("could not set hifi_bypass_gpios[POWER_LDO_ON] pins\n");
		}
		if (hifi_bypass_gpios[RESET_OFF].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, hifi_bypass_gpios[RESET_OFF].gpioctrl);
			if (retval)
				pr_err("could not set hifi_bypass_gpios[RESET_OFF] pins\n");
		}
#ifdef CONFIG_GPIO_PCAL6416A
		nxp_gpio_set_value(2, 1);
#endif
	} else {
		if (hifi_bypass_gpios[POWER_LDO_OFF].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, hifi_bypass_gpios[POWER_LDO_OFF].gpioctrl);
			if (retval)
				pr_err("could not set hifi_bypass_gpios[POWER_LDO_OFF] pins\n");
		}
		if (hifi_bypass_gpios[RESET_ON].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, hifi_bypass_gpios[RESET_ON].gpioctrl);
			if (retval)
				pr_err("could not set hifi_bypass_gpios[RESET_ON] pins\n");
		}
#ifdef CONFIG_GPIO_PCAL6416A
...
#if defined(CONFIG_MACH_MT6750S_CV5A) && !defined(CONFIG_SND_SOC_ES9218P)
	for (i = 0; i < ARRAY_SIZE(hifi_bypass_gpios); i++) {
		hifi_bypass_gpios[i].gpioctrl = pinctrl_lookup_state(pinctrlaud, hifi_bypass_gpios[i].name);
		if (IS_ERR(hifi_bypass_gpios[i].gpioctrl)) {
			ret = PTR_ERR(hifi_bypass_gpios[i].gpioctrl);
			pr_err("%s pinctrl_lookup_state %s fail %d\n", __func__, hifi_bypass_gpios[i].name,
			       ret);
		} else {
			hifi_bypass_gpios[i].gpio_prepare = true;
		}
	}
#endif
