...
#ifdef CONFIG_MTK_LGE_EXT_SPEAKER_AMP
#include "../amp/AudDrv_Ext_Amp.h"
#endif

#if defined(CONFIG_MACH_MT6750S_CV5A) && !defined(CONFIG_SND_SOC_ES9218P) || defined(CONFIG_MACH_MT6750S_CV7A)
...
#define GPIO_AUDIO_EN 88
#define GPIO_AUDIO_SW 85
#endif

#ifdef CONFIG_MACH_MT6750S_CV7A
#define GPIO_EARJACK_SW_EN 83
#endif
...
#ifdef CONFIG_LGE_ACCDET_IMPEDANCE
static DEFINE_MUTEX(Accdet_Impedance_lock);
#endif
...
void CalculateDCCompenForEachdB(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		...
		pr_debug("mHplOffset_CompenBydB[%d]=%d, mHprOffset_CompenBydB[%d]=%d\n", i, mHplOffset_CompenBydB[i],
...
static void SetHprOffset(int OffsetTrimming)
{
	...
	if (DCoffsetValue >= -5 && DCoffsetValue <= 5) {
		RegValue = 0;
	}

	pr_warn("%s(), OffsetTrimming = %d, DCoffsetValue = %d, RegValue = 0x%x\n", __func__, OffsetTrimming, DCoffsetValue, RegValue);
...
#ifdef CONFIG_MACH_MT6750_TH8
static int HP_Analog_Switch_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s() GPIO_AUDIO_EN = %d\n", __func__, gpio_get_value(GPIO_AUDIO_EN));
	pr_warn("%s() GPIO_AUDIO_SW = %d\n", __func__, gpio_get_value(GPIO_AUDIO_SW));
	return 0;
}

static int HP_Analog_Switch_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int switch_enable = ucontrol->value.integer.value[0];

	pr_warn("HP_Analog_Switch_Set = %d\n", switch_enable);
	gpio_set_value(GPIO_AUDIO_EN, switch_enable);
	gpio_set_value(GPIO_AUDIO_SW, switch_enable);
	return 0;
}
#endif

#ifdef CONFIG_MACH_MT6750S_CV7A
static int Earjack_Switch_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s() GPIO_EARJACK_SW_EN = %d\n", __func__, gpio_get_value(GPIO_EARJACK_SW_EN));
	ucontrol->value.integer.value[0] = gpio_get_value(GPIO_EARJACK_SW_EN);
	return 0;
}

static int Earjack_Switch_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int switch_enable = ucontrol->value.integer.value[0];
	int rev = lge_get_board_revno();
	if(rev >= HW_REV_0_1) {
		pr_warn("Earjack_Switch_Set = %d\n", switch_enable);
		gpio_set_value(GPIO_EARJACK_SW_EN, switch_enable);
	}
	return 0;
}
#endif
...
static void Voice_Amp_Change(bool enable)
{
#ifdef CONFIG_MTK_LGE_HSOUT_GAIN
	int pga_index;
	pga_index = mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HSOUTL];
#endif
	...
	if (enable) {
		...
		if (GetDLStatus() == false) {
			...
			udelay(50);
#ifdef CONFIG_MTK_LGE_HSOUT_GAIN
			pr_warn("%s pga gain set by user, index(%d)\n", __func__,pga_index);
			Ana_Set_Reg(MT6353_ZCD_CON3, pga_index, 0x001f);
#else
...
#ifndef CONFIG_MTK_LGE_EXT_SPEAKER_AMP
...
#else
#ifdef CONFIG_MACH_MT6750_TH8
		AudDrv_GPIO_EXTAMP_Select(0,0);
		AudDrv_GPIO_EXTAMP2_Select(0,0);
#else
...
#else
#ifdef CONFIG_MACH_MT6750_TH8
		AudDrv_GPIO_EXTAMP_Select(1,1);
		AudDrv_GPIO_EXTAMP2_Select(1,1);
#else
...
#if defined(CONFIG_USING_EXTAMP_LO)
#if defined(CONFIG_TI_TCA6418)
		ioexp_gpio_set_value(13, 1);
#else
...
#endif /* !CONFIG_MTK_LGE_EXT_SPEAKER_AMP */
...
#ifdef CONFIG_MACH_MT6750_TH8
	SOC_ENUM_EXT("HP_Analog_Switch", Audio_DL_Enum[13], HP_Analog_Switch_Get, HP_Analog_Switch_Set),
#endif
#ifdef CONFIG_MACH_MT6750S_CV7A
	SOC_ENUM_EXT("Earjack_Switch", Audio_DL_Enum[13], Earjack_Switch_Get, Earjack_Switch_Set),
#endif
};
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0701, 0xff19);
				/* Enable MICBIAS0, MISBIAS0 = 1P9V,
...
{
	pr_aud("+%s(), start\n", __func__);
#ifndef CONFIG_LGE_ACCDET_IMPEDANCE
...
#ifdef CONFIG_LGE_ACCDET_IMPEDANCE
bool accdet_OpenHeadPhoneImpedanceSetting(bool bEnable)
{
	if (bEnable == true) {
		TurnOnDacPower();
...
/*static int accdet_Detect_Impedance_By_Phase(void)
{
	unsigned int i;
	unsigned int j;
	unsigned short dcoffset = 0, average = 0;
	...
	int AdcValues[4];
	AdcValues[0] = 0;
	AdcValues[1] = HpImpedancePhase0AdcValue;
	AdcValues[2] = HpImpedancePhase1AdcValue;
	AdcValues[3] = HpImpedancePhase2AdcValue;
	...
	for(j = 0; j < 4; j++){
		...
		if (value == HpImpedancePhase0AdcValue) {
			...
			if (detect_impedance) {
				pr_debug("[DC][phase %d] SUM = %d 1st = %d 2nd = %d 3rd = %d 4th = %d\n",
					value, average, ibuffer_v[0], ibuffer_v[1], ibuffer_v[2], ibuffer_v[3]);
				pr_debug("[DC][phase %d]average = %d dcinit_value = %d detect_impedance = %d\n",
					value, average, dcoffset, detect_impedance);
				break;
			}
		}
...
int accdet_getHeadsetPowerStatus(void)
{
	if(!mCodec_data)
		return false;
	return mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL];
}
EXPORT_SYMBOL(accdet_getHeadsetPowerStatus);
...
int accdet_Audio_Hp_Impedance_Get(void)
{
	...
	if(!mInitCodec){
		int initCount = 5;
		while(initCount){
			usleep_range(300*1000, 350*1000);
			if(mInitCodec)
				break;
			initCount--;
		}
	...
	DLStatus = GetDLStatus();
	pr_warn("+%s(), start DLStatus=%d\n", __func__, DLStatus);
	mutex_lock(&Accdet_Impedance_lock);
	if (DLStatus){
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I05, Soc_Aud_InterConnectionOutput_O03);
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I06, Soc_Aud_InterConnectionOutput_O04);
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I07, Soc_Aud_InterConnectionOutput_O03);
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I08, Soc_Aud_InterConnectionOutput_O04);
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O03);
		SetConnectionState(0, Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O04);

		backupreg1 = Ana_Get_Reg(MT6353_AUDDEC_ANA_CON0);
		backupreg2 = Ana_Get_Reg(MT6353_SPK_CON0);
		backupreg3 = Ana_Get_Reg(MT6353_SPK_ANA_CON3);

		Ana_Set_Reg(MT6353_SPK_CON0, 0x0000, 0xf000);
		usleep_range(1*2500, 1*5000);
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0000, 0xffff);
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0, 0xffff);
		TurnOffDacPower();
		pr_warn("%s(), disconnect afe backupreg1=%x backupreg2=%x backupreg3=%x\n", __func__, backupreg1, backupreg2, backupreg3);
	}
	if (accdet_OpenHeadPhoneImpedanceSetting(true) == true) {
#if defined(CONFIG_MTK_HP_ANASWITCH)
		AudDrv_GPIO_HPDEPOP_Select(false);
#endif
		mhp_impedance = Detect_Impedance_By_Phase();
		accdet_OpenHeadPhoneImpedanceSetting(false);
	} else
		pr_warn("%s(), Pmic DL Busy, HPDET do nothing\n", __func__);

	if(DLStatus) {
		DLStatus = GetDLStatus();
		pr_warn("%s(), Recheck DLStatus=%d\n", __func__, DLStatus);
	}

	if (DLStatus){
		TurnOnDacPower();
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, backupreg3, 0xffff);
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, backupreg1, 0xffff);
		if(Speaker_pga_gain)
		    Ana_Set_Reg(MT6353_SPK_ANA_CON0, Speaker_pga_gain << 11, 0x7800);
		usleep_range(1*2500, 1*5000);
		Ana_Set_Reg(MT6353_SPK_CON0, backupreg2, 0xf000);
	}
    mutex_unlock(&Accdet_Impedance_lock);
	pr_warn("-%s(), mhp_impedance = %d\n", __func__, mhp_impedance);
	return mhp_impedance;
}
EXPORT_SYMBOL(accdet_Audio_Hp_Impedance_Get);
#endif
...
#ifdef CONFIG_MTK_LGE_HSOUT_GAIN
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HSOUTL] = 0;
#else
...
#if defined(CONFIG_MACH_MT6750S_CV5A) && !defined(CONFIG_SND_SOC_ES9218P)
	if(lge_get_board_revno() == HW_REV_0_1)
		AudDrv_GPIO_HIFI_DAC_BYPASS_Select(1);
#endif
	return 0;
}
