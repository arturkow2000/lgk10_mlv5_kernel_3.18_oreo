...
#endif
#ifdef CONFIG_DEBUG_FRAGINFO_DEFAULT_ENABLE
int fraginfo_enabled = 1;
#else
int fraginfo_enabled = 0;
#endif
...
void warn_alloc(gfp_t gfp_mask, const char *fmt, ...)
{
	...
	pr_cont(", mode:%#x(%pGg)", gfp_mask, &gfp_mask);
...
void start_memalloc_timer(const gfp_t gfp_mask, const int order)
{
	...
	/* We don't check for stalls for !__GFP_WAIT allocations. */
...
void stop_memalloc_timer(const gfp_t gfp_mask)
{
	...
	if ((gfp_mask & __GFP_WAIT) && !--m->in_flight)
...
#if defined(CONFIG_DMAUSER_PAGES) || defined(CONFIG_ZONE_MOVABLE_CMA) || defined(CONFIG_HSWAP)
...
void show_free_areas(unsigned int filter)
{
	...
	for_each_populated_zone(zone) {
		...
		if (skip_free_areas_node(filter, zone_to_nid(zone)))
			continue;
		show_node(zone);
		pgdat = NODE_DATA(zone_to_nid(zone));
		printk("%s"
			" free:%lukB"
			" min:%lukB"
			" low:%lukB"
...
int fraginfo_enable_sysctl_handler(struct ctl_table *table, int write,
		void __user *buffer, size_t *length, loff_t *ppos)
{
	int ret, order;
	struct zone *zone;
