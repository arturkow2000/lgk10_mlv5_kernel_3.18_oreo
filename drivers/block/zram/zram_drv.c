...
static void zram_revalidate_disk(struct zram *zram)
{
	...
	/* revalidate_disk reset the BDI_CAP_SYNCHRONOUS_IO so set again */
...
#ifdef CONFIG_ZSM
	if (!zram_test_flag(meta, index, ZRAM_SAME) && zsm_test_flag_index(meta, index, ZRAM_ZSM_NODE)) {
...
static int zram_mlog_cb(int id, void *ptr, void *data)
{
	struct zram *zram;
	unsigned long *mem_pool_size = data;

	zram = idr_find(&zram_index_idr, id);
	if (zram && init_done(zram))
		*mem_pool_size += zs_get_total_pages(zram->meta->mem_pool);
...
unsigned long zram_mlog(void)
{
	...
	idr_for_each(&zram_index_idr, &zram_mlog_cb, &mem_pool_size);

#define P2K(x) (((unsigned long)x) << (PAGE_SHIFT - 10))
	return P2K(mem_pool_size);
#undef P2K
}

#ifdef CONFIG_PROC_FS
static int zraminfo_cb(int id, void *ptr, void *data)
{
	struct zram *zram;
	struct zs_pool_stats pool_stats;
	...
	if (zram) {
		...
		if (!init_done(zram)) {
			up_read(&zram->init_lock);
			seq_printf(m, "zram%d is not initialized\n", id);
			return 0;
		}
		up_read(&zram->init_lock);
...
#endif
				"MaxUsedPages:   %8lu kB\n"
				"PageCompacted:	 %8lu kB\n"
				,
				B2K(zram->disksize),
				P2K(atomic64_read(&zram->stats.pages_stored)),
				B2K(atomic64_read(&zram->stats.compr_data_size)),
				P2K(zs_get_total_pages(zram->meta->mem_pool)),
				P2K(atomic64_read(&zram->stats.same_pages)),
				P2K(atomic64_read(&zram->stats.notify_free)),
				P2K(atomic64_read(&zram->stats.failed_reads)),
				P2K(atomic64_read(&zram->stats.failed_writes)),
				P2K(atomic64_read(&zram->stats.num_reads)),
				P2K(atomic64_read(&zram->stats.num_writes)),
				P2K(atomic64_read(&zram->stats.invalid_io)),
#ifdef CONFIG_ZSM
				B2K(atomic64_read(&zram->stats.zsm_saved)),
				B2K(atomic64_read(&zram->stats.zsm_saved4k)),
#endif
				P2K(atomic_long_read(&zram->stats.max_used_pages)),
				P2K(atomic64_read(&pool_stats.pages_compacted)));
#undef P2K
#undef B2K
...
{
	idr_for_each(&zram_index_idr, &zraminfo_cb, m);
