...
#if defined(CONFIG_LGE_USB_TYPE_C) && defined(CONFIG_LGE_USB_MOISTURE_DETECTION)
...
int battery_in_uevent = 0;
#endif
#ifdef CONFIG_LGE_PM
struct wake_lock battery_notify_lock;
#endif
/* ////////////////////////////////////////////////////////////////////////////// */
...
#if defined(CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT)
...
static struct hrtimer battery_resume_running_kthread_timer;
kal_bool g_battery_soc_ready = KAL_FALSE;
unsigned char fg_ipoh_reset;

static ktime_t resume_running_kthread_remaining_time;
kal_bool resume_over_kthread_period = KAL_FALSE;
kal_bool kthread_timeout_checked = KAL_FALSE;
kal_bool resume_running_kthread_timeout = KAL_FALSE;
...
#define FLOATED_CHARGER_CONFIRM_MS (2000)	/* recheck charger type after 2sec */
static struct delayed_work floated_charger_confirm;
static bool floated_charger = false;
#endif
...
#ifdef PUMP_EXPRESS_SERIES
	POWER_SUPPLY_PROP_FASTCHG,
	POWER_SUPPLY_PROP_FAKE_HVDCP_MODE,
#endif
...
#endif
#ifdef CONFIG_LGE_PM_MTK_SAVE_BATTERY_CYCLE_GM_2_0
...
{
#ifdef CONFIG_LGE_PM_VTS
	return BMT_status.vts_battery / 10;
#else	/* MTK Original */
				 ...
				 enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		/* convert unit 10mA -> 1mA */
		val->intval = BMT_status.input_current_max * 10;
		break;
#endif
#endif
	default:
			   ...
			   enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = BMT_status.charger_vol * 1000;
		break;
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		/* convert unit 10mA -> 1mA */
	...
	case POWER_SUPPLY_PROP_FASTCHG:
		if (!BMT_status.charger_exist)
			val->intval = 0;
		else if (mtk_pep20_get_is_connect() ||
				mtk_pep_get_is_connect())
			val->intval = 1;
		else if ((BMT_status.fake_hvdcp_enabled) &&
				(BMT_status.charger_vol > 8000))
			val->intval = 1;
		else
			val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_INCOMPATIBLE_CHG:
		...
		if (BMT_status.typec_mode == POWER_SUPPLY_TYPEC_SOURCE_HIGH)
			val->intval = 0;
		if (BMT_status.typec_mode == POWER_SUPPLY_TYPEC_SOURCE_MEDIUM)
			val->intval = 0;
...
#ifdef CONFIG_LGE_PM
static int ac_set_property(struct power_supply *psy,
...
static int ac_property_is_writeable(struct power_supply *psy,
				    enum power_supply_property psp)
{
	int ret = 0;
...
#ifdef CONFIG_LGE_USB_TYPE_C
static void usb_typec_update_current(void)
{
	unsigned int enable = false;

	battery_log(BAT_LOG_CRTI, "suspend input to update typec current\n");

	/* suspend input here for current transition */
	battery_charging_control(
			CHARGING_CMD_ENABLE_POWER_PATH, &enable);
	battery_charging_control(
			CHARGING_CMD_ENABLE_POWER_PATH_SLAVE, &enable);

	/* changing charging current will be done in BAT_Thread */
	wake_up_bat();
}

static int usb_typec_source_changed(int old, int new)
{
	/* source not changed */
	if (old == new)
		return 0;
	...
	if (old > POWER_SUPPLY_TYPEC_SOURCE_HIGH ||
			old < POWER_SUPPLY_TYPEC_SOURCE_MEDIUM)
		return 0;

	if (new > POWER_SUPPLY_TYPEC_SOURCE_HIGH ||
			new < POWER_SUPPLY_TYPEC_SOURCE_MEDIUM)
		return 0;

	return 1;
}

static void usb_typec_mode_update(int mode)
{
	int prev_mode = BMT_status.typec_mode;

	if (mode < POWER_SUPPLY_TYPEC_NONE)
		return;
	if (mode > POWER_SUPPLY_TYPEC_NON_COMPLIANT)
		return;

	if (BMT_status.typec_mode == mode)
		return;
	BMT_status.typec_mode = mode;

	battery_log(BAT_LOG_CRTI, "typec mode = %d\n", mode);

	if (usb_typec_source_changed(prev_mode, mode))
		usb_typec_update_current();
}

static void usb_typec_check_source(void)
{
	...
	if (!typec_psy || !typec_psy->get_property)
		return;

	rc = typec_psy->get_property(typec_psy,
				POWER_SUPPLY_PROP_TYPEC_MODE, &val);
	if (rc)
		return;

	usb_typec_mode_update(val.intval);
}
#endif
			    ...
			    enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FASTCHG:
		if (!BMT_status.charger_exist)
			...
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_FAKE_HVDCP_MODE:
		val->intval = BMT_status.fake_hvdcp_enabled;
		break;
#endif
	case POWER_SUPPLY_PROP_FASTCHG_SUPPORT:
		val->intval = batt_cust_data.mtk_pump_express_plus_support;
		break;
#ifdef CONFIG_LGE_PM_FLOATED_CHARGER_DETECT
	case POWER_SUPPLY_PROP_INCOMPATIBLE_CHG:
		if (floated_charger)
	...
	case POWER_SUPPLY_PROP_TYPEC_MODE:
		val->intval = BMT_status.typec_mode;
		break;
#endif
	default:
		ret = -EINVAL;
...
#ifdef PUMP_EXPRESS_SERIES
	case POWER_SUPPLY_PROP_FAKE_HVDCP_MODE:
		if (BMT_status.fake_hvdcp_enabled != val->intval) {
			BMT_status.fake_hvdcp_enabled = val->intval;
			wake_up_bat();
		}
		break;
#endif
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
...
#if defined (CONFIG_LGE_PM_USB_ID) && defined (CONFIG_LGE_USB_FACTORY)
		if (BMT_status.bat_exist == KAL_FALSE &&
				lge_is_factory_cable_boot())
			val->intval = 100;
#endif
		break;
...
#ifdef CONFIG_LGE_PM_VTS
		val->intval = BMT_status.vts_battery;
#else	/* MTK Original */
		val->intval = tbatt_temp_tc1;
#endif
#ifdef CONFIG_LGE_PM_USB_ID
		if (BMT_status.bat_exist == KAL_FALSE &&
				lge_is_factory_cable_boot())
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		/* convert 0.1mA -> 1uA */
		val->intval = (BMT_status.IBattery * 100) * (-1);
		break;
	...
	case POWER_SUPPLY_PROP_VALID_BATT_ID:
		...
		if (lge_battery_check())
			val->intval = 1;
#if defined (CONFIG_LGE_PM_USB_ID) && defined (CONFIG_LGE_USB_FACTORY)
		else if ((!BMT_status.bat_exist) && lge_is_factory_cable_boot()){
			val->intval = 1;
		}
#endif
		else
	...
	case POWER_SUPPLY_PROP_FIRST_SOC_EST_DONE:
		if (BMT_status.first_soc_est_done)
			val->intval = 1;
		else
			val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if (BMT_status.charging_enabled == KAL_TRUE)
			val->intval = 1;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		if (BMT_status.battery_charging_enabled == KAL_TRUE)
			val->intval = 1;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = BMT_status.constant_charge_current_max;

		/* convert unit 10mA -> 1mA */
		val->intval *= 10;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		val->intval = BMT_status.constant_charge_voltage_max;
		break;
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		val->intval = BMT_status.safety_timer_enabled ? 1 : 0;
		break;
#endif
#ifdef CONFIG_LGE_PM_MTK_SAVE_BATTERY_CYCLE_GM_2_0
	case POWER_SUPPLY_PROP_BATTERY_CYCLE:
		val->intval = get_battery_cycle_count();
		break;
	case POWER_SUPPLY_PROP_CALIBRATE:
		val->intval = get_battery_cycle_offset();
		break;
#endif
#if defined(CONFIG_LGE_USB_TYPE_C) && defined(CONFIG_LGE_USB_MOISTURE_DETECTION)
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		val->intval	= sm5513_get_input_suspend();
		break;
#endif
#endif
	default:
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	/* overide property for LG scenario */
	chgctrl_property_override(psp, val);
#endif
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if (BMT_status.charging_enabled != val->intval) {
			BMT_status.charging_enabled = val->intval;
			wake_up_bat();
		}
		break;

	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		if (BMT_status.battery_charging_enabled != val->intval) {
			BMT_status.battery_charging_enabled = val->intval;
			wake_up_bat();
		}
		break;
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		if (BMT_status.safety_timer_enabled != val->intval) {
			BMT_status.safety_timer_enabled = val->intval;
			wake_up_bat();
		}
		break;
#endif
#ifdef CONFIG_LGE_PM_MTK_SAVE_BATTERY_CYCLE_GM_2_0
	case POWER_SUPPLY_PROP_BATTERY_CYCLE:
		set_battery_cycle_count(val->intval);
		break;
	case POWER_SUPPLY_PROP_CALIBRATE:
		set_battery_cycle_offset(val->intval);
		break;
#endif
	default:
		rc = -EINVAL;
...
{
	battery_log(BAT_LOG_CRTI, "wake_up_bat from %s\n", psy->name);
...
#ifdef CONFIG_LGE_PM
		.set_property = ac_set_property,
		.property_is_writeable = ac_property_is_writeable,
				...
				char *buf)
{
	...
	battery_charging_control(CHARGING_CMD_GET_SHIP_MODE, &ship_mode);
				...
				const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &ship_mode) == 0) {
		battery_log(BAT_LOG_CRTI, "ship_mode = %d\n", ship_mode);
		battery_charging_control(CHARGING_CMD_SET_SHIP_MODE, &ship_mode);
		return size;
	}
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
...
#ifdef CONFIG_LGE_PM
	bat_data->BAT_RAW_CAPACITY = BMT_status.UI_SOC;
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	/* POWER_SUPPLY_STATUS_FULL will be handled in charger controller */
#else
	...
	if(get_boot_mode() == CHARGER_BOOT){
		if (bat_data->BAT_CAPACITY <= 0){
			...
			battery_log(BAT_LOG_CRTI,"[mt_battery_update_EM] chargerlogo boot need to change BAT_CAPACITY %d\n", bat_data->BAT_CAPACITY);
		}
	}
#endif
...
static void battery_update(struct battery_data *bat_data)
{
...
#ifdef CONFIG_LGE_PM_CHARGERLOGO_WAIT_FOR_FG_INIT
	if (!BMT_status.first_soc_est_done) {
		BMT_status.first_soc_est_done = 1;
		battery_log(BAT_LOG_CRTI, "first_soc_est_done set\n");
	}
#endif
	...
	if (update_cnt == 6) {
		...
		cable_in_uevent = 0;
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	} else if (battery_in_uevent == 1) {
		/* update when battery not exist */
		power_supply_changed(bat_psy);
		battery_in_uevent = 0;
#endif
	} else {
...
#ifdef CONFIG_INPUT_EPACK
		    (BMT_status.charger_type == EPACK_CHARGER) ||
#endif
...
#ifdef CONFIG_LGE_PM
#ifdef PUMP_EXPRESS_SERIES
static void pep_update(void)
{
	static int pep_status = 0;
	int pep_is_connected = 0;

	if (mtk_pep20_get_is_connect() || mtk_pep_get_is_connect())
		pep_is_connected = 1;

	if (BMT_status.fake_hvdcp_enabled && (BMT_status.charger_vol > 8000))
		pep_is_connected = 1;

	if (!BMT_status.charger_exist)
		pep_is_connected = 0;

	if (pep_status != pep_is_connected) {
		pep_status = pep_is_connected;
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
static void chgctrl_update(void)
{
	static int ac_online = 0;
	static int usb_online = 0;
	static kal_bool safety_timer_enabled;
	static signed int bat_charging_state;
	static signed int UI_SOC2;

	bool notify_batt = false;

	if (ac_online != ac_main.AC_ONLINE) {
		ac_online = ac_main.AC_ONLINE;
		notify_batt = true;
		notify_charger_controller(&ac_main.psy);
	}

	if (usb_online != usb_main.USB_ONLINE) {
		usb_online = usb_main.USB_ONLINE;
		notify_batt = true;
		notify_charger_controller(&usb_main.psy);
	}

	if (BMT_status.safety_timer_enabled != safety_timer_enabled)
		notify_batt = true;
	if (BMT_status.bat_charging_state != bat_charging_state)
		notify_batt = true;
	if (BMT_status.UI_SOC2 != UI_SOC2)
		notify_batt = true;

	if (notify_batt)
		notify_charger_controller(&battery_main.psy);

	safety_timer_enabled = BMT_status.safety_timer_enabled;
	bat_charging_state = BMT_status.bat_charging_state;
	UI_SOC2 = BMT_status.UI_SOC2;
}
#endif
...
#ifdef CONFIG_LGE_PM_VTS
static int vts_battery_get_temp(int tbat)
{
#ifdef CONFIG_LGE_PM_VTS_COMPENSATE
	static int pre_vts;
	int vts_now;
#endif

	if (!lge_vts_get_enabled()) {
		pr_info("[Not Ready] return tbat\n");
		return tbat;
	}

#ifdef CONFIG_LGE_PM_VTS_COMPENSATE
	vts_now = lge_vts_get_battery_temp();

	battery_log(BAT_LOG_CRTI,
			"[VTS_COMPESATE] vts_now %d, pre_vts %d, tbat %d\n",
			vts_now, pre_vts ,tbat);

	if (vts_now <= tbat) {
		battery_log(BAT_LOG_CRTI, "[VTS_COMPESATE] tbat over vts_now : update vts_now\n");
		pre_vts = vts_now;
		return vts_now;
	}

	if (vts_now < pre_vts) {
		battery_log(BAT_LOG_CRTI, "[VTS_COMPESATE] pre_vts over vts_now : update vts_now\n");
		pre_vts = vts_now;
		return vts_now;
	}

	if (vts_now <= 540) {
		battery_log(BAT_LOG_CRTI, "[VTS_COMPESATE] vts_now under 54 : update vts_now\n");
		pre_vts = vts_now;
		return vts_now;
	}

	if (tbat < pre_vts) {
		battery_log(BAT_LOG_CRTI, "[VTS_COMPESATE] vts_now under tbat : update pre_vts\n");
		return pre_vts;
	}

	battery_log(BAT_LOG_CRTI, "[VTS_COMPESATE] tbat over pre_vts : update tbat\n");
	return tbat;
#else
	return lge_vts_get_battery_temp();
#endif
}
#endif
...
#ifdef CONFIG_LGE_PM
	/* Do not use average battery temperature for OTP */
	if(type == BATTERY_AVG_TEMP)
		return data;
#ifdef CONFIG_LGE_PM_USB_ID
	/* Do not use average battery voltage for AT%BATL */
	if ((type == BATTERY_AVG_VOLT) && lge_is_factory_cable_boot())
		return data;
#endif
#endif
...
void mt_battery_GetBatteryData(void)
{
...
#ifdef CONFIG_LGE_PM_VTS
	BMT_status.vts_battery = vts_battery_get_temp(tbatt_temp_tc1);
	BMT_status.temperature =
	    mt_battery_average_method(BATTERY_AVG_TEMP, &batteryTempBuffer[0], BMT_status.vts_battery / 10,
	...
	if (BMT_status.bat_exist == KAL_FALSE && lge_is_factory_cable_boot()) {
		BMT_status.temperature = temperature = 20;
		BMT_status.SOC = BMT_status.UI_SOC = BMT_status.UI_SOC2 = 100;
		battery_log(BAT_LOG_CRTI, "[BATTERY] No battery and Factory cable insert. Use dummy soc and temp.\n");
	}
#endif
...
#ifdef CONFIG_LGE_PM_VTS
	battery_log(BAT_LOG_CRTI,
		"[kernel]AvgVbat %d, bat_vol %d, AvgI %d, I %d, VChr %d, AvgT(v) %d, T(v) %d, RawT %d, ZCV %d, CHR_Type %d, SOC %3d:%3d:%3d, bcct %d:%d, Ichg %d, IBat %d\n",
		BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		BMT_status.charger_vol, BMT_status.temperature, BMT_status.vts_battery / 10, temperature, BMT_status.ZCV,
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	/* battery temp check will be done in charger controller */
	return status;
#endif
...
#endif // MTK_TEMPERATURE_RECHARGE_SUPPORT
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* Charging IC or PMIC will check charger voltage */
#else
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* Charging IC will run safety timer */
#else
...
#ifdef CONFIG_LGE_PM_FLOATED_CHARGER_DETECT
void mt_charger_confirm_floated_charger(struct work_struct *work)
{
	struct power_supply *usb_psy = &usb_main.psy;
	CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;

	if (!BMT_status.charger_exist)
		return;

	if (BMT_status.fake_hvdcp_enabled)
		return;
	...
	battery_log(BAT_LOG_CRTI, "retry charger type detection\n");

	battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
	if (BMT_status.charger_type == CHR_Type_num) {
		floated_charger = true;

		battery_log(BAT_LOG_CRTI, "NONSTANDARD_CHARGER confirmed\n");
	...
	if (!BMT_status.charger_exist) {
		battery_log(BAT_LOG_CRTI, "charger plugged out during detection\n");
	...
	battery_log(BAT_LOG_CRTI, "redetected CHR_Type = %d\n", BMT_status.charger_type);

	mutex_unlock(&charger_type_mutex);

	/* change charging state to PRE to check PEP */
	if (BMT_status.charger_type == STANDARD_CHARGER)
		BMT_status.bat_charging_state = CHR_PRE;
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
#ifdef CONFIG_INPUT_EPACK
	CHARGER_TYPE prev = BMT_status.charger_type;
#endif
...
#ifdef CONFIG_LGE_PM_FLOATED_CHARGER_DETECT
		if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
			schedule_delayed_work(&floated_charger_confirm,
					msecs_to_jiffies(FLOATED_CHARGER_CONFIRM_MS));
		}
#endif
#endif
...
#ifdef CONFIG_INPUT_EPACK
	if (prev != BMT_status.charger_type) {
		battery_log(BAT_LOG_CRTI, "epack CHARGER_TYPE is ( %d )\n", BMT_status.charger_type);
		if (BMT_status.charger_type == EPACK_CHARGER && !epack_is_ok())
			epack_call_update_handler(1);
	}
#endif
...
extern int rt9460_get_noise_wa_cv(unsigned int *cv);
#endif
...
#ifdef CONFIG_INPUT_EPACK
		if (BMT_status.charger_type != CHARGER_UNKNOWN) {
			battery_log(BAT_LOG_CRTI, "epack CHARGER_TYPE is ( 0 )\n");
			if (BMT_status.charger_type == EPACK_CHARGER && epack_get_pwr_path() == EPACK_PWR_PATH_EPACK)
				epack_call_update_handler(0);
		}
#endif
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
		BMT_status.input_current_max = 0;
		BMT_status.constant_charge_current_max = 0;
		BMT_status.constant_charge_voltage_max = 0;
#endif
#ifdef CONFIG_LGE_PM_FLOATED_CHARGER_DETECT
		floated_charger = false;
		cancel_delayed_work(&floated_charger_confirm);
#endif
...
#endif
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
		battery_charging_control(CHARGING_CMD_ENABLE_SLAVE,
				&BMT_status.charger_exist);
		battery_charging_control(CHARGING_CMD_ENABLE_POWER_PATH_SLAVE,
				&BMT_status.charger_exist);
#endif
	}
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_RT9460_NOISE_WA
	{
		unsigned int cv = BATTERY_VOLT_04_400000_V;

		rt9460_get_noise_wa_cv(&cv);

		battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv);
	}
#endif

#if defined (CONFIG_LGE_TOUCH_CORE) || defined(CONFIG_LGE_TOUCH_CORE_NOS)
	touch_notify_connect(BMT_status.charger_type);
#endif
}
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
static void _do_chrdet_int_task(struct work_struct *work);

#define CHRDET_PLUGIN_MS 200
static struct delayed_work do_chrdet_work;
void do_chrdet_int_task(void)
{
	...
	if (!init) {
		INIT_DELAYED_WORK(&do_chrdet_work, _do_chrdet_int_task);
		init = true;
	}

	charger_exist = upmu_is_chr_det();
	if (charger_exist == BMT_status.charger_exist)
		return;

	battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task] charger %s\n",
			(charger_exist ? "plug in" : "plug out"));

	if (charger_exist == KAL_TRUE) {
		cancel_delayed_work(&do_chrdet_work);
		schedule_delayed_work(&do_chrdet_work, msecs_to_jiffies(CHRDET_PLUGIN_MS));
	} else {
		cancel_delayed_work_sync(&do_chrdet_work);
		schedule_delayed_work(&do_chrdet_work, 0);
	}
}
...
#endif
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			wake_unlock(&battery_suspend_lock);
#ifdef CONFIG_LGE_PM
			wake_lock_timeout(&battery_notify_lock, msecs_to_jiffies(3000));
#endif
#if defined(CONFIG_POWER_EXT)
			mt_usb_disconnect();
			mt_bc12_dcd_release();
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
		/* do not need to force bat_full here */
#else
		if (BMT_status.UI_SOC2 == 100 && BMT_status.charger_exist == KAL_TRUE
		    && BMT_status.bat_charging_state != CHR_ERROR) {
			BMT_status.bat_charging_state = CHR_BATFULL;
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
void do_baton_undet_int_task(void)
{
	if (g_bat_init_flag != KAL_TRUE) {
		battery_log(BAT_LOG_CRTI,
			   "[do_baton_undet_int_task] battery thread not ready, will do after bettery init.\n");
		return;
	}
	...
	battery_log(BAT_LOG_CRTI, "[do_baton_undet_int_task] battery %s\n",
			BMT_status.bat_exist ? "exist" : "not exist");
}
#endif
...
void BAT_thread(void)
{
	...
	} else if (bat_spm_timeout || resume_running_kthread_timeout) {
		wakeup_fg_algo((FG_MAIN + FG_RESUME));
		bat_spm_timeout = FALSE;
		resume_running_kthread_timeout = FALSE;
	} else {
		wakeup_fg_algo(FG_MAIN);
	}
	mt_battery_charger_detect_check();
...
#ifdef CONFIG_LGE_PM
		/* avoid reporting too fast in bat_update_thread */
...
void check_battery_exist(void)
{
	...
	if (baton_count >= 3) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] No VBAT_Therm and Charging disable \n");
		battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
...
{
	resume_over_kthread_period = KAL_TRUE;
	bat_thread_wakeup();

	return HRTIMER_NORESTART;
}

enum hrtimer_restart battery_resume_running_kthread_hrtimer_func(struct hrtimer *timer)
{
	battery_log(BAT_LOG_CRTI, "battery_kthread_hrtimer_remaining_func : xxxx =====\n");
	return HRTIMER_NORESTART;
}
...
/*watchdog timer before 40 secs*/
	...
	hrtimer_init(&battery_resume_running_kthread_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	battery_resume_running_kthread_timer.function = battery_resume_running_kthread_hrtimer_func;
	resume_running_kthread_remaining_time = ktime_set(BAT_TASK_PERIOD, 0);
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	batt_cust_data.charger_configuration = CHARGER_CONFIG_SINGLE;
	__batt_parse_node(np, "charger_configuration",
		&batt_cust_data.charger_configuration);
#endif

#ifdef CONFIG_LGE_USB_TYPE_C
	/* default values for typec */
	batt_cust_data.typec_high_input_current = CHARGE_CURRENT_3000_00_MA;
	batt_cust_data.typec_high_charging_current = CHARGE_CURRENT_3000_00_MA;
	batt_cust_data.typec_medium_input_current = CHARGE_CURRENT_1500_00_MA;
	batt_cust_data.typec_medium_charging_current = CHARGE_CURRENT_1500_00_MA;

	__batt_parse_node(np, "typec_high_input_current",
		&batt_cust_data.typec_high_input_current);
	__batt_parse_node(np, "typec_high_charging_current",
		&batt_cust_data.typec_high_charging_current);
	__batt_parse_node(np, "typec_medium_input_current",
		&batt_cust_data.typec_medium_input_current);
	__batt_parse_node(np, "typec_medium_charging_current",
		&batt_cust_data.typec_medium_charging_current);
#endif
...
int batt_init_cust_data(void)
{
	...
	if (lge_get_modify_model()) {
#ifdef CONFIG_MACH_MT6750S_CV7A
		batt_cust_data.mtk_pump_express_plus_support = 0;
#endif
		batt_cust_data.ac_charger_current = CHARGE_CURRENT_1200_00_MA;
		batt_cust_data.ac_charger_input_current = CHARGE_CURRENT_1200_00_MA;
		batt_cust_data.charging_host_charger_current = CHARGE_CURRENT_1200_00_MA;
	}
#endif
...
static int battery_probe(struct platform_device *dev)
{
...
#ifdef CONFIG_LGE_PM
	battery_log(BAT_LOG_CRTI, "[BAT_probe] get_charging_control : done !!\n ");
#endif
...
#ifdef CONFIG_LGE_PM
	wake_lock_init(&battery_notify_lock, WAKE_LOCK_SUSPEND, "battery notify wakelock");
#endif

#ifdef CONFIG_LGE_PM_FLOATED_CHARGER_DETECT
	INIT_DELAYED_WORK(&floated_charger_confirm, mt_charger_confirm_floated_charger);
#endif
...
#endif
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Ship_Mode);
#endif
	}
	...
	BMT_status.battery_charging_enabled = KAL_TRUE;
	BMT_status.safety_timer_enabled = KAL_TRUE;
	BMT_status.fake_hvdcp_enabled = KAL_FALSE;
	BMT_status.input_current_max = 0;
	BMT_status.constant_charge_current_max = 0;
	BMT_status.constant_charge_voltage_max = 0;
#endif
#ifdef CONFIG_LGE_PM_CHARGERLOGO_WAIT_FOR_FG_INIT
	BMT_status.first_soc_est_done = 0;
#endif
#ifdef CONFIG_LGE_USB_TYPE_C
	BMT_status.typec_mode = POWER_SUPPLY_TYPEC_NONE;
#endif
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* don't need to run thread */
	{
		int hv = batt_cust_data.v_charger_max * 1000;
		int lv = batt_cust_data.v_charger_min * 1000;

		battery_charging_control(CHARGING_CMD_SET_HV_THRESHOLD, &hv);
		battery_charging_control(CHARGING_CMD_SET_LV_THRESHOLD, &lv);
	}
#else
...
static void battery_timer_pause(void)
{
	...
	resume_running_kthread_remaining_time = hrtimer_get_remaining(&battery_resume_running_kthread_timer);
	hrtimer_cancel(&battery_resume_running_kthread_timer);
	battery_log(BAT_LOG_CRTI, "[battery_timer_pause]hrtimer_cancel:");
	battery_log(BAT_LOG_CRTI, " resume_running_kthread_remaining_time %lld ms  xxxx\n",
		ktime_to_ms(resume_running_kthread_remaining_time));
...
kal_bool check_remaining_kthread_timeout(void)
{
	kal_bool ret = KAL_FALSE;
	s64 remaining_running_kthread_time;

	kthread_timeout_checked = KAL_TRUE;
	remaining_running_kthread_time = ktime_to_ms(resume_running_kthread_remaining_time);
	battery_log(BAT_LOG_CRTI, "[check_remaining_kthread_timeout]");
	battery_log(BAT_LOG_CRTI, " resume_running_kthread_remaining_time %lldms over_kthread_period %d  xxxx\n",
		ktime_to_ms(resume_running_kthread_remaining_time),
		resume_over_kthread_period);

	if (remaining_running_kthread_time > BAT_TASK_PERIOD * 1000 || remaining_running_kthread_time <= 0) {
		if (resume_over_kthread_period == KAL_FALSE)
			ret = KAL_TRUE;

		resume_running_kthread_remaining_time = ktime_set(BAT_TASK_PERIOD, 0);
	}

	resume_running_kthread_timeout = ret;
	battery_log(BAT_LOG_CRTI, "[check_remaining_kthread_timeout] return=%d xxxx\n", ret);
...
static void battery_timer_resume(void)
{
	...
	if (kthread_timeout_checked == KAL_FALSE)
		check_remaining_kthread_timeout();
	else
		battery_log(BAT_LOG_CRTI, "[check_remaining_kthread_timeout] kthread_timeout_checked=TRUE xxxx\n");

	battery_log(BAT_LOG_CRTI, "[battery_timer_resume] is_pcm_timer_trigger %d", is_pcm_timer_trigger);
	battery_log(BAT_LOG_CRTI, " bat_spm_timeout %d lbat %d running_kthread_timeout %d\n",
		bat_spm_timeout, battery_meter_get_low_battery_interrupt_status(),
		resume_running_kthread_timeout);

	if (is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout
		|| battery_meter_get_low_battery_interrupt_status() || resume_running_kthread_timeout) {
		mutex_lock(&bat_mutex);
		battery_meter_reset_sleep_time();
		BAT_thread();
		mutex_unlock(&bat_mutex);
...
/* don't need to check charger hv */
	...
	hrtimer_start(&battery_resume_running_kthread_timer, resume_running_kthread_remaining_time, HRTIMER_MODE_REL);
	resume_over_kthread_period = KAL_FALSE;
	kthread_timeout_checked = KAL_FALSE;
	battery_log(BAT_LOG_CRTI, "[battery_timer_pause] hrtimer_start xxxx\n");
...
static void battery_shutdown(struct platform_device *dev)
{
	if (mtk_pep_get_is_connect() || mtk_pep20_get_is_connect()) {
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
		{
			...
			battery_charging_control(CHARGING_CMD_ENABLE_SLAVE, &enable);
			battery_charging_control(CHARGING_CMD_ENABLE_POWER_PATH_SLAVE,
					&enable);
		}
#endif
