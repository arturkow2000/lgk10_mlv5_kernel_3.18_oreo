...
#ifdef CONFIG_LGE_PM
int Enable_FGADC_LOG = BM_DAEMON_DEFAULT_LOG_LEVEL;
#else
...
static signed int cust_min_uisoc_voltage = SHUTDOWN_SYSTEM_VOLTAGE;
...
/* function prototype */
/* ============================================================ // */
#define BAT_PROFILE_SIZE        110

struct battery_meter_table_custom_data {

	/* cust_battery_meter_table.h */
	int battery_profile_t0_size;
	BATTERY_PROFILE_STRUCT battery_profile_t0[BAT_PROFILE_SIZE];
	int battery_profile_t1_size;
	BATTERY_PROFILE_STRUCT battery_profile_t1[BAT_PROFILE_SIZE];
	int battery_profile_t2_size;
	BATTERY_PROFILE_STRUCT battery_profile_t2[BAT_PROFILE_SIZE];
	int battery_profile_t3_size;
	BATTERY_PROFILE_STRUCT battery_profile_t3[BAT_PROFILE_SIZE];
	int battery_profile_temperature_size;
	BATTERY_PROFILE_STRUCT battery_profile_temperature[BAT_PROFILE_SIZE];

	int r_profile_t0_size;
	R_PROFILE_STRUCT r_profile_t0[BAT_PROFILE_SIZE];
	int r_profile_t1_size;
	R_PROFILE_STRUCT r_profile_t1[BAT_PROFILE_SIZE];
	int r_profile_t2_size;
	R_PROFILE_STRUCT r_profile_t2[BAT_PROFILE_SIZE];
	int r_profile_t3_size;
	R_PROFILE_STRUCT r_profile_t3[BAT_PROFILE_SIZE];
	int r_profile_temperature_size;
	R_PROFILE_STRUCT r_profile_temperature[BAT_PROFILE_SIZE];
};
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
static struct device_node *battery_id_find_profile(struct device_node *np)
{
	struct power_supply *psy;
	union power_supply_propval id;
	const char *profile_name;
	struct device_node *node;

	psy = power_supply_get_by_name("battery_id");
	if (!psy)
		goto use_default_profile;

	psy->get_property(psy, POWER_SUPPLY_PROP_MANUFACTURER, &id);
	if (!id.strval)
		goto use_default_profile;

	if (of_property_read_string(np, id.strval, &profile_name))
		goto use_default_profile;

	node = of_find_node_by_name(np, profile_name);
	if (!node)
		goto use_default_profile;
...
use_default_profile:
	if (of_property_read_string(np, "UNKNOWN", &profile_name))
		return NULL;

	return of_find_node_by_name(np, profile_name);
}

#define BATTERY_ID_REPLACE_NODE(np, property) \
	{ \
		struct device_node *profile; \
		struct device_node *backup = np; \
		profile = battery_id_find_profile(np); \
		if (profile) \
			np = profile; \
		if (property && !of_find_property(np, property, NULL)) \
			np = backup; \
		if (profile && np != profile) \
			of_node_put(profile);

#define BATTERY_ID_RESTORE_NODE(np) \
		if (np != backup) \
			of_node_put(np); \
		np = backup; \
	}
...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
	BATTERY_ID_REPLACE_NODE(np, "batt_temperature_table_num");
#endif
	if (!of_property_read_u32(np, "batt_temperature_table_num", &val))
		size = (int)val;
	else
		bm_err("Get batt_temperature_table_num failed\n");

#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
	/* to prevent overflow,
	  reduce size to Batt_Temperature_Table array size */
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#else
	if (size)
		size = min(size, (int)ARRAY_SIZE(Batt_Temperature_Table));
#endif
#endif
	idx = 0;
	...
	if (idx != size * 2)
		bm_err("Get batt_temperature_table failed\n");
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
	BATTERY_ID_RESTORE_NODE(np);
#endif

#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
	BATTERY_ID_REPLACE_NODE(np, NULL);
#endif
	if (!of_property_read_u32(np, "battery_profile_t0_num", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t1_num", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t2_num", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t3_num", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t0_num", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t1_num", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t2_num", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t3_num", &val))
	...
	if (!of_property_read_u32(np, "difference_vbat_rtc", &val)) {
	...
	if (!of_property_read_u32(np, "difference_swocv_rtc_pos", &val)) {
	...
	if (!of_property_read_u32(np, "batterypseudo100", &val)) {
	...
	if (!of_property_read_u32(np, "batterypseudo1", &val)) {
	...
	if (!of_property_read_u32(np, "q_max_by_sys", &val)) {
	...
	if (!of_property_read_u32(np, "shutdown_system_voltage", &val)) {
	...
	if (!of_property_read_u32(np, "recharge_tolerance", &val)) {
	...
	if (!of_property_read_u32(np, "cust_tracking_gap", &val)) {
		batt_meter_cust_data.tracking_gap = (int)val;
		bm_debug("Get tracking_gap: %d\n",
			batt_meter_cust_data.tracking_gap);
	} else {
		bm_err("Get tracking_gap failed\n");
	}

	if (!of_property_read_u32(np, "cust_trackingoffset", &val)) {
		batt_meter_cust_data.trackingoffset = (int)val;
		bm_debug("Get trackingoffset: %d\n",
			batt_meter_cust_data.trackingoffset);
	} else {
		bm_err("Get trackingoffset failed\n");
	}

	if (!of_property_read_u32(np, "cust_trackingen", &val)) {
		batt_meter_cust_data.trackingen = (int)val;
		bm_debug("Get trackingen: %d\n",
			batt_meter_cust_data.trackingen);
	} else {
		bm_err("Get trackingen failed\n");
	}

	if (!of_property_read_u32(np, "rbat_pull_up_r", &val)) {
		g_RBAT_PULL_UP_R = (int)val;
		bm_err("Get rbat_pull_up_r: %d\n",
			g_RBAT_PULL_UP_R);
	} else {
		bm_err("Get rbat_pull_up_r failed\n");
	}

	if (!of_property_read_u32(np, "rbat_pull_up_volt", &val)) {
	...
	if (!of_property_read_u32(np, "difference_voltage_update", &val)) {
	...
	if (!of_property_read_u32(np, "aging1_load_soc", &val)) {
	...
	if (!of_property_read_u32(np, "aging1_update_soc", &val)) {
	...
	if (!of_property_read_u32(np, "charge_tracking_time", &val)) {
	...
	if (!of_property_read_u32(np, "discharge_tracking_time", &val)) {
	...
	if (!of_property_read_u32(np, "cust_min_uisoc_percentage", &val)) {
	...
	if (!of_property_read_u32(np, "cust_min_uisoc_voltage", &val)) {
		batt_meter_cust_data.cust_min_uisoc_voltage = (int)val;
		cust_min_uisoc_voltage = (int)val;
		bm_err("Get cust_min_uisoc_voltage: %d\n",
			batt_meter_cust_data.cust_min_uisoc_voltage);
	} else {
		bm_err("Get cust_min_uisoc_voltage failed");
		if (batt_meter_cust_data.shutdown_system_voltage != 0) {
			batt_meter_cust_data.cust_min_uisoc_voltage = batt_meter_cust_data.shutdown_system_voltage;
			cust_min_uisoc_voltage = batt_meter_cust_data.shutdown_system_voltage;
			bm_err("cust_min_uisoc_voltage==shutdown_system_voltage (%d)\n",
				batt_meter_cust_data.cust_min_uisoc_voltage);
		} else {
			batt_meter_cust_data.cust_min_uisoc_voltage = cust_min_uisoc_voltage;
			bm_err("Get cust_min_uisoc_voltage failed, default val=%d\n", cust_min_uisoc_voltage);
		}
	}
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= batt_temperature_table[0].TemperatureR) {
		...
		tbatt_temp_tc1 = ((((Res - RES2) * TMP1) + ((RES1 - Res) * TMP2)) * 10) / (RES1 - RES2);
	}
...
static int comp_temp_by_chg_current(int fg_temp, int first_ctemp)
{
	...
	int cnt, chg_current, avg_chg_current, temp, comp_temp;
	...
	if (is_batt_charging == true){
		battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT, &chg_current);
	}
	else {
	...
	for (cnt = (CHG_CURR_SAMPLE_COUNT-1); cnt >= 0; cnt--) {
		...
		bm_debug("chg_curr_data[%d] = %d\n", cnt, chg_curr_data[cnt]);
	}
	avg_chg_current = (chg_current / 10) / CHG_CURR_SAMPLE_COUNT;

	/* assume that temp is proportional to current*current */
	comp_delta = (avg_chg_current * avg_chg_current * COMP_FACTOR) / 10000000;
	comp_temp = temp - comp_delta;

	if (pre_delta / 10 != (fg_temp - comp_temp) / 10) {
		bm_info("fg_temp = %d comp_temp1 = %d comp_temp2 = %d "
			"delta1 = %d delta2 = %d avg_chg_current = %d "
			"now_chg_curr = %d batt_charging = %d pre_delta = %d\n ",
			fg_temp, temp, comp_temp,
			temp - fg_temp, comp_delta, avg_chg_current,
			chg_curr_data[0], is_batt_charging, pre_delta);
		pre_delta = fg_temp - comp_temp;
	} else
		bm_debug("fg_temp = %d  comp_temp1 = %d comp_temp2 = %d "
			 "delta1 = %d delta2 = %d avg_chg_current = %d "
			 "now_chg_curr = %d  batt_charging = %d pre_delta = %d\n",
			 fg_temp, temp, comp_temp,
			 temp - fg_temp, comp_delta, avg_chg_current,
...
static int calc_tuned_temp(int temp)
{
	...
	bm_info("comp_factor = %d, org temp : %d, delta temp : %d, report temp : %d\n",
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
		...
		tbatt_temp_tc1 = Batt_Temperature_Table[0].BatteryTemp * 10;
	} else if (Res <= Batt_Temperature_Table[i_max].TemperatureR) {
		TBatt_Value = Batt_Temperature_Table[i_max].BatteryTemp;
		tbatt_temp_tc1 = Batt_Temperature_Table[i_max].BatteryTemp * 10;
	} else {
		RES1 = Batt_Temperature_Table[0].TemperatureR;
		TMP1 = Batt_Temperature_Table[0].BatteryTemp;
		...
		tbatt_temp_tc1 = ((((Res - RES2) * TMP1) + ((RES1 - Res) * TMP2)) * 10) / (RES1 - RES2);
#ifdef CONFIG_LGE_PM_TEMP_BY_CHG
		tbatt_temp_tc1 = calc_tuned_temp(tbatt_temp_tc1);
		TBatt_Value = tbatt_temp_tc1 / 10;
#endif
	}
...
signed int fgauge_read_d_by_v(signed int volt_bat)
{
	...
	saddles = batt_meter_table_cust_data.battery_profile_t0_size;
...
signed int fgauge_read_r_bat_by_v(signed int voltage)
{
...
#ifdef CONFIG_LGE_PM
		/* WBT : do not reference profile_p if NULL */
		return 0;
#else
	...
	saddles = batt_meter_table_cust_data.r_profile_t0_size;
...
void fgauge_construct_battery_profile(signed int temperature,
		BATTERY_PROFILE_STRUCT_P temp_profile_p, R_PROFILE_STRUCT_P
		temp_r_profile_p)
{
	BATTERY_PROFILE_STRUCT_P low_profile_p, high_profile_p;
	R_PROFILE_STRUCT_P low_r_profile_p, high_r_profile_p;
	signed int low_temperature, high_temperature;
	int i = 0, j = 0, k = 0;
	signed int temp_v_1 = 0, temp_v_2 = 0;
	int temp_value = 0, temp_diff = 0, temp = 0;
	int saddles = 0, positive = 0;
	int temp_value_r = 0, temp_diff_r = 0, temp_r = 0;
	...
	if (temperature <= batt_meter_cust_data.temperature_t1) {
		...
		low_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0);
		high_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1);
		low_temperature = (-10);
		...
		low_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1);
		high_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2);
		...
		low_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2);
		high_r_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t3);
...
#ifdef CONFIG_LGE_PM
	/* WBT : do not reference profiles if NULL */
	if (!low_profile_p || !high_profile_p ||
			!low_r_profile_p || !high_r_profile_p) {
		bm_err("[fgauge_construct_battery_profile] failed with %d\n",
				temperature);
		return;
	}
#endif
    ...
    for (i = 0; i < saddles; i++) {
        if (((high_profile_p + i)->percentage > (low_profile_p + i)->percentage)
                || ((high_profile_p + i)->percentage == 100 &&
                ((high_profile_p + i)->percentage > (low_profile_p + i)->percentage))) {
            for (k = 1; k < (saddles - i); k++) {
                if ((high_profile_p + i)->percentage > (low_profile_p + (i +
                                k))->percentage) {
                    j = i;
                    break;
                } else {
        ...
        } else if ((high_profile_p + i)->percentage < (low_profile_p + i)->percentage) {
            for (k = 1; k < i; k++) {
                if ((high_profile_p + i)->percentage == (low_profile_p + (i -
                                k))->percentage) {
                    j = i - k;
                    break;
                } else if ((high_profile_p + i)->percentage > (low_profile_p + (i -
                                k))->percentage) {
                    j = i - k;
					positive = 1;
                    break;
        ...
        } else if (((high_profile_p + i)->percentage == 100)
                && ((low_profile_p + i)->percentage == 100)) {
            for (k = 1; k < i; k++) {
                if ((high_profile_p + i)->percentage > (low_profile_p + (i -
                                k))->percentage) {
                    j = i - (k - 1);
        ...
        if (((high_profile_p + i)->voltage) > ((low_profile_p + j)->voltage)) {
            temp_v_1 = (high_profile_p + i)->voltage;
			if (positive != 0) {
				temp_v_2 = (low_profile_p + j)->voltage - (((low_profile_p + j)->voltage - (low_profile_p
							+ (j + 1))->voltage) / 2);
				positive = 0;
			} else
				temp_v_2 = (low_profile_p + j)->voltage;

			temp_value = (temperature - low_temperature) * (temp_v_1 - temp_v_2);
			temp_diff = high_temperature - low_temperature;
			temp = (((temp_value * 10) / temp_diff) + 5) / 10;

            (temp_profile_p + i)->voltage = temp_v_2 + temp;
            (temp_r_profile_p + i)->voltage = temp_v_2 + temp;
			bm_debug("<DOD,Percentage> at %d = %d, %d <%d,%d>\r\n",
					temperature, i, j, (high_profile_p + i)->percentage,
					(low_profile_p + j)->percentage);
		} else {
			if (positive != 0) {
				temp_v_1 = (low_profile_p + j)->voltage - (((low_profile_p +
								j)->voltage - (low_profile_p
							+ (j + 1))->voltage) / 2);
				positive = 0;
			} else
				temp_v_1 = (low_profile_p + j)->voltage;

			temp_v_2 = (high_profile_p + i)->voltage;
			temp_value = (high_temperature - temperature) * (temp_v_1 - temp_v_2);
			temp_diff = high_temperature - low_temperature;
			temp = (((temp_value * 10) / temp_diff) + 5) / 10;
		...
		if (((high_r_profile_p + i)->resistance) > ((low_r_profile_p + i)->resistance)) {
			...
			temp_r_2 = (low_r_profile_p + j)->resistance;

			temp_value_r = (temperature - low_temperature) * (temp_r_1 - temp_r_2);
			temp_diff_r = high_temperature - low_temperature;
			temp_r = (((temp_value_r * 10) / temp_diff_r) + 5) / 10;

			(temp_r_profile_p + i)->resistance = temp_r_2 + temp_r;
		} else {
			temp_r_1 = (low_r_profile_p + j)->resistance;
			temp_r_2 = (high_r_profile_p + i)->resistance;

			temp_value_r = (high_temperature - temperature) * (temp_r_1 - temp_r_2);
			temp_diff_r = high_temperature - low_temperature;
			temp_r = (((temp_value_r * 10) / temp_diff_r) + 5) / 10;
...
#if 0
	for (i = 0; i < saddles; i++) {
		bm_err("<Rbat,VBAT> at %d = <%d,%d>\r\n",
			 temperature, (temp_r_profile_p + i)->resistance,
			 (temp_r_profile_p + i)->voltage);
	}
#endif
#if 0
	/* Dumpt new battery profile*/
	for (i = 0; i < saddles; i++) {
		bm_err("<DOD,Voltage> at %d = %d <%d,%d>\r\n",
			 temperature, i, (temp_profile_p + i)->percentage,
			 (temp_profile_p + i)->voltage);
	}
#endif
}

void fgauge_re_construct(signed int temperature)
{
	fgauge_construct_battery_profile(temperature,
			fgauge_get_profile(batt_meter_cust_data.temperature_t),
			fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t));
/*	fgauge_construct_r_table_profile(temperature,
			fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t));*/
}
...
signed int fgauge_compensate_battery_voltage(signed int ori_voltage)
{
	...
	signed int resistance_bat = 0;
	signed int now_current = 0;
	int ret = 0;

	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT, &now_current);
	resistance_bat = fgauge_read_r_bat_by_v(temp_voltage_2);	/* Ohm */
	ret_compensate_value = (now_current * (resistance_bat + g_R_FG_VALUE +
	...
	bm_err("[fgauge_compensate_battery_voltage] %d,%d,%d,%d\r\n",
		 temp_voltage_1, temp_voltage_2, resistance_bat, ret_compensate_value);
...
signed int lge_ocv_compensate(int rtc_soc, int voltage) {
	...
	/* do not compensate when rtc_soc is not valid */
	if (!rtc_soc)
		return 0;
	...
	if (get_boot_reason() == BR_POWER_KEY) {
		...
		if (diff_rtc_swocv >= DIFF_RTC_SWOCV_M && diff_rtc_swocv < DIFF_RTC_SWOCV_H) {
			...
			if (swocv > rtc_soc)
				...
				compensate_rtc_soc = rtc_soc - COMPENSATE_VAL_M;
		} else if (diff_rtc_swocv < DIFF_RTC_SWOCV_L) {
			compensate_rtc_soc = rtc_soc;
		}
	}
...
/* 1. Get Raw Data */
	...
	/* do not need to enable charging here */
#else
	charging_enable = KAL_TRUE;
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
#endif
#ifdef CONFIG_LGE_PM
	/* clear rtc fg soc entering recovery boot with non-reboot reason */
	if (get_boot_mode() == RECOVERY_BOOT) {
		if (get_boot_reason() == BR_POWER_KEY || get_boot_reason() == BR_USB) {
			bm_info("[fgauge_algo_run_get_init_data] RECOVERY_BOOT and Power key or USB/TA boot clear RTC value\n");
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
static int lge_battery_id_check(void)
{
	struct power_supply *psy;
	static int count = 0;
	...
	bm_info("[battery_meter_probe] defered %d\n", ++count);
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	if (g_sleep_total_time.tv_sec < g_spm_timer) {
		if (wake_up_smooth_time == 0) {
			if (bat_is_charger_exist() == KAL_FALSE) {
				/* self_correct_dod_scheme(duration_time); */
				if (check_remaining_kthread_timeout() == KAL_FALSE)
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
			/* use SW_OCV when boot with USB or TA */
			if ((bat_is_charger_exist() || get_boot_reason() == BR_USB)
					&& !g_battery_soc_ready){
				voltage = gFG_voltage;
				bm_info("[fg_res] use SW_OCV when boot with USB or TA\n");
			}
#endif
	...
	case FG_DAEMON_CMD_GET_BATTERY_PLUG_STATUS:
		{
			...
			battery_meter_ctrl(BATTERY_METER_CMD_GET_BATTERY_PLUG_STATUS,
					   &plugout_status);
#ifdef CONFIG_LGE_PM
			if (batt_meter_cust_data.embedded_battery){
				plugout_status = 0;
			} else {
	...
	case FG_DAEMON_CMD_GET_RTC_SPARE_FG_VALUE:
		{
			...
			if (!g_battery_soc_ready && rtc_fg_soc != 0) {
				...
				fgauge_re_construct(temperature);
	...
	case FG_DAEMON_CMD_GET_CUSTOM_SETTING:
		{
			...
			{
				static int pseudo100_cust_data = 0;
				int uisoc2_offset = 0;

				if (!pseudo100_cust_data) {
					pseudo100_cust_data =
						batt_meter_cust_data.batterypseudo100;
				}

				uisoc2_offset = get_battery_cycle_offset();
				if (uisoc2_offset) {
					batt_meter_cust_data.batterypseudo100 =
						pseudo100_cust_data - uisoc2_offset;
				}

				bm_err("[CCD] changed batterypseudo100 = %d (%d-%d)\n",
						batt_meter_cust_data.batterypseudo100,
						pseudo100_cust_data, uisoc2_offset);
			}
#endif
...
#ifdef CONFIG_LGE_PM_MTK_SAVE_BATTERY_CYCLE_GM_2_0
			battery_cycle_update(BMT_status.SOC);
#endif
		}
		break;
