...
static void set_debug_reason(struct device *dev, int type)
{
	...
	lg4894_reg_write(dev, TCI_DEBUG_FAIL_BIT_CTRL, &wdata1, sizeof(wdata1));
...
#ifdef LG4894_TCL_OFF_VIA_MIPI
	struct touch_core_data *ts = to_touch_core(dev);
#if 0 //[Bringup]temp block for mtk Lg4894
...
static int lg4894_notify(struct device *dev, ulong event, void *data)
{
	...
	case LCD_EVENT_TOUCH_RESET_START:
		...
		touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
		touch_gpio_direction_output(ts->reset_pin, GPIO_OUT_ZERO);
		break;
	case LCD_EVENT_TOUCH_RESET_END:
		atomic_set(&ts->state.hw_reset, event);

		TOUCH_I("LCD_EVENT_TOUCH_RESET_END!\n");
		touch_gpio_direction_output(ts->reset_pin, GPIO_OUT_ONE);
...
#if defined (CONFIG_MACH_MT6755M_K6P) || defined (CONFIG_MACH_MT6750_LGPS32) || defined (CONFIG_MACH_MT6750_LV5)
...
static int lg4894_upgrade(struct device *dev)
{
	...
	if (ts->test_fwpath[0]) {
		...
		if (d->fw.lcd_fpcb_revision < LCD_FPCB_REV_4) {
			memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath));
		} else if (d->fw.lcd_fpcb_revision == LCD_FPCB_REV_4) {
			memcpy(fwpath, ts->def_fwpath[1], sizeof(fwpath));
		} else if (d->fw.lcd_fpcb_revision >= LCD_FPCB_REV_5) {
...
static int lg4894_init(struct device *dev)
{
	...
	data = atomic_read(&ts->state.qmemo);
	...
	ret = lg4894_reg_write(dev, REG_CALL_STATE, &data, sizeof(data));
	if (ret)
...
int lg4894_check_status(struct device *dev)
{
	...
	if ((status_mask & INT_GLOBAL_RESET_CLR_BIT) || (ic_status & (1 << 0))) {
		TOUCH_I("%s : Need Global Reset, status = %x, ic_status = %x\n",
			__func__, status, ic_status);
		ret = -ERESTART;
	} else if ((status_mask & INT_HW_RESET_CLR_BIT) || (ic_status & (1 << 3))) {
...
#if defined (CONFIG_MACH_MT6755M_K6P) || defined (CONFIG_LGE_MODULE_DETECT)
...
static int __init touch_device_init(void)
{
	...
	TOUCH_I("4894__[%s] get_maker_id = %d \n", __func__, lcm_id);
	if(lcm_id != 0)
	{
		TOUCH_I("%s, NOT lg4894 panel\n", __func__);
		return 0;
	}
#endif
#endif
