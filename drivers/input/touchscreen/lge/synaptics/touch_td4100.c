...
/*check status for esd recovery*/
bool esd_recovery_state = false;
...
static void synaptics_palm_filter(struct device *dev)
{
	...
	if (!synaptics_is_product(d, "PLG632", 6) || !d->use_palm_filter)
		return;
...
static int synaptics_change_ime_status(struct device *dev)
{
	...
	if (synaptics_is_product(d, "PLG632", 6)) { /* Use only SF3 */
...
static int synaptics_closed_cover_set(struct device *dev)
{
	...
	if (synaptics_is_product(d, "PLG632", 6))	/* SF3 don't use closed cover */
		return ret;
...
static int synaptics_swipe_debug(struct device *dev)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	...
	synaptics_read(dev, SWIPE_FAIL_COUNT_REG, &count, sizeof(count));
	synaptics_read(dev, SWIPE_FAIL_INDEX_REG, &index, sizeof(index));

	for (i = 1; i <= count; i++) {
		addr = SWIPE_FAIL_BUFFER_REG +
			((index + SWIPE_MAX_BUFFER - i) % SWIPE_MAX_BUFFER);
		synaptics_read(dev, addr, &buf, sizeof(buf));
		TOUCH_I("SWIPE-Fail[%d/%d] : %s\n", count - i + 1, count, swipe_f_str[buf-1]);
	}
...
static int synaptics_swipe_start_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	u8 buffer[8];
	...
	TOUCH_TRACE();
	TOUCH_I("SWIPE START_AREA SET: x1[%4d], y1[%4d], x2[%4d], y2[%4d]\n",
		ts->swipe.info.start.x1, ts->swipe.info.start.y1, ts->swipe.info.start.x2, ts->swipe.info.start.y2);

	buffer[0] = (ts->swipe.info.start.x1 >> 0) & 0xff;
	buffer[1] = (ts->swipe.info.start.x1 >> 8) & 0xff;
	buffer[2] = (ts->swipe.info.start.y1 >> 0) & 0xff;
	buffer[3] = (ts->swipe.info.start.y1 >> 8) & 0xff;
	buffer[4] = (ts->swipe.info.start.x2 >> 0) & 0xff;
	buffer[5] = (ts->swipe.info.start.x2 >> 8) & 0xff;
	buffer[6] = (ts->swipe.info.start.y2 >> 0) & 0xff;
	buffer[7] = (ts->swipe.info.start.y2 >> 8) & 0xff;

	synaptics_write(dev, SWIPE_START_AREA, buffer, sizeof(buffer));

	return 0;
}

static int synaptics_swipe_active_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	u8 buffer[8];
	...
	TOUCH_TRACE();
	TOUCH_I("SWIPE ACTIVE_AREA SET: x1[%4d], y1[%4d], x2[%4d], y2[%4d]\n",
		ts->swipe.info.area.x1, ts->swipe.info.area.y1, ts->swipe.info.area.x2, ts->swipe.info.area.y2);

	buffer[0] = (ts->swipe.info.area.x1 >> 0) & 0xff;
	buffer[1] = (ts->swipe.info.area.x1 >> 8) & 0xff;
	buffer[2] = (ts->swipe.info.area.y1 >> 0) & 0xff;
	buffer[3] = (ts->swipe.info.area.y1 >> 8) & 0xff;
	buffer[4] = (ts->swipe.info.area.x2 >> 0) & 0xff;
	buffer[5] = (ts->swipe.info.area.x2 >> 8) & 0xff;
	buffer[6] = (ts->swipe.info.area.y2 >> 0) & 0xff;
	buffer[7] = (ts->swipe.info.area.y2 >> 8) & 0xff;

	synaptics_write(dev, SWIPE_ACTIVE_AREA, buffer, sizeof(buffer));

	return 0;
}

static void synaptics_swipe_area_set(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	...
	ts->swipe.info.area.x1 = 39;
	ts->swipe.info.area.y1 = 39;
	ts->swipe.info.area.x2 = 680;
	ts->swipe.info.area.y2 = 1279;
	synaptics_swipe_active_area(dev);
	
	ts->swipe.info.start.x1 = 220;
	ts->swipe.info.start.y1 = 1135;
	ts->swipe.info.start.x2 = 500;
	ts->swipe.info.start.y2 = 1279;
	synaptics_swipe_start_area(dev);

	synaptics_set_page(dev, DEFAULT_PAGE);

}

static void synaptics_swipe_control(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	u8 buffer;
	...
	buffer = ts->swipe.mode;
	buffer1 = 100;
	synaptics_set_page(dev, LPWG_PAGE);

	synaptics_write(dev, SWIPE_REPORT_ENABLE, &buffer, sizeof(buffer));
	synaptics_write(dev, SWIPE_WRONG_TH, &buffer1, sizeof(buffer1));
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->role.mfts_lpwg) {
...
#if defined(CONFIG_LGD_INCELL_TD4100_HD_SF3) /*only use SF3*/
			touch_msleep(20);
			rt4832_dsv_ctrl(0); /*DSV_ENREG disable - Discharge*/
#endif
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				synaptics_sleep_control(dev, IC_NORMAL);
#if defined(CONFIG_LGD_INCELL_TD4100_HD_SF3) /*only use SF3*/
				rt4832_dsv_ctrl(1); /*DSV_ENREG enable - Charge*/
#endif
			}
			synaptics_tci_area_set(dev, QUICKCOVER_OPEN);
			synaptics_swipe_area_set(dev);
			synaptics_swipe_control(dev);
			synaptics_lpwg_control(dev, ts->lpwg.mode);
...
static void synaptics_init_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ts->swipe.info.init_rat_chk_dist = 5;
...
static int synaptics_irq_status(struct device *dev, u8 dev_status)
{
	...
	if (!synaptics_is_product(d, "PLG632", 6)) { /* Receive esd state info from display except for sf3 */
		/* ESD status from Display, 0:end, 1:run*/
		atomic_set(&d->state.recovery, esd_recovery_state);
	}

	if (dev_status == 0x81) {
		if (atomic_read(&d->state.init) == INIT_RUN
			|| atomic_read(&d->state.recovery) == ESD_RECOVERY_RUN) {
			TOUCH_I("Touch Initialize or ESD recovery is not over yet\n");
			return 0;
		}
		/* Call FB blank event from display (only sf3)*/
		if (synaptics_is_product(d, "PLG632", 6))
			goto ESD_RECOVERY; /* ASIC reset (Unknown reset) Status */

		 /*update set configured after ESD recovery*/
		if (atomic_read(&d->state.esd_error) == ESD_ERROR) {
			TOUCH_I("Set configured. dev_status : 0x%x\n", dev_status);
			data = 0x80; // Set configured
			ret = synaptics_write(dev, DEVICE_CONTROL_REG, &data, sizeof(data));
			ret |= synaptics_read(dev, DEVICE_STATUS_REG, &dev_status, sizeof(dev_status));
			if (ret < 0) {
				TOUCH_E("Set configured error\n");
				return ret;
			}
			atomic_set(&d->state.esd_error, ESD_NONE);
			TOUCH_I("[%s] device_status bit cleared : 0x%x\n", __func__, dev_status);
		} else {
			/* ASIC reset (Unknown reset) Status */
			goto ESD_RECOVERY;
		}
	} else if ((dev_status & 0x0F) == 0x09) {
...
ESD_RECOVERY:
	...
	if (atomic_read(&d->state.recovery) == ESD_RECOVERY_RUN) {
	...
	if (!synaptics_is_product(d, "PLG632", 6)) { /* Receive esd state info from display except for sf3 */
		esd_recovery_state = atomic_read(&d->state.recovery);
		atomic_set(&d->state.esd_error, ESD_ERROR);
	}
	return -EGLOBALRESET;
}
...
static int synaptics_get_cmd_version(struct device *dev, char *buf)
{
	...
	if (synaptics_is_product(d, "PLG632", 6) || synaptics_is_product(d, "PLG635", 6)
			|| synaptics_is_product(d, "PLG640", 6) || synaptics_is_product(d, "PLG655", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : TD4100\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
...
static ssize_t store_swipe_enable(struct device *dev,const char *buf, size_t count)
{
	...
	ts->swipe.mode = value ? 1 : 0;

	TOUCH_I("Swipe mode = %s\n", ts->swipe.mode == 1 ? "SWIPE_UP" : "Disable");
...
static TOUCH_ATTR(swipe_enable, NULL, store_swipe_enable);
