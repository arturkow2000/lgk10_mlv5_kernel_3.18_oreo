...
static void sw49106_get_swipe_info(struct device *dev)
{
	...
	d->swipe.info.area.x2 = 1079;
	d->swipe.info.area.y2 = 2159;
	d->swipe.info.start.x1 = 336;
	d->swipe.info.start.y1 = 1914;
	d->swipe.info.start.x2 = 744;
	d->swipe.info.start.y2 = 2159;
...
{
	struct sw49106_data *d = to_sw49106_data(dev);
	struct sw49106_swipe_info *up = &d->swipe.info;
...
#if defined(LGE_DISPLAY_RECOVERY_ESD)
...
static void sw49106_int_pin_work_func(struct work_struct *int_pin_work)
{
	...
	if((int_pin_state==0) && (d->lcd_mode == LCD_MODE_U3)){
		...
		primary_display_esd_report_touchintpin_keep_low();
	}
}
#endif
...
static int sw49106_notify_charger(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49106_data *d = to_sw49106_data(dev);
	int ret = 0;
...
static int sw49106_notify(struct device *dev, ulong event, void *data)
{
	...
	if (touch_boot_mode() == TOUCH_CHARGER_MODE) {
		...
		return sw49106_notify_charger(dev,event,data);
	}
...
static int sw49106_upgrade(struct device *dev)
{
	...
	if (ts->test_fwpath[0]) {
		memcpy(fwpath, &ts->test_fwpath[0], sizeof(fwpath)-1);
		TOUCH_I("get fwpath from test_fwpath:%s\n",
			&ts->test_fwpath[0]);
	} else if (ts->def_fwcnt) {
		/*if (d->fw.revision <= REV_6) {*/
			memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath)-1);
		/*} else if (d->fw.revision == REV_8) {
			memcpy(fwpath, ts->def_fwpath[1], sizeof(fwpath)-1);
		} else if (d->fw.revision == REV_9) {
			memcpy(fwpath, ts->def_fwpath[2], sizeof(fwpath)-1);
		}*/
...
int sw49106_check_status(struct device *dev)
{
		...
		if(ABNORMAL_IC_BIT == status){
           ...
           // Add Global reset due to abnormal status
