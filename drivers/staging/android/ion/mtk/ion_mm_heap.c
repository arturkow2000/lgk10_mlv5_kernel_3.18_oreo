...
static unsigned int m_highorder_gfp_flags = (GFP_HIGHUSER | __GFP_ZERO
		| __GFP_NOWARN | __GFP_NORETRY | __GFP_NO_KSWAPD | __GFP_HIGHORDER) & ~__GFP_WAIT;
...
static struct page *alloc_buffer_highorder_page(struct ion_system_heap *heap,
	...
	if (!page) {
		pr_err_ratelimited("[ion_dbg] alloc_pages order=%lu in highorder\n", order);
		return 0;
	}
...
int ion_mm_heap_pool_total(struct ion_heap *heap)
{
	...
	for (i = 0; i < num_orders; i++) {
		...
		total += (atomic_read(&pool->high_count) + atomic_read(&pool->low_count)) * (1 << pool->order);
	...
	*sys_heap = container_of(heap, struct ion_system_heap, heap);
	...
	for (i = 0; i < num_highorders; i++) {
		...
		ION_PRINT_LOG_OR_SEQ(s,
				...
				(1 << pool->order) * PAGE_SIZE * atomic_read(&pool->high_count), dev, heap->id);
		ION_PRINT_LOG_OR_SEQ(s,
...
int ion_mm_heap_for_each_pool(int (*fn)(int high, int order, int cache,
	...
	*sys_heap = container_of(heap, struct ion_system_heap, heap);
	...
	for (i = 0; i < num_orders; i++) {
		...
		fn(1, pool->order, 0, (1 << pool->order) * PAGE_SIZE * atomic_read(&pool->high_count));
		fn(0, pool->order, 0, (1 << pool->order) * PAGE_SIZE * atomic_read(&pool->low_count));

		pool = sys_heap->cached_pools[i];
		fn(1, pool->order, 1, (1 << pool->order) * PAGE_SIZE * atomic_read(&pool->high_count));
		fn(0, pool->order, 1, (1 << pool->order) * PAGE_SIZE * atomic_read(&pool->low_count));
	}
	return 0;
}
