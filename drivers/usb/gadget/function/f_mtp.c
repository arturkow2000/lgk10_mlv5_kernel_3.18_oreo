...
#endif	/* CONFIG_LGE_USB_G_MULTIPLE_CONFIGURATION */
...
static int mtp_receive_cancel_event(struct mtp_dev *dev, struct mtp_event *event) {
	...
	/* DBG(dev->cdev, "mtp_receive_cancel_event(%zu)\n", event->length); */
...
#ifdef CONFIG_LGE_USB_G_ANDROID
	if (!_mtp_dev->mtp_binded) {
		pr_err("%s mtp is not binded\n", __func__);
		return -EPERM;
	}
#endif
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		if (ctrl->bRequest == MTP_REQ_CANCEL && w_index == 0
			...
			#ifdef CONFIG_LGE_USB_G_ANDROID
			request_cancel_count++;
			pr_info("got MTP_REQ_CANCEL. request_cancel_count : %d, dev_state : %d\n", request_cancel_count, dev->state);
			#endif
