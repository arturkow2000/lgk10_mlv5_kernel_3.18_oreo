...
static int android_enable(struct android_dev *dev)
{
	...
	#ifdef CONFIG_LGE_USB_G_ANDROID
	pr_notice("%s: check disable_depth(%d)\n", __func__, dev->disable_depth);
	#endif
...
#ifdef CONFIG_LGE_USB_FACTORY
	if (dev->check_pif) {
		pr_notice("%s: pif cable is plugged, not permitted\n", __func__);
		return;
	}
#endif
...
static void ffs_function_enable(struct android_usb_function *f)
{
	...
	#ifdef CONFIG_LGE_USB_G_ANDROID
	pr_notice("%s: config->enabled(%d)\n", __func__, config->enabled);
	#endif
...
static int functionfs_ready_callback(struct ffs_data *ffs)
{
	...
	#ifdef CONFIG_LGE_USB_G_ANDROID
	pr_notice("%s: config->enabled(%d), dev(%p)\n", __func__, config->enabled, dev);
	#endif
...
#ifdef CONFIG_LGE_USB_G_ANDROID
	/*1st:ATD, 2nd:Modem for ETS, 3rd:X, 4th:MD logger for ELT*/
	for (i = 0; i < MAX_ACM_INSTANCES; i++) {
		config->port_index_on[i] = 0;
	}
#endif
}
...
#ifdef CONFIG_LGE_USB_G_ANDROID
	// if instances is used, the port_index should not be used
	/*1st:ATD, 2nd:Modem for ETS, 3rd:X, 4th:MD logger for ELT*/
	for (i = 0; i < MAX_ACM_INSTANCES; i++) {
		config->port_index[i] = 0;
	}
...
#ifdef CONFIG_LGE_USB_G_ANDROID
	// if port is used, the instances should not be used
	config->instances = 0;
#endif
...
serial_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	...
	#ifdef CONFIG_LGE_USB_G_ANDROID
	pr_notice("%s Open /dev/ttyGS%d\n", __func__, MAX_ACM_INSTANCES + config->port_num);
	#else
...
#if !defined (CONFIG_USBIF_COMPLIANCE) && defined (CONFIG_LGE_USB_G_ANDROID)
			    ...
			    const char *buff, size_t size)
{
	...
	if (dev->check_pif) {
		if (enabled && !dev->enabled) {
			...
			dev->enabled = true;
			mutex_unlock(&dev->mutex);
			pr_notice("%s: pif cable is plugged, enable\n", __func__);
			return size;
		}
		mutex_unlock(&dev->mutex);
		pr_notice("%s: pif cable is plugged, not permitted\n", __func__);
...
#endif
	if (enabled && !dev->enabled) {
		/* Fix the problem of usb midi mode */
		#ifndef CONFIG_LGE_USB_G_ANDROID
		/* ALPS01770952
...
#ifdef CONFIG_LGE_USB_FACTORY
	if (dev->check_pif) {
		pr_notice("%s: pif cable is plugged, bind factory composition\n",
				__func__);
