...
static void sysrq_handle_full_pmic_reset(int key)
{
	extern void pmic_full_reset(void);

	pr_err("call full pmic reset\n");
...
static struct sysrq_key_op sysrq_full_pmic_reset_op = {
	.handler	= sysrq_handle_full_pmic_reset,
	.help_msg	= "pmic-full-reset(x)",
	.action_msg	= "Do full pmic reset",
	.enable_mask = SYSRQ_ENABLE_FULL_PMIC_RESET,
};
#endif
