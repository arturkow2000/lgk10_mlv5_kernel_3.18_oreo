 ...
 * Buffer hash
 */
#define DM_BUFIO_HASH_BITS	CONFIG_DM_BUFIO_HASH_BITS
...
void* dm_direct_read(sector_t block, struct dm_bufio_client *bufio)
{
	...
	if(data) {
		...
		printk(KERN_ERR "%s block:%d, data:%p(page:%p)(phys:%llx)\n",__func__, (int)block, data, (void*)virt_to_page(data), virt_to_phys(data));
	}
return_data:
	return data;
}
...
{
	printk(KERN_ERR "%s data:%p(page:%p)(phys:%llx)\n",__func__, data, (void*)virt_to_page(data), virt_to_phys(data));
	if(data)
		free_pages((unsigned long)data, 0);
}
