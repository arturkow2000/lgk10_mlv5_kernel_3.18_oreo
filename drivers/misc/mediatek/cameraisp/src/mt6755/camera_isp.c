...
#if 1 //blocked by LGE, soojong.jin@lge.com
				    ...
				    unsigned long long sec, unsigned long usec, MBOOL bDrop)
{
	...
	if (0 == pstRTBuf->dropCnt) {
		...
		/* last update buffer index     */
/*MTK_CHANGE_S, 2018-01-30, ALPS03749066, to avoid division by zero, soojong.jin@lge.com*/
        if(camsv_fbc.Bits.FB_NUM == 0)
        {
            camsv_fbc.Reg_val = ISP_RD32(ISP_REG_ADDR_CAMSV2_IMGO_FBC);
            LOG_ERR("ISP_CAMSV_SOF_Buf_Get camsv_fbc.Bits.FB_NUM reload (%d)\n", camsv_fbc.Bits.FB_NUM);
        }
        ...
        {
            LOG_ERR("ISP_CAMSV_SOF_Buf_Get camsv_fbc.Reg_val reload (%x)\n", camsv_fbc.Reg_val);
        }
/*MTK_CHANGE_E, 2018-01-30, ALPS03749066, to avoid division by zero, soojong.jin@lge.com*/
...
#define	STATUSX_WARNING	(ISP_IRQ_STATUSX_ESFKO_ERR_ST| \
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT_STATUSX] & IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT_STATUSX]) {
		...
		/* } */
/*MTK_CHANGE_S, 2018-01-19, ALPSALPS03683005, Removed ISP Overrun worning log, soojong.jin@lge.com*/
		if ((IrqStatus[ISP_IRQ_TYPE_INT_STATUSX] &
		IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT_STATUSX]) & (~STATUSX_WARNING)) {
...
/*MTK_CHANGE_E, 2018-01-19, ALPSALPS03683005, Removed ISP Overrun worning log, soojong.jin@lge.com*/
	}
