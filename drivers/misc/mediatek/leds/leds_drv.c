...
{
	struct led_classdev *led_cdev = &g_leds_data[MT65XX_LED_TYPE_LCD]->cdev;

	led_cdev->max_brightness = max_level;
	led_set_brightness(led_cdev, led_cdev->max_brightness);
	pr_info("[JJB] %s: Change : %d\n", __func__, led_cdev->max_brightness);
}
...
{
	struct led_classdev *led_cdev = &g_leds_data[MT65XX_LED_TYPE_LCD]->cdev;
	int max_brightness_data = 0;

	max_brightness_data = led_cdev->max_brightness;
	//pr_info("[JJB] %s: brightness = %d \n", __func__, max_brightness_data);
	return max_brightness_data;
}

int get_brightness(void)
{
	struct led_classdev *led_cdev = &g_leds_data[MT65XX_LED_TYPE_LCD]->cdev;
	int brightness_data = 0;

	brightness_data = led_cdev->brightness;
	//pr_info("[JJB] %s: brightness = %d \n", __func__, brightness_data);
...
static ssize_t store_led_pattern(struct device *dev,struct device_attribute *attr, const char *buf,size_t size)
{
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	LEDS_DRV_DEBUG("[LED] store_led_blink:rgb[%x],delay on[%d], "
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	if(get_boot_mode() == NORMAL_BOOT) {
		mt_mt65xx_breath_mode(1);
	} else if(get_boot_mode() == CHARGER_BOOT) {
		mt_mt65xx_breath_mode(0);
	} else {
		// if miniOS or laf downlod, turn off LED
		mt_led_off();
	}
#endif
