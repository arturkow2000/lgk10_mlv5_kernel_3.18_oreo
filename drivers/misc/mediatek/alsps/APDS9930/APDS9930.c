...
static const struct of_device_id alsps_of_match[] = {
	{.compatible = "mediatek,alsps"},	/* mediatek,apds9930 */
	{},
};
#endif
/*----------------------------------------------------------------------------*/
...
static int APDS9930_get_ps_caldata(struct i2c_client *client)
{
	...
	if (0 == obj->ps_cali) {	/* if.. doesn't init on lgpserver */
		...
		set_psensor_threshold(obj->client, obj->hw->ps_threshold_low,
...
struct apds9930_hw *get_apds9930_dts_func(const char *name, struct apds9930_hw *hw)
{
	...
	if (node) {
		ret =
		    of_property_read_u32_array(node, "near_offset", near_offset,
					       ARRAY_SIZE(near_offset));
		if (ret == 0)
			hw->near_offset = near_offset[0];

		ret =
		    of_property_read_u32_array(node, "far_offset", far_offset,
					       ARRAY_SIZE(far_offset));
		if (ret == 0)
			hw->far_offset = far_offset[0];

		ret =
		    of_property_read_u32_array(node, "crosstalk_max", crosstalk_max,
					       ARRAY_SIZE(crosstalk_max));
		if (ret == 0)
		...
		ret =
		    of_property_read_u32_array(node, "ps_led_current", ps_led_current,
					       ARRAY_SIZE(ps_led_current));
		if (ret == 0)
			hw->ps_led_current = ps_led_current[0];

		ret =
		    of_property_read_u32_array(node, "als_threshold", als_threshold,
					       ARRAY_SIZE(als_threshold));
		if (ret == 0)
		...
		ret =
		    of_property_read_u32_array(node, "als_coe_b", als_coe_b, ARRAY_SIZE(als_coe_b));
		if (ret == 0)
			hw->als_coe_b = als_coe_b[0];
		ret =
		    of_property_read_u32_array(node, "als_coe_c", als_coe_c, ARRAY_SIZE(als_coe_c));
		if (ret == 0)
			hw->als_coe_c = als_coe_c[0];

		ret =
		    of_property_read_u32_array(node, "als_coe_d", als_coe_d, ARRAY_SIZE(als_coe_d));
		if (ret == 0)
			hw->als_coe_d = als_coe_d[0];

		ret = of_property_read_string(node, "vdd", (char const **)&vdd);
		if (ret == 0)
			hw->vdd = vdd;
...
static long APDS9930_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (enable) {
		...
		set_psensor_threshold(client, obj->hw->ps_threshold_low,
				      obj->hw->ps_threshold_high);
		/*fix bug */
...
int APDS9930_irq_registration(struct i2c_client *client)
{
	...
	ret =
	    request_irq(alsps_irq, alsps_interrupt_handler, IRQF_TRIGGER_FALLING, "ALS-eint", NULL);
...
int APDS9930_read_als(struct i2c_client *client, u16 *data)
{
	...
	IAC2 =
	    obj->local_hw->als_coe_c * c0_value / 100 - obj->local_hw->als_coe_d * c1_value / 100;
#ifdef APS_DEBUG
	...
	luxValue =
	    ((IAC * obj->local_hw->als_ga * APDS9930_DF) / 100 /
	     (apds9930_als_integration_tb[obj->als_atime_index] / 100 *
	      apds9930_als_again_tb[obj->als_again_index]));

#ifdef CONFIG_LGE_SENSOR_DYNAMIC_AGAIN
	if (obj->als_reduce) {
		luxValue =
		    (((IAC * obj->local_hw->als_ga * APDS9930_DF) / 100) * 60) / 10 /
		    (apds9930_als_integration_tb[obj->als_atime_index] / 100 *
		     apds9930_als_again_tb[obj->als_again_index]);
	}
#endif
...
static int APDS9930_do_calibration(struct i2c_client *client)
{
	...
	/* we should store it to storage ( it should be free from factory reset ) */
	/* but ATCI Demon will store it through LGP Demon */
	return 0;
}
	...
	if (res == ALS_INT || res == PS_ALS_INT) {
		...
		if (atomic_read(&driver_suspend_flag) == 1) {
			als_pocket_threshold =
			    obj->local_hw->als_threshold *
			    apds9930_als_again_tb[obj->als_again_index] /
			    apds9930_als_again_tb[ALS_DEFAULT_GAIN];
			...
			APS_ERR("normal mode\n");
			als_low_threshold =
			    (als_ch0 * (100 - APDS9930_ALS_THRESHOLD_HSYTERESIS)) / 100;
			als_high_threshold =
			    (als_ch0 * (100 + APDS9930_ALS_THRESHOLD_HSYTERESIS)) / 100;
			...
			if (als_ch0 >= apds9930_als_res_tb[obj->als_atime_index] ||
				...
				set_asensor_threshold(obj->client, als_low_threshold,
						      als_high_threshold);
			}
#else
...
static ssize_t APDS9930_store_cali_value(struct device_driver *dev, char *buf, size_t count)
{
	...
	ret = APDS9930_do_calibration(client);
...
static ssize_t APDS9930_store_ps_led(struct device_driver *dev, char *buf, size_t count)
{
	struct i2c_client *client = APDS9930_i2c_client;
	struct APDS9930_priv *obj = i2c_get_clientdata(client);
...
static ssize_t APDS9930_store_ps_pulse(struct device_driver *dev, char *buf, size_t count)
{
	struct i2c_client *client = APDS9930_i2c_client;
	struct APDS9930_priv *obj = i2c_get_clientdata(client);
...
static ssize_t APDS9930_store_ps_enable(struct device_driver *dev, char *buf, size_t count)
{
	struct i2c_client *client = APDS9930_i2c_client;
	unsigned long val = kstrtoul(buf, 10, NULL);
	int ret = 0;
...
#ifdef APS_DEBUG
	APS_ERR("APDS9930_show_als_tunningdata con_reg = 0x%x, als_gain = 0x%x\n",
		buffer[0], als_gain);
#endif

	return sprintf(buf, "c0:%d, c1:%d, gain:%dx\n", c0_value, c1_value,
		       apds9930_als_again_tb[als_gain]);
...
static DRIVER_ATTR(ps_led, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_ps_led,
		   APDS9930_store_ps_led);
static DRIVER_ATTR(ps_pulse, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH,
		   APDS9930_show_ps_pulse, APDS9930_store_ps_pulse);
static DRIVER_ATTR(pilt, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_pilt,
		   APDS9930_store_pilt);
static DRIVER_ATTR(piht, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_piht,
		   APDS9930_store_piht);
static DRIVER_ATTR(pdata, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_pdata,
		   NULL);
static DRIVER_ATTR(alsdata, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_alsdata,
		   NULL);
static DRIVER_ATTR(luxdata, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, APDS9930_show_luxdata,
		   NULL);
static DRIVER_ATTR(enable_ps, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH,
		   APDS9930_show_ps_enable, APDS9930_store_ps_enable);
static DRIVER_ATTR(enable_als, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH,
		   APDS9930_show_als_enable, APDS9930_store_als_enable);
static DRIVER_ATTR(als_enable, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH,
		   APDS9930_show_als_enable, APDS9930_store_als_enable);
static DRIVER_ATTR(als_tunningdata, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP
| S_IROTH, APDS9930_show_als_tunningdata, NULL);	/* jin.joo temp code */
