...
#define COMBO_IOC_SENSOR_QUERY       16
...
mtk_thermal_get_temp(int id)
{
	GPS_DBG_FUNC("E_WF: %s doesn't exist, id:%u\n", __func__, id);
	return -127000;
}
...
long GPS_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOC_SENSOR_QUERY:
		retval = mtk_thermal_get_temp(arg);
		/*GPS_DBG_FUNC("sensor query ctrl, retval(%d), arg(%lu)\n", retval, arg);*/
		break;
