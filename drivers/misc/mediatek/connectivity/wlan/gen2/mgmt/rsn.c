...
void rsnStartSaQueryTimer(IN P_ADAPTER_T prAdapter)
{
	...
	if (prBssSpecInfo->u4SaQueryCount > 0 && rsnCheckSaQueryTimeout(prAdapter)) {
        ...
        // Null Point Error Fix ALPS03523223
        if (prBssInfo->eConnectionState != PARAM_MEDIA_STATE_CONNECTED || prBssInfo->prStaRecOfAP == NULL) {
            DBGLOG(RSN, WARN, "MFP: WARN eConnectionState =%d\n", prBssInfo->eConnectionState);
            return;
        }
        // Null Point Error Fix ALPS03523223
