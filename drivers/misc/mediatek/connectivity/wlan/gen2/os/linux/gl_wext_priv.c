		...
		IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	case PRIV_CMD_SET_TX_POWER:
		...
		break;
	//ALPS03695580 	New API for set_fcc_channel
	case PRIV_CMD_SET_FCC:
		{
			INT_32 i4TotalLen = strlen(CMD_SET_FCC_CERT)+2;
			PCHAR pCommand = NULL;
			...
			prTestStruct = (P_PARAM_MTK_WIFI_TEST_STRUCT_T)&aucOidBuf[0];
			pCommand = kalMemAlloc(strlen(CMD_SET_FCC_CERT)+2, VIR_MEM_TYPE);
			if (pCommand == NULL) {
				DBGLOG(REQ, INFO, "alloc fail\n");
				return -EINVAL;
			}
			kalMemZero(pCommand, i4TotalLen);
			kalSprintf(pCommand, "%s %s", CMD_SET_FCC_CERT,((prTestStruct->u4FuncData==1)?"0":"-1"));
...
#endif
		else if (strnicmp(pcCommand, CMD_ADD_TS, strlen(CMD_ADD_TS)) == 0 ||
			strnicmp(pcCommand, CMD_DELETE_TS, strlen(CMD_DELETE_TS)) == 0) {
