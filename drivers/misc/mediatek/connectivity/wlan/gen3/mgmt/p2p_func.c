		    ...
		    IN PUINT_8 pucNewBcnBody, IN UINT_32 u4NewBodyLen)
{
	...
	do {
		...
		if ((prP2pBssInfo->eCurrentOPMode != OP_MODE_ACCESS_POINT) ||
		    (prP2pBssInfo->eIntendOPMode != OP_MODE_NUM)) { /* LGE_P2P_PATCH, 20180417, cheolsook.lee, short preamble */
		...
		IN P_BSS_INFO_T prP2pBssInfo, IN BOOLEAN fgSendDeauth, IN UINT_16 u2ReasonCode)
{
	...
	do {
		...
		if (prAdapter->rWifiVar.prP2PConnSettings->fgIsApMode) {
...
P_MSDU_INFO_T p2pFuncProcessP2pProbeRsp(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx, IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	...
	do {
		...
		bssComposeBeaconProbeRespFrameHeaderAndFF((PUINT_8)
							  ...
							  /* LGE_P2P_PATCH, 20180417, cheolsook.lee, short preamble */
							  (ucBssIdx == P2P_DEV_BSS_INDEX) ? prProbeRspFrame->u2BeaconInterval : prP2pBssInfo->u2BeaconInterval,
							  (ucBssIdx == P2P_DEV_BSS_INDEX) ? prProbeRspFrame->u2CapInfo : prP2pBssInfo->u2CapInfo);
