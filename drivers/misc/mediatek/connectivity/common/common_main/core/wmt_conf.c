...
static INT32 wmt_conf_parse_char_array(P_DEV_WMT pWmtDev, const struct parse_data *data, const PINT8 pos);

static PINT8 wmt_conf_write_char_array(P_DEV_WMT pWmtDev, const struct parse_data *data);
...
#define ARRAY(f) {#f, wmt_conf_parse_char_array, wmt_conf_write_char_array, OFFSET(rWmtGenConf.f), NULL, NULL}
...
static const struct parse_data wmtcfg_fields[] = {
	...
	ARRAY(bt_tssi_average_cal_value),
...
static INT32 wmt_conf_parse_char_array(P_DEV_WMT pWmtDev, const struct parse_data *data, const PINT8 pos)
{
	...
	dst = (PINT8)(((PUINT8) pWmtDev) + (long)data->param1);
	str[2] = '\0';
	if ((osal_strlen(pos) > 2) && ((*pos) == '{')) {
		do {
			osal_memcpy(str, pos+i, 2);
			osal_strtol(str, 16, &res);
			*dst = (UINT8)res;
			WMT_DBG_FUNC("wmtcfg==> %s=0x%x\n", data->name, *dst);
			i += 3;
			dst++;
...
static PINT8 wmt_conf_write_char_array(P_DEV_WMT pWmtDev, const struct parse_data *data)
{
	...
	src = (PINT8) (((PUINT8) pWmtDev) + (long)data->param1);
	value = osal_malloc(33);
	if (value == NULL)
		return NULL;
	osal_memcpy(value, src, 32);
	value[33 - 1] = '\0';
	return value;
}
