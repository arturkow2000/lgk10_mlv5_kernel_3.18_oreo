...
#ifdef CONFIG_MTK_LGE_FM_TDMB_SWITCH
#include "fm_gpio.h"
#endif
...
#if (defined(CONFIG_OF) && defined(CONFIG_MTK_LGE_FM_TDMB_SWITCH))
			FmDrv_GPIO_FM_TDMB_Select(FM_SEL);
#endif
...
#if (defined(CONFIG_OF) && defined(CONFIG_MTK_LGE_FM_TDMB_SWITCH))
			FmDrv_GPIO_FM_TDMB_Select(TDMB_SEL);
#endif
...
#if (defined(CONFIG_OF) && defined(CONFIG_MTK_LGE_FM_TDMB_SWITCH))
static const struct of_device_id fm_tdmb_switch_of_ids[] = {
...
static struct platform_driver mt_fm_dev_drv = {
	...
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#if (defined(CONFIG_OF) && defined(CONFIG_MTK_LGE_FM_TDMB_SWITCH))
		   .of_match_table = fm_tdmb_switch_of_ids,
#endif
		   }
};
