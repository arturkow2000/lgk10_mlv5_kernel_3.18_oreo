...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x1dc60471,	/*0x6d01485c // Auto Test Mode ÃßÈÄ.. */
	.pre = {
	...
	.cap2 = {		/*capture for PIP 24fps relative information, capture1 mode must use same framelength,
						linelength with Capture mode for shutter calculate */
		.pclk = 480000000 / 2,
		/*.linelength = 9600, */
		/*.framelength = 3330, */
...
#if defined (CONFIG_MACH_MT6750_LV5)
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{4240, 2880, 0, 0, 4240, 2880, 706, 480, 33, 0, 640, 480, 0, 0, 640, 480}, /*high speed video(640 x 480)*/
...
/*X-Flip*/
	...
	write_cmos_sensor(0x3050, 0xfb80);//[LGE_UPDATE_S] [kyunghun.oh@lge.com] [2016-11-29] modified by hynix request
	...
	/*OB*/ write_cmos_sensor(0x0c12, 0x0040);
	/*OB*/ write_cmos_sensor(0x0c14, 0x0040);
	/*OB*/ write_cmos_sensor(0x0c16, 0x0040);
	/*OB*/ write_cmos_sensor(0x0c18, 0x8000);
	write_cmos_sensor(0x0000, 0x0100);
	write_cmos_sensor(0x003c, 0x0001);
	...
	write_cmos_sensor(0x0050, 0x0300);	/*0x4300 enable, 0x0300 disable of per-frame ctl*/
...
void hi1332_hs_video_setting_normal(void)
{
	...
	/*write_cmos_sensor(0x0a04, 0x016e); *//*otp */
	write_cmos_sensor(0x0a04, 0x016a);
	write_cmos_sensor(0x0050, 0x0720);	/*0x4720 enable, 0x0720 disable of per-frame ctl*/
	write_cmos_sensor(0x0722, 0x0301);
	write_cmos_sensor(0x0756, 0x003f);
...
unsigned short hi1332_init_setting[] = {
	...
	0x3048, 0x0140, //[LGE_UPDATE_S] [kyunghun.oh@lge.com] [2016-11-29] modified by hynix request
	0x404a, 0x0000,
	0x304c, 0x4b0b,
	0x404c, 0xf808,
	0x304e, 0x0088,
	0x404e, 0x0000,
	0x3050, 0xfb80, //[LGE_UPDATE_S] [kyunghun.oh@lge.com] [2016-11-29] modified by hynix request
	0x4050, 0x0300, //[LGE_UPDATE_S] [kyunghun.oh@lge.com] [2016-11-29] modified by hynix request
	0x0c00, 0x1190,
	0x0c02, 0x0011,
	0x0c04, 0x0000,
	0x0c06, 0x01b0,
	...
	0x0046, 0x0000,
	0x003a, 0x0000,
	0x0050, 0x0300,	/*0x4300 enable, 0x0300 disable of per-frame ctl*/
	0x0722, 0x0300,
	0x0756, 0x003f,
	0x005e, 0xf000,
	0x0060, 0x0000,
...
unsigned short hi1332_hs_video_setting[] = {
	...
	0x0062, 0x0000,
	0x0a04, 0x016e,
	0x0050, 0x0720,	/*0x4720 enable, 0x0720 disable of per-frame ctl*/
	0x0722, 0x0301,
	0x0208, 0x01e0,
	0x0006, 0x0341,
	0x0008, 0x0258,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		/*f Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF */
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF
			    ("Warning: current_fps %d fps is not support, so use cap2's setting: %d fps!\n",
	...
	LOG_INF("capture: frame_length = %d, line_length = %d\n", imgsensor.frame_length,
		imgsensor.line_length);
	return ERROR_NONE;
}				/* capture() */
...
#ifdef HI1332_BURST_ENABLE
	/*[mook][20160217] 640*480 */
	Hynix_i2c_burst(hi1332_hs_video_setting, sizeof(hi1332_hs_video_setting) / 2);
#else
	hi1332_hs_video_setting_normal();
#endif
	set_mirror_flip(imgsensor.mirror);
	LOG_INF("hs_video: frame_length = %d, line_length = %d\n", imgsensor.frame_length,
		imgsensor.line_length);
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		...
		ihdr_write_shutter_gain((UINT16) *feature_data, (UINT16) *(feature_data + 1),
					(UINT16) *(feature_data + 2));
		break;
	//[LGE_UPDATE_S][youmi.jun@lge.com][2017-05-18] update FOV value
	case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
		{
			...
			*pHorFOV = (0x40)<<16 | (0x40); // 16:9 = 64, 4:3 = 64 // (64)<<16 | (64)   //64 degree
			*pVerFOV = (0x31)<<16 | (0x31); // 16:9 = 49, 4:3 = 49 // (49)<<16 | (49)
			*feature_para_len = sizeof(uintptr_t) * 2;
		}
		break;
	//[LGE_UPDATE_E][youmi.jun@lge.com][2017-05-18] update FOV value
	default:
		break;
	}
...
#define HYNIX_SIZEOF_I2C_BUF 254
	...
	for (set_count = 0; set_count < (total_set_count - 2); set_count += 2) {
		...
		/* check i2c address. if address is add +2 , add data only. */
		if ((setting_table[set_count + 2] == setting_table[set_count] + 2) &&
		    (buf_count < HYNIX_SIZEOF_I2C_BUF)) {
			continue;
		}

		err = iBurstWriteReg(Hynix_i2c_buf, buf_count, imgsensor.i2c_write_id);	/* i2c write function */
	...
	write_cmos_sensor1D(setting_table[total_set_count - 2],
			    setting_table[total_set_count - 1] >> 8);
	write_cmos_sensor1D(setting_table[total_set_count - 2] + 1,
			    setting_table[total_set_count - 1] & 0xFF);
...
#define HYNIX_SIZEOF_I2C_BUF 254
	...
	for (set_count = 0; set_count < (total_set_count - 2); set_count += 2) {
		...
		err = iWriteRegI2C(Hynix_i2c_buf, buf_count, imgsensor.i2c_write_id);	/* i2c write function */
