...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 2592, 1944,  16,  12,   2560, 1920,   640, 4800,   0, 0,  640,  480,   0, 0,  640, 480},     // high speed video
...
// [LGE_UPDATE_S] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
static int hi556_read_eepromVendorID(void)
{
//    int retry = 3;
    unsigned char vendorID_readFrom_front_imageSensor = 0;
/*
    for(; retry > 0; retry--) {
...
*/
    read_moduleID_BRCF016GWZ3(&vendorID_readFrom_front_imageSensor);
    printk("[hi556_read_eepromVendorID] read eeprom id = %d\n", vendorID_readFrom_front_imageSensor);
    if(vendorID_readFrom_front_imageSensor != 0x01)
    {
        printk("[hi556_read_eepromVendorID] matching fail!! It should be 0x01 but read %d\n", vendorID_readFrom_front_imageSensor);
        return -1;
    }
    else
    {
        printk("[hi556_read_eepromVendorID] matching OK\n");
        strcpy(front_sensor_name, "sl556_lgit");
        return 0;
    }
}
// [LGE_UPDATE_E] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
...
// [LGE_UPDATE_S] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
                    if(hi556_read_eepromVendorID() != 0)
                        printk("[HI556] vendor id is different!! you should check the EEPROM or Module hardware\n");
	...
	case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
		{
			...
			*pHorFOV = (0x53)<<16 | (0x53); // 16:9 = 83, 4:3 = 83 // (83)<<16 | (83)   //83 degree
			*pVerFOV = (0x34)<<16 | (0x34);
