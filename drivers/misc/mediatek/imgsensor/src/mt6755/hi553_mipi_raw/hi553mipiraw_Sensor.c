 ...
 * Description:
 ...
 add group hold function at shutter gain setting, for ae peak
 -------------------------------
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI553MIPI_SENSOR_ID,	/*Sensor ID Value: 0x30C8 record sensor id defined in Kd_imgsensor.h */
	...
	/*sensor gain delay frame for AE cycle,2frame with ispGain_delay-sensor_gain_delay=2-0=2 */
	...
	.i2c_addr_table = {0x50, 0x40, /*0x20, */ 0xff},
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_DEBUG("Read sensor id fail, id: 0x%x, sensor id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		LOG_DEBUG("capture30fps: use cap30FPS's setting: %d fps!\n",
		...
		LOG_DEBUG("cap115fps: use cap1's setting: %d fps!\n", imgsensor.current_fps / 10);
		...
		LOG_DEBUG("Warning:=== current_fps %d fps is not support, so use cap1's setting\n",
			imgsensor.current_fps / 10);
		imgsensor.pclk = imgsensor_info.cap1.pclk;
		imgsensor.line_length = imgsensor_info.cap1.linelength;
		imgsensor.frame_length = imgsensor_info.cap1.framelength;
    ...
    case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
        {
            ...
            *pHorFOV = (0x5C)<<16 | (0x5C); // 16:9 = 67, 4:3 = 67 // (67)<<16 | (67)  //92 degree
            *pVerFOV = (0x44)<<16 | (0x44); // 16:9 = 49, 4:3 = 49 // (49)<<16 | (49)
