...
#if 1 //for LGE, soojong.jin@lge.com
#include "kd_sensorlist_lge.h"
#else
...
********************************************************************************/
#if defined (CONFIG_MACH_MT6750S_CV5A) || defined (CONFIG_MACH_MT6750S_CV7A)
int iBurstReadRegI2C(u8 *a_pSendData, u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData,
		     u16 i2cId)
{
	u8 *buf = NULL;
	int i4RetValue = 0;
	...
	/* mutex_lock(&kdCam_i2c_Mutex); */
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2017-11-9] Conditional statements for EEPROM devices using different i2c.
    if(g_CurrentSensorIdx == 1) {
        spin_lock(&kdsensor_drv_lock);
        g_pstI2Cclient->addr = (i2cId >> 1);
        spin_unlock(&kdsensor_drv_lock);
    }
	...
	buf =
	    dma_alloc_coherent(&(pCamerahw_platform_device->dev), a_sizeRecvData,
			       (dma_addr_t *) &phyAddr, GFP_KERNEL);

	if (NULL == buf) {
		PK_DBG("[iBurstWriteReg] Not enough memory\n");
		/* mutex_unlock(&kdCam_i2c_Mutex); */
		return -1;
	}

	*((char *)buf) = *a_pSendData;
	*((char *)buf + 1) = *(a_pSendData + 1);

    if(g_CurrentSensorIdx == 1) {

        spin_lock(&kdsensor_drv_lock);
        msgs[0].addr = (g_pstI2Cclient->addr & I2C_MASK_FLAG);
        msgs[0].ext_flag =
        ...
        spin_lock(&kdsensor_drv_lock);
        msgs[0].addr = (g_pstI2Cclient2->addr & I2C_MASK_FLAG);
        msgs[0].ext_flag =
            ((g_pstI2Cclient2->ext_flag) & I2C_MASK_FLAG) | (I2C_RS_FLAG) | I2C_WR_FLAG |
            I2C_DMA_FLAG;
        msgs[0].len = (a_sizeRecvData << 8 | a_sizeSendData);
        msgs[0].buf = (u8 *) phyAddr;
        spin_unlock(&kdsensor_drv_lock);

        i4RetValue = i2c_transfer(g_pstI2Cclient2->adapter, msgs, 1);

    }
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-11-9] Conditional statements for EEPROM devices using different i2c.
	/* PK_DBG("[iBurstReadRegI2C] i2c_transfer return (%d)\n",ret); */

	memcpy(a_pRecvData, buf, a_sizeRecvData);
	dma_free_coherent(&(pCamerahw_platform_device->dev), a_sizeRecvData, buf, phyAddr);
...
int iBurstWriteReg_multi(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		buf =
		    dma_alloc_coherent(&(pCamerahw_platform_device->dev), bytes,
				       (dma_addr_t *) &phyAddr, GFP_KERNEL);
		...
		PK_DBG("transfer_length %d\n",
		       bytes ==
		       transfer_length ? transfer_length : (((bytes / transfer_length) << 16) |
							    transfer_length));
...
inline static int adopt_CAMERA_HW_Open(void)
{
	...
	/* kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM*)
				g_invokeSocketIdx, g_invokeSensorNameStr,true, CAMERA_HW_DRVNAME); */
	/* wait for power stable */
	/* mDELAY(10); */
	/* KD_IMGSENSOR_PROFILE("kdModulePowerOn"); */
	/*  */
	if (g_pSensorFunc) {
		...
		if (ERROR_NONE != err) {
			...
			/* kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *)
					g_invokeSocketIdx, g_invokeSensorNameStr, false, CAMERA_HW_DRVNAME1); */
...
inline static int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (copy_to_user
	    ((void __user *)(pSensorGetInfo->pInfo), (void *)(pSensorInfo),
...
bool _hwPowerOn(PowerType type, int powerVolt)
{
	...
	if (!IS_ERR(reg)) {
		...
		if (type == DVDD && powerVolt == Vol_1200) {
			...
			{
				PK_ERR
				    ("[_hwPowerOn]fail to set PMIC_RG_VCAMD_CAL, powertype:%d powerId:%d\n",
				     type, powerVolt);
				return ret;
			}
		} else {
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	if (_IOC_NONE == _IOC_DIR(a_u4Command)) {
		PK_DBG("[CAMERA SENSOR] command did not found\n");
	} else {
...
{
    struct device*  camera_sensor_name_dev;// [LGE_UPDATE] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* set I2C clock rate */
	g_pstI2Cclient->timing = 400; //LGE_UPDATE [youmi.jun@lge.com] change I2C clock 400KHz
...
static int CAMERA_HW_i2c_probe2(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* set I2C clock rate */
	g_pstI2Cclient2->timing = 400; //LGE_UPDATE [youmi.jun@lge.com] change I2C clock 400KHz
