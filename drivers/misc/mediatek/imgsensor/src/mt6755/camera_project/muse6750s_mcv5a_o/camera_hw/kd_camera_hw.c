...
static DEFINE_MUTEX(kdCamGPIO_Mutex); //[LGE_UPDATE][kyunghun.oh@lge.com][2018-04-25] Add the mutex for race condition with Flash
...
extern int nxp_gpio_set_cfg(uint8_t gpio, uint8_t direction, uint8_t pull, uint8_t drvstr);
...
PowerUp PowerOnList = {
    {
        ...
        {SENSOR_DRVNAME_HI846_MIPI_RAW,
            {
                ...
                {DVDD, Vol_1200, 2},
#else
                {FRONT_POWERS, Vol_Low, 1},
                {FRONT_POWERS, Vol_High, 3},
#endif
        ...
        {SENSOR_DRVNAME_HI556_MIPI_RAW,
            {
#if 0
                {DOVDD_LDO, Vol_1800, 1},
                {AVDD_LDO, Vol_2800, 1},
                {DVDD_LDO, Vol_1200, 1},
#else
                {FRONT_POWERS, Vol_Low, 1},
                {FRONT_POWERS, Vol_High, 2},
#endif
                {SensorMCLK, Vol_High, 3},
                {RST, Vol_Low, 3},
                {RST, Vol_High, 1},
...
#endif
struct pinctrl_state *cam_flash_tx_h = NULL;
struct pinctrl_state *cam_flash_tx_l = NULL;
#if 0
struct pinctrl_state *cam_flash_front_en_h = NULL;//[LGE_UPDATE][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH
struct pinctrl_state *cam_flash_front_en_l = NULL;
#endif
struct pinctrl_state *cam0_mclk_gpio = NULL;
struct pinctrl_state *cam0_mclk_0 = NULL;
struct pinctrl_state *cam1_mclk_gpio = NULL;
struct pinctrl_state *cam1_mclk_1 = NULL;
struct pinctrl_state *cam1_powers_h = NULL;
struct pinctrl_state *cam1_powers_l = NULL;
int has_mipi_switch = 0;
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2018-04-25] Add the mutex for race condition with Flash
void flash_pin_ctrl(int enableFlash)
{
    mutex_lock(&kdCamGPIO_Mutex);

    if(enableFlash) {
        mtkcam_gpio_set(0,CAM_FLASH_EN,1);
        mtkcam_gpio_set(0,CAM_FLASH_TX,1);
    }
    else {
	    mtkcam_gpio_set(0,CAM_FLASH_EN,0);
	    mtkcam_gpio_set(0,CAM_FLASH_TX,0);
    }

    mutex_unlock(&kdCamGPIO_Mutex);
}
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2018-04-25] Add the mutex for race condition with Flash
...
if(lge_get_board_revno() >= 3) {
	cam1_powers_h = pinctrl_lookup_state(camctrl, "cam1_powers1");
	if (IS_ERR(cam1_powers_h)) {
		ret = PTR_ERR(cam1_powers_h);
		PK_DBG("%s : pinctrl err, cam1_powers_h\n", __func__);
	}
	cam1_powers_l = pinctrl_lookup_state(camctrl, "cam1_powers0");
	if (IS_ERR(cam1_powers_l)) {
		ret = PTR_ERR(cam1_powers_l);
		PK_DBG("%s : pinctrl err, cam1_powers_l\n", __func__);
	}
}
...
if(lge_get_board_revno() >= 3) {
	cam_flash_en_h = pinctrl_lookup_state(camctrl, "cam_flash_en1");
	if (IS_ERR(cam_flash_en_h)) {
		ret = PTR_ERR(cam_flash_en_h);
		PK_DBG("%s : pinctrl err, cam_flash_en_h\n", __func__);
	}

	cam_flash_en_l = pinctrl_lookup_state(camctrl, "cam_flash_en0");
	if (IS_ERR(cam_flash_en_l)) {
		ret = PTR_ERR(cam_flash_en_l);
		PK_DBG("%s : pinctrl err, cam_flash_en_l\n", __func__);
	}
}
	cam_flash_tx_h = pinctrl_lookup_state(camctrl, "cam_flash_tx1");
	if (IS_ERR(cam_flash_tx_h)) {
		ret = PTR_ERR(cam_flash_tx_h);
		PK_DBG("%s : pinctrl err, cam_flash_tx_h\n", __func__);
	}

	cam_flash_tx_l = pinctrl_lookup_state(camctrl, "cam_flash_tx0");
	if (IS_ERR(cam_flash_tx_l)) {
		ret = PTR_ERR(cam_flash_tx_l);
		PK_DBG("%s : pinctrl err, cam_flash_tx_l\n", __func__);
	}
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH
#if 0
	cam_flash_front_en_h = pinctrl_lookup_state(camctrl, "cam_flash_front_en1");
    printk("KYUNGHUN_KD_CAMERA_HW_EN_H\n");
	if (IS_ERR(cam_flash_front_en_h)) {
		ret = PTR_ERR(cam_flash_front_en_h);
		PK_DBG("%s : pinctrl err, cam_flash_front_en_h\n", __func__);
	}

	cam_flash_front_en_l = pinctrl_lookup_state(camctrl, "cam_flash_front_en0");
    printk("KYUNGHUN_KD_CAMERA_HW_EN_L\n");
	if (IS_ERR(cam_flash_front_en_l)) {
		ret = PTR_ERR(cam_flash_front_en_l);
		PK_DBG("%s : pinctrl err, cam_flash_front_en_l\n", __func__);
	}
#endif
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH

	cam0_mclk_gpio = pinctrl_lookup_state(camctrl, "cam0_mclk_gpio");
	if (IS_ERR(cam0_mclk_gpio)) {
		ret = PTR_ERR(cam0_mclk_gpio);
		PK_DBG("%s : pinctrl err, cam0_mclk_gpio\n", __func__);
	}

	cam0_mclk_0 = pinctrl_lookup_state(camctrl, "cam0_mclk_0");
	if (IS_ERR(cam0_mclk_0)) {
		ret = PTR_ERR(cam0_mclk_0);
		PK_DBG("%s : pinctrl err, cam0_mclk_0\n", __func__);
	}

	cam1_mclk_gpio = pinctrl_lookup_state(camctrl, "cam1_mclk_gpio");
	if (IS_ERR(cam1_mclk_gpio)) {
		ret = PTR_ERR(cam1_mclk_gpio);
		PK_DBG("%s : pinctrl err, cam1_mclk_gpio\n", __func__);
	}

	cam1_mclk_1 = pinctrl_lookup_state(camctrl, "cam1_mclk_1");
	if (IS_ERR(cam1_mclk_1)) {
		ret = PTR_ERR(cam1_mclk_1);
		PK_DBG("%s : pinctrl err, cam1_mclk_1\n", __func__);
	}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAM_FLASH_EN:
    if(lge_get_board_revno() >= 3) {
		if (Val == 0 && !IS_ERR(cam_flash_en_l))
        ...
        #if defined(CONFIG_GPIO_PCAL6416A)
        nxp_gpio_set_cfg(9, 1, 0, 0);
        #endif
		if (Val == 0){
            #if defined(CONFIG_GPIO_PCAL6416A)
            nxp_gpio_set_value(9, Vol_Low);
            #endif
        }
		else {
            #if defined(CONFIG_GPIO_PCAL6416A)
            nxp_gpio_set_value(9, Vol_High);
            #endif
        }
    }
		break;
	case CAM_FLASH_TX:
		if (Val == 0 && !IS_ERR(cam_flash_tx_l))
			pinctrl_select_state(camctrl, cam_flash_tx_l);
		else
			pinctrl_select_state(camctrl, cam_flash_tx_h);
		break;

//[LGE_UPDATE][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH
#if 0
    case CAM_FLASH_FRONT_EN:
		if (Val == 0 && !IS_ERR(cam_flash_front_en_l))
			pinctrl_select_state(camctrl, cam_flash_front_en_l);
		else
			pinctrl_select_state(camctrl, cam_flash_front_en_h);
		break;
#endif
//[LGE_UPDATE][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
    case FRONT_POWERS:
        if(lge_get_board_revno() >= 3) {
            if (Val == 0 && !IS_ERR(cam1_powers_l))
                pinctrl_select_state(camctrl, cam1_powers_l);

            else if (Val == 1 && !IS_ERR(cam1_powers_h))
                pinctrl_select_state(camctrl, cam1_powers_h);

            else
                PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, FRONT_POWERS\n", __func__,PinIdx ,Val);
        }
        break;
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
	default:
...
#if defined(CONFIG_MACH_MT6750_TH8)	//Because sub camera and main camera use the same DVDD
		} else if (pinSetIdx == 1) {
...
//#endif
		...
		if (pinSetIdx == 0) {
			if (pwInfo.Voltage == Vol_High) {
			ISP_MCLK1_EN(TRUE);
				pinctrl_select_state(camctrl, cam0_mclk_0);
			}
			else {
				ISP_MCLK1_EN(FALSE);
				pinctrl_select_state(camctrl, cam0_mclk_gpio);
			}
		} else {
			if (pwInfo.Voltage == Vol_High) {
			ISP_MCLK2_EN(TRUE);
				pinctrl_select_state(camctrl, cam1_mclk_1);
			}
			else {
				ISP_MCLK2_EN(FALSE);
				pinctrl_select_state(camctrl, cam1_mclk_gpio);
			}
		}
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
    } else if (pwInfo.PowerType == FRONT_POWERS) {
        if(lge_get_board_revno() >= 3) {
            mtkcam_gpio_set(pinSetIdx, FRONT_POWERS, 0);
            if (pwInfo.Voltage == Vol_High) {
                if (mtkcam_gpio_set(pinSetIdx, FRONT_POWERS, 1)) {
                    PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
                }
            } else {
                if (mtkcam_gpio_set(pinSetIdx, FRONT_POWERS, 0)) {
                    PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
                }
            }
        }
            ...
            if(lge_get_board_revno() == HW_REV_0) {
                #if defined(CONFIG_TI_TCA6418)
                ioexp_gpio_set_value(GPIO_SUB_AVDD, Vol_High);
                #endif
            } else {
                #if defined(CONFIG_GPIO_PCAL6416A)
                nxp_gpio_set_cfg(4, 1, 0, 0);
                nxp_gpio_set_value(4, Vol_High);
                #endif
            }
        }
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2017-12-28] ADD FLASH HWEN PIN control logic
    } else if (pwInfo.PowerType == CAM_FLASH_EN) {
		if(lge_get_board_revno() < HW_REV_C) {
        mtkcam_gpio_set(pinSetIdx, CAM_FLASH_EN, 0);
        if (pwInfo.Voltage == Vol_High) {
            if (mtkcam_gpio_set(0, CAM_FLASH_EN, 1)) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
        } else {
            if (mtkcam_gpio_set(0, CAM_FLASH_EN, 0)) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
        }
        }
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-12-28] ADD FLASH HWEN PIN control logic
	} else if (pwInfo.PowerType == AVDD_LDO) {
        if(lge_get_board_revno() == HW_REV_0) {
            #if defined(CONFIG_TI_TCA6418)
			ioexp_gpio_set_value(GPIO_SUB_AVDD, Vol_High);
            #endif
    ...
    } else if (pwInfo.PowerType == DVDD_LDO) {
        if(lge_get_board_revno() == HW_REV_0) {
            #if defined(CONFIG_TI_TCA6418)
            ioexp_gpio_set_value(GPIO_SUB_DVDD, Vol_High);
            #endif
		}
    } else if (pwInfo.PowerType == DOVDD_LDO) {
        if(lge_get_board_revno() == HW_REV_0) {
            #if defined(CONFIG_TI_TCA6418)
            ioexp_gpio_set_value(GPIO_SUB_DOVDD, Vol_High);
            #endif
		}
	} else {
	}
...
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
        ...
        if(lge_get_board_revno() >= 3) {
            ...
            if(lge_get_board_revno() == HW_REV_0) {
                #if defined(CONFIG_TI_TCA6418)
                ioexp_gpio_set_value(GPIO_SUB_AVDD, Vol_Low);
                #endif
            } else {
                #if defined(CONFIG_GPIO_PCAL6416A)
                nxp_gpio_set_value(4, Vol_Low);
                #endif
            }
        }
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-11-28] ADD FRONT POWERS controlled by GPIO
...
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2017-12-28] ADD FLASH HWEN PIN control logic
        ...
        if(lge_get_board_revno() == HW_REV_0) {
            #if defined(CONFIG_TI_TCA6418)
            ioexp_gpio_set_value(GPIO_SUB_DVDD, Vol_Low);
            #endif
		}
    } else if (pwInfo.PowerType == DOVDD_LDO) {
        if(lge_get_board_revno() == HW_REV_0) {
            #if defined(CONFIG_TI_TCA6418)
            ioexp_gpio_set_value(GPIO_SUB_DOVDD, Vol_Low);
            #endif
		}
	} else {
	}
		       ...
		       char *mode_name)
{
    ...
    mutex_lock(&kdCamGPIO_Mutex);//[LGE_UPDATE][kyunghun.oh@lge.com][2018-04-25] Add the mutex for race condition with Flash
	/* power ON */
	if (On) {
    ...
    mutex_unlock(&kdCamGPIO_Mutex);//[LGE_UPDATE][kyunghun.oh@lge.com][2018-04-25] Add the mutex for race condition with Flash
	return 0;
