...
static DEFINE_MUTEX(kdCam_power_i2c_Mutex);
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	mutex_lock(&kdCam_power_i2c_Mutex);
	switch (PwrType) {
	case RST:
		if (PinIdx == 0) {
...
//[LGE_UPDATE][kyunghun.oh@lge.com][2016-10-25] ADD FRONT FLASH
	...
	mutex_unlock(&kdCam_power_i2c_Mutex);
...
#endif
printk("KYUNGHUN_THIS IS PROJECT KD_CAMERA_HW\n");
printk("KYUNGHUN_THIS IS PROJECT KD_CAMERA_HW\n");
