...
typedef enum {
	CHARGER_CONFIG_SINGLE,		/* use main charger only */
	CHARGER_CONFIG_DUAL,		/* use MID-VBUS to supply slave */
	CHARGER_CONFIG_PARALLEL,	/* VBUS supply slave directly */
} CHARGER_CONFIG;
#endif
...
typedef struct {
	...
	unsigned int ZCV;
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	kal_bool charging_enabled;		/* block input current */
	kal_bool battery_charging_enabled;	/* block battery current */
	kal_bool safety_timer_enabled;
	kal_bool fake_hvdcp_enabled;		/* fake hvdcp mode */
	CHR_CURRENT_ENUM input_current_max;	/* current from power-source */
	CHR_CURRENT_ENUM constant_charge_current_max; /* current to battery */
	BATTERY_VOLTAGE_ENUM constant_charge_voltage_max;
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
	int typec_high_input_current;
	int typec_high_charging_current;
	int typec_medium_input_current;
	int typec_medium_charging_current;
#endif
};
...
extern void do_baton_undet_int_task(void);
#endif
...
extern int get_battery_cycle_count(void);
extern int set_battery_cycle_count(int cycle);
extern int get_battery_cycle_offset(void);
extern int set_battery_cycle_offset(int offset);
extern void battery_cycle_init(void);
extern void battery_cycle_update(int soc);
#endif
