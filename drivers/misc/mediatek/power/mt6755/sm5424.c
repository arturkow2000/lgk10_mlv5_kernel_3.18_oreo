...
static struct i2c_board_info i2c_sm5424 __initdata = { I2C_BOARD_INFO(SM5424_DEV_NAME, 0x6B) };

static struct i2c_client *new_client;

int init_gpio_number = (8 | 0x80000000);
int en_gpio_number = (24 | 0x80000000);
...
static enum power_supply_property sm5424_charger_properties[] = {
	...
	POWER_SUPPLY_PROP_CHARGER_DUMP,
};
...
static int sm5424_get_en_hiz(struct sm5424_info *info)
{
	...
	sm5424_read_reg(new_client, SM5424_CNTL, &read_reg);
...
static int sm5424_get_chgcmp(struct sm5424_info *info)
{
	...
	pr_info("[sm5424] %s: STATUS2 : 0x%02X CHGON = %d TOPOFF = %d DONE = %d\n", __func__,
		read_reg, chgon_ret, topoff_ret, done_ret);
...
static int sm5424_remove_set_reg(struct sm5424_info *info)
{
	u8 read_reg_00;
	u8 read_reg_01;

	pr_info("[sm5424] %s: I_limit/ IBAT 500mA set!!\n", __func__);

	sm5424_set_vbuslimit(info, 500);
	sm5424_set_fastchg(info, 500);
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_BQ25898S_SLAVE_SUPPORT
	bq2589x_adapter_out_handler();
#endif

	sm5424_read_reg(new_client, SM5424_VBUSCNTL, &read_reg_00);
	sm5424_read_reg(new_client, SM5424_CHGCNTL2, &read_reg_01);

	pr_info("[sm5424] %s: VBUSCNTL : 0x%02X CHGCNTL2 : 0x%02X\n", __func__, read_reg_00,
		read_reg_01);
...
static int sm5424_suspend(struct sm5424_info *info, int suspend)
{
	...
	battery_log(BAT_LOG_FULL, "[sm5424] suspend : %d\n", suspend);

	return 0;
}

int sm5424_dump_register(void)
{
	int i = 6;
	int ret = 0;
...
/* For Check*/
	...
	battery_log(BAT_LOG_CRTI,
		    ...
		    sm5424_reg[11], sm5424_reg[12], sm5424_reg[13], sm5424_reg[14], sm5424_reg[15],
		    sm5424_reg[16]);
...
/* For Check*/
	...
	pr_debug
	    ("[sm5424] %s: STATUS1 : 0x%02X STATUS2 : 0x%02X STATUS3 : 0x%02X CNTL : 0x%02X VBUSCNTL : 0x%02X\n"
	     "[sm5424] %s: CHGCNTL1 : 0x%02X CHGCNTL2 : 0x%02X CHGCNTL3 : 0x%02X CHGCNTL4 : 0x%02X CHGCNTL5 : 0x%02X CHGCNTL6 : 0x%02X\n",
	     __func__, read_reg_00, read_reg_01, read_reg_02, read_reg_03, read_reg_04, __func__,
	     read_reg_05, read_reg_06, read_reg_07, read_reg_08, read_reg_09, read_reg_10);
#endif
...
static int sm5424_set_OTGCURRENT(struct sm5424_info *info, u8 val)
{
	sm5424_update_reg(info->i2c, SM5424_CHGCNTL5,
			  ((val & SM5424_CHGCNTL5_OTGCURRENT_MASK) <<
			   SM5424_CHGCNTL5_OTGCURRENT_SHIFT),
...
{
	sm5424_update_reg(info->i2c, SM5424_CHGCNTL5,
			  ((val & SM5424_CHGCNTL5_BST_IQ3LIMIT_MASK) <<
			   SM5424_CHGCNTL5_BST_IQ3LIMIT_SHIFT),
			  (SM5424_CHGCNTL5_BST_IQ3LIMIT_MASK <<
			   SM5424_CHGCNTL5_BST_IQ3LIMIT_SHIFT));
...
static int sm5424_charger_get_property(struct power_supply *psy, enum power_supply_property prop,
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_DUMP:
		val->intval = sm5424_dump_register();
		break;
	default:
		return -EINVAL;
	}
...
static int sm5424_charger_set_property(struct power_supply *psy, enum power_supply_property prop,
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
#if 0				/* def CONFIG_LGE_PM_EXTERNAL_CHARGER_BQ25898S_SLAVE_SUPPORT */
		battery_log(BAT_LOG_CRTI, "[sm5424] %s:interval(%d),fastchg(%d),lcd_bl(%d)\n",
			    __func__, val->intval / 1000, info->fastchg, old_bl_level);
		if (val->intval / 1000 > 1800 && !old_bl_level && BMT_status.UI_SOC2 < 100) {
...
static int sm5424_charger_property_is_writeable(struct power_supply *psy,
...
int sm5424_pe_plus_ta_pattern_increase(void)
{
...
int sm5424_pe_plus_ta_pattern_decrease(void)
{
	struct sm5424_info *info = i2c_get_clientdata(new_client);
	int ma_100 = VBUSLIMIT_100mA;
	int ma_500 = VBUSLIMIT_500mA;
...
void sm5424_schedule_hrtimeout(u64 msec)
{
...
int sm5424_pe_plus_20_ta_pattern_increase(void)
{
	struct sm5424_info *info = i2c_get_clientdata(new_client);
	int ma_100 = VBUSLIMIT_100mA;
	int ma_500 = VBUSLIMIT_500mA;
	...
	for (i = 4; i >= 0; i--) {
		...
		if (bit_flag) {	/* bit 1 */
			...
			get_monotonic_boottime(&ptime[j++]);	/* changing */
			sm5424_write_reg(new_client, SM5424_VBUSCNTL, ma_500);
			/* get_monotonic_boottime(&ptime[j++]); */
			sm5424_schedule_hrtimeout(PEONTIME);

			get_monotonic_boottime(&ptime[j]);
			cptime_bufer[j] = dtime(j);
			battery_log(BAT_LOG_CRTI,
				    "%s: bit: %d = High(500mA) cptime_bufer[%d] = %d (%d)\n",
			...
			sm5424_write_reg(new_client, SM5424_VBUSCNTL, ma_100);
			/* get_monotonic_boottime(&ptime[j++]); */
			sm5424_schedule_hrtimeout(PEOFFTIME);

			get_monotonic_boottime(&ptime[j]);
			cptime_bufer[j] = dtime(j);
			battery_log(BAT_LOG_CRTI,
				    "%s: bit: %d = High(100mA) cptime_bufer[%d] = %d (%d)\n",
			...
			cptime_bufer[j] = dtime(j);
			battery_log(BAT_LOG_CRTI,
				    "%s: bit: %d = Low(500mA) cptime_bufer[%d] = %d (%d)\n",
				    __func__, i, j, cptime_bufer[j], PEOFFTIME);
			...
			cptime_bufer[j] = dtime(j);
			battery_log(BAT_LOG_CRTI,
				    "%s: bit: %d = Low(100mA) cptime_bufer[%d] = %d (%d)\n",
				    __func__, i, j, cptime_bufer[j], PEONTIME);
	...
	sm5424_write_reg(new_client, SM5424_VBUSCNTL, ma_500);
	/* get_monotonic_boottime(&ptime[j++]); */
	/* sm5424_schedule_hrtimeout(160); */
	sm5424_schedule_hrtimeout(170);
	get_monotonic_boottime(&ptime[j]);
	cptime_bufer[j] = dtime(j);
	...
	if (cptime_bufer[j] < 150 || cptime_bufer[j] > 240) {
		battery_log(BAT_LOG_CRTI,
			    "%s: WDT fail: target:150 actual:%d\n", __func__, cptime_bufer[j]);
		return STATUS_FAIL;
	}

	sm5424_write_reg(new_client, SM5424_VBUSCNTL, ma_100);
	get_monotonic_boottime(&ptime[j++]);
	sm5424_schedule_hrtimeout(30);
...
int sm5424_pe_plus_20_ta_pattern_decrease(void)
{
	struct sm5424_info *info = i2c_get_clientdata(new_client);
	int ma_100 = VBUSLIMIT_100mA;
	int ma_500 = VBUSLIMIT_500mA;
...
{
	sm5424_update_reg(new_client, SM5424_CHGCNTL5, topoff_min << TOPOFFTIMER_SHIFT,
...
int sm5424_set_otg_boost_enable(int enable)
{
	...
	if (enable) {
		...
		sm5424_update_reg(new_client, SM5424_CNTL, ENBOOST_EN << SM5424_CNTL_ENBOOST_SHIFT,
				  SM5424_CNTL_ENBOOST_MASK << SM5424_CNTL_ENBOOST_SHIFT);
	} else {
		sm5424_update_reg(new_client, SM5424_CNTL, ENBOOST_DIS << SM5424_CNTL_ENBOOST_SHIFT,
				  SM5424_CNTL_ENBOOST_MASK << SM5424_CNTL_ENBOOST_SHIFT);
	}
...
/* IRQ read & clear for nINT pin state initialization */
	...
	sm5424_set_TOPOFF(info, TOPOFF_150mA);
	...
	sm5424_set_FASTTIMER_timer(FASTTIMER_DISABLED);
	...
	if (read_reg_3 & SM5424_INT3_FASTTMROFF) {
		...
		if (safty_cnt == 0) {
			if (BMT_status.bat_full) {
				battery_log(BAT_LOG_CRTI, "[sm5424] %s: FASTTIMER complete!\n",
					    __func__);
			} else {
				sm5424_enable(info, CHARGE_DIS);
				sm5424_enable(info, CHARGE_EN);
				battery_log(BAT_LOG_CRTI, "[sm5424] %s: FASTTIMER restart!\n",
					    __func__);
			}
			safty_cnt += 1;
		}
		battery_log(BAT_LOG_CRTI, "[sm5424] %s: safty_cnt = %d\n", safty_cnt, __func__);
	}
#endif
	temp = read_reg_3 & SM5424_INT3_FAIL;
	pr_debug("[sm5424] %s: SM5424_STATUS3_FAIL : 0x%02X , FAIL_No : %d\n", __func__, read_reg_3,
		 temp);
...
/* bringup build error */
#if 0
static ssize_t show_sm5424_addr_register(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	return sprintf(buf, "0x%x\n", att_addr);
}

static ssize_t store_sm5424_addr_register(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(sm5424_addr_register, 0664, show_sm5424_addr_register,
		   store_sm5424_addr_register);

static ssize_t show_sm5424_status_register(struct device *dev, struct device_attribute *attr,
					   char *buf)
{
	BYTE ret = 0;
	struct sm5424_info *info = dev_get_drvdata(dev);
...
static ssize_t store_sm5424_status_register(struct device *dev, struct device_attribute *attr,
					    const char *buf, size_t size)
{
	BYTE ret = 0;
	struct sm5424_info *info = dev_get_drvdata(dev);
...
static DEVICE_ATTR(sm5424_status_register, 0664, show_sm5424_status_register,
		   store_sm5424_status_register);
#endif
...
static int sm5424_otg_regulator_enable(struct regulator_dev *rdev)
{
	...
	sm5424_suspend(info, SUSPEND_DIS);
	sm5424_enable(info, CHARGE_DIS);
	sm5424_update_reg(info->i2c, SM5424_CNTL, ENBOOST_EN << SM5424_CNTL_ENBOOST_SHIFT,
			  SM5424_CNTL_ENBOOST_MASK << SM5424_CNTL_ENBOOST_SHIFT);
...
static int sm5424_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	sm5424_update_reg(info->i2c, SM5424_CNTL, ENBOOST_DIS << SM5424_CNTL_ENBOOST_SHIFT,
			  SM5424_CNTL_ENBOOST_MASK << SM5424_CNTL_ENBOOST_SHIFT);
...
static int sm5424_regulator_init(struct sm5424_info *info)
{
	...
	if (init_data->constraints.name) {
		...
		if (IS_ERR(info->otg_vreg.rdev)) {
			...
			if (rc != -EPROBE_DEFER)
				battery_log(BAT_LOG_CRTI,
					    "[sm5424] failed to register regulator, rc=%d\n", rc);
		}
	}
...
static int sm5424_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	battery_log(BAT_LOG_CRTI, "[sm5424] PROBE_START\n");
	...
	ret = request_threaded_irq(info->irq, NULL, sm5424_irq_thread,
				   IRQF_TRIGGER_LOW | IRQF_ONESHOT, "sm5424_irq", info);
