...
#if defined(CONFIG_LGE_MOISTURE_DETECTION) && defined(CONFIG_LGE_ACCDET_IMPEDANCE)
...
#define LGE_UNKNOWN_DEVICE 1<<8
#define MOISTURE_IMPEDANCE 5000
#define MOISTURE_AFE 6
#define MOISTURE_PMIC 12
#define MOISTURE_USB_MODE_PATH "/sys/class/dual_role_usb/otg_default/mode"
#define MOISTURE_USB_MODE "fault"

int moisture_detected;
extern char mConnectionState[Soc_Aud_InterConnectionInput_Num_Input][Soc_Aud_InterConnectionOutput_Num_Output];
#endif
...
#ifdef CONFIG_LGE_ACCDET
#define ACCDET_DEBOUNCE3_PLUG_OUT	(0x20*20)
#else
#define ACCDET_DEBOUNCE3_PLUG_OUT	(0x20*30) /* 30 msec */
#endif
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
static int button_press_debounce = 0x400;
#elif defined(CONFIG_LGE_ACCDET)
static int button_press_debounce = 0x210;
#else
static int button_press_debounce = 0x400;
#endif
...
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
static struct delayed_work accdet_work_delay;
#endif
...
#if defined(CONFIG_LGE_ACCDET) && !defined(CONFIG_LGE_ACCDET_WORKQUEUE)
...
#ifdef CONFIG_LGE_ACCDET_SANITY
#define HEADSET_HOOK 3
int accdet_control_hs(int hs_state)
{
	if(hs_state == accdet_data.state){
		return 0;
	}

	switch(hs_state){
		case NO_DEVICE:
			printk(KERN_INFO "accdet_sanity_control : NO_DEVICE, current : %d\n",
					accdet_data.state);

			switch_set_state(&accdet_data, NO_DEVICE);
			if(HEADSET_MIC == accdet_data.state){
		...
		case HEADSET_MIC:
			printk(KERN_INFO "accdet_sanity_control : HEADSET_WITH_MIC, current : %d\n",
					accdet_data.state);

			if(HEADSET_NO_MIC == accdet_data.state){
				switch_set_state(&accdet_data, NO_DEVICE);
				input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
				input_sync(kpd_accdet_dev);
			}

			switch_set_state(&accdet_data, HEADSET_MIC);
		...
		case HEADSET_NO_MIC:
			printk( KERN_INFO "accdet_sanity_control : HEADSET_NO_MIC, current : %d\n",
					accdet_data.state);
			...
			switch_set_state(&accdet_data, HEADSET_NO_MIC);
			input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 1);
			input_sync(kpd_accdet_dev);
			break;

		case HEADSET_HOOK:
			printk( KERN_INFO "accdet_sanity_control : HOOK, current : %d\n",
					accdet_data.state);
		...
		default:
			printk( KERN_INFO "accdet_sanity_control : \n");
			break;
	}

	return 0;
}
EXPORT_SYMBOL(accdet_control_hs);
#endif
...
{
#ifdef CONFIG_LGE_ACCDET
	ACCDET_ERROR( " [accdet] headset_plug_out cable_type = %d\n",cable_type);
	input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
	...
	if(moisture_detected){
		...
		switch_set_state((struct switch_dev *)&accdet_data, LGE_UNKNOWN_DEVICE);
	}else
#endif
...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	ACCDET_DEBUG("%s start cur_eint_state=%d\n", __func__, cur_eint_state);
...
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
		/*force check accdet state to detect aux cable*/
		{
			int ret;

			ret = queue_delayed_work(accdet_workqueue, &accdet_work_delay, msecs_to_jiffies(1000));
			if (!ret)
				ACCDET_DEBUG("[Accdet]accdet_work_delay return:%d!\n", ret);
		}
#endif
	} else {
/*EINT_PIN_PLUG_OUT*/
/*Disable ACCDET*/
		...
		mutex_lock(&accdet_eint_irq_sync_mutex);
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
		/*Cancel force check accdet state to detect aux cable*/
		{
			cancel_delayed_work_sync(&accdet_work_delay);
		}
#endif
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
...
#if defined(CONFIG_LGE_ACCDET) && !defined(CONFIG_LGE_ACCDET_WORKQUEUE)
		while( is_ready_to_detect_jack_type == 0 && loop_count < 15 ) /* need to set maximun wait time is under 5 sec */
		{
		...
		if(is_ready_to_detect_jack_type == 0)	//only EINT Accepted
		{
			...
			accdet_irq_handler();
			ACCDET_DEBUG("[ACCDET] Maybe this cable is unsupported, Y jack, old apple's or etc. it is forced to 4 pin type.\n");
		}
#endif
	} else {
/*EINT_PIN_PLUG_OUT*/
...
static irqreturn_t accdet_eint_func(int irq, void *data)
{
	...
	ACCDET_DEBUG("[Accdet]Enter accdet_eint_func !!!!!! cur_eint_state=%d\n", cur_eint_state);
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		/*
...
static int isMoisture(void)
{
	...
	mfd = sys_open (MOISTURE_USB_MODE_PATH, O_RDONLY, 0);
	if(mfd >= 0){
		if(sys_read(mfd, mbuf, 24)){
		    ACCDET_DEBUG("[Accdet] usb mode=%s\n", mbuf);
			if((!strncmp(mbuf, MOISTURE_USB_MODE, strlen(MOISTURE_USB_MODE)))){
				ret = true;
			}
		}
		sys_close(mfd);
		...
		ACCDET_DEBUG("[Accdet] can't open sysfs %d\n", mfd);
	return ret;
}
#endif
...
static void send_key_event(int keycode, int flag)
{
	...
	case MD_KEY:
		...
		input_sync(kpd_accdet_dev);
		ACCDET_DEBUG("[accdet]KEY_MEDIA %d\n", flag);
#else
...
#if defined(CONFIG_LGE_MOISTURE_DETECTION) && defined(CONFIG_LGE_ACCDET_IMPEDANCE)
	int plugOutFlag = 0;
	int backupAFE[MOISTURE_AFE];
	int backupPMIC[MOISTURE_PMIC];
	int backupPMIC_Reg[MOISTURE_PMIC] = {
		MT6353_AUDNCP_CLKDIV_CON0,
		MT6353_AUDNCP_CLKDIV_CON1,
		MT6353_AUDNCP_CLKDIV_CON2,
		MT6353_AUDNCP_CLKDIV_CON3,
		MT6353_AUDNCP_CLKDIV_CON4,
		MT6353_ZCD_CON0,
...
#define PIN_ADC_CHANNEL 5
	...
	ACCDET_DEBUG("[Accdet]accdet interrupt happen:[%s]current AB = %d cable type = %d\n",
		     accdet_status_string[accdet_status], current_status, cable_type);
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (1 == eint_accdet_sync_flag) {
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
				if((cur_eint_state == EINT_PIN_PLUG_IN) && (accdet_status == NO_DEVICE)){
					accdet_status = HOOK_SWITCH;
					cable_type = HEADSET_MIC;
					ACCDET_DEBUG("[Accdet]Force to change cable type %d!\n", cable_type);
				}else
#endif
				{
					accdet_status = PLUG_OUT;
...
#endif
#if defined CONFIG_ACCDET_EINT || defined CONFIG_ACCDET_EINT_IRQ
			ACCDET_DEBUG("[Accdet] do not send plug out event in hook switch eint_accdet_sync_flag=%d\n", eint_accdet_sync_flag);
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (1 == eint_accdet_sync_flag)
				accdet_status = PLUG_OUT;
			else
...
#if defined(CONFIG_LGE_ACCDET_IMPEDANCE)
	if(cable_type){
		backupAFE[0] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I05][Soc_Aud_InterConnectionOutput_O03];
		backupAFE[1] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I06][Soc_Aud_InterConnectionOutput_O04];
		backupAFE[2] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I07][Soc_Aud_InterConnectionOutput_O03];
		backupAFE[3] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I08][Soc_Aud_InterConnectionOutput_O04];
		backupAFE[4] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I14][Soc_Aud_InterConnectionOutput_O03];
		backupAFE[5] = (int)mConnectionState[Soc_Aud_InterConnectionInput_I14][Soc_Aud_InterConnectionOutput_O04];

		for(backup_index = 0; backup_index < MOISTURE_PMIC; backup_index++){
			backupPMIC[backup_index] =  Ana_Get_Reg(backupPMIC_Reg[backup_index]);
		}

		if(eint_accdet_sync_flag && current_type == NO_DEVICE)
		    impedance = accdet_Audio_Hp_Impedance_Get();
		ACCDET_DEBUG("[accdet] Impedance=%d cable_type = %d current_type=%d eint_accdet_sync_flag=%d accdet_getHeadsetPowerStatus=%d\n",
		impedance, cable_type, current_type, eint_accdet_sync_flag, accdet_getHeadsetPowerStatus());

#if defined(CONFIG_LGE_MOISTURE_DETECTION)
		if(impedance < 1 && plugOutFlag){
		    impedance = accdet_Audio_Hp_Impedance_Get();
		    if(impedance < 1 )
		         cable_type = NO_DEVICE;
		}

		if(isMoisture() && impedance > MOISTURE_IMPEDANCE){
			Ana_Set_Reg(MT6353_SPK_CON0, 0x0000, 0xf000);
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0000, 0xffff);
			cable_type = cable_type | LGE_UNKNOWN_DEVICE;
			moisture_detected = true;

			ACCDET_DEBUG("[accdet] resume audio path\n");
			for(backup_index = 0; backup_index < MOISTURE_PMIC; backup_index++){
			    ACCDET_DEBUG("[accdet] resume pmic register addr=%x, value=%x\n", backupPMIC_Reg[backup_index], backupPMIC[backup_index]);
				Ana_Set_Reg(backupPMIC_Reg[backup_index], backupPMIC[backup_index], 0xffff);
			}
			SetConnectionState(backupAFE[0], Soc_Aud_InterConnectionInput_I05, Soc_Aud_InterConnectionOutput_O03);
			SetConnectionState(backupAFE[1], Soc_Aud_InterConnectionInput_I06, Soc_Aud_InterConnectionOutput_O04);
			SetConnectionState(backupAFE[2], Soc_Aud_InterConnectionInput_I07, Soc_Aud_InterConnectionOutput_O03);
			SetConnectionState(backupAFE[3], Soc_Aud_InterConnectionInput_I08, Soc_Aud_InterConnectionOutput_O04);
			SetConnectionState(backupAFE[4], Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O03);
			SetConnectionState(backupAFE[5], Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O04);
		}else
#endif
		if(accdet_getHeadsetPowerStatus() && current_type == NO_DEVICE){
			Ana_Set_Reg(MT6353_SPK_CON0, 0x0000, 0xf000);
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0000, 0xffff);

			ACCDET_DEBUG("[accdet] resume audio path hppw=%d\n", accdet_getHeadsetPowerStatus());
			for(backup_index = 0; backup_index < MOISTURE_PMIC; backup_index++){
			    ACCDET_DEBUG("[accdet] resume pmic register addr=%x, value=%x\n", backupPMIC_Reg[backup_index], backupPMIC[backup_index]);
				Ana_Set_Reg(backupPMIC_Reg[backup_index], backupPMIC[backup_index], 0xffff);
			}
	...
	ACCDET_DEBUG("[Accdet] cable type:[%s], status switch:[%s]->[%s]\n",
		     accdet_report_string[cable_type&LGE_UNKNOWN_DEVICE?MOISTURE:cable_type], accdet_status_string[pre_status],
...
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
	if (1 == eint_accdet_sync_flag){
		ACCDET_DEBUG( " [Accdet] accdet_work_callback cable_type = %d, prev_cable_type = %d\n",cable_type,prev_cable_type);
		if(cable_type == NO_DEVICE)
		{
			switch_set_state((struct switch_dev *)&accdet_data, cable_type);
			input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
		...
		{
			if(cable_type == HEADSET_MIC && prev_cable_type == HEADSET_NO_MIC)
			{
				ACCDET_DEBUG( " [Accdet] accdet_work_callback remove jack before jack type is changed. \n");
				switch_set_state((struct switch_dev *)&accdet_data, NO_DEVICE);
				input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
				input_sync(kpd_accdet_dev);
			}
			else if(cable_type == HEADSET_NO_MIC && prev_cable_type == HEADSET_MIC)
			{
				ACCDET_DEBUG( " [Accdet] accdet_work_callback remove jack before jack type is changed. \n");
				if (cur_key != 0) {
					//send_key_event(cur_key, 0);
...
#elif defined(CONFIG_LGE_ACCDET)
	if (1 == eint_accdet_sync_flag) {
		ACCDET_ERROR( " [Accdet] accdet_work_callback cable_type = %d current_type = %d\n",cable_type, current_type);
		if (cable_type == NO_DEVICE) {
			input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
			input_report_switch(kpd_accdet_dev, SW_MICROPHONE_INSERT, 0);
			input_sync(kpd_accdet_dev);
...
#if defined(CONFIG_LGE_MOISTURE_DETECTION) && defined(CONFIG_LGE_ACCDET_IMPEDANCE)
			if(current_type != cable_type && current_type != NO_DEVICE && !moisture_detected)
#else
			if(current_type != cable_type && current_type != NO_DEVICE)
#endif
				switch_set_state((struct switch_dev *)&accdet_data, NO_DEVICE);

			input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 1);
			if (cable_type == HEADSET_MIC) {
				ACCDET_ERROR( " [Accdet] accdet_work_callback HEADSET_MIC = %d\n",cable_type);
				input_report_switch(kpd_accdet_dev, SW_MICROPHONE_INSERT, 1);
			}
			input_sync(kpd_accdet_dev);
		}
#if defined(CONFIG_LGE_MOISTURE_DETECTION) && defined(CONFIG_LGE_ACCDET_IMPEDANCE)
		if(!moisture_detected || (cable_type & LGE_UNKNOWN_DEVICE))
#endif
		switch_set_state((struct switch_dev *)&accdet_data, cable_type);
		current_type = cable_type;
	} else {
...
#ifdef CONFIG_LGE_ACCDET_WORKQUEUE
	INIT_DELAYED_WORK(&accdet_work_delay, accdet_work_callback );
#endif
