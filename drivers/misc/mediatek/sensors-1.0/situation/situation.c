...
static int handle_to_index(int handle)
{
	...
	case ID_PHONE_TILT_GESTURE:
		index = phone_tilt_gesture;
		break;
#endif

#ifdef CONFIG_LGE_SENSOR_SLOPE_DETECTOR
		case ID_SLOPE_DETECTOR:
			index = slope_detector;
			break;
#endif
	...
	SITUATION_PR_ERR("handle_to_index handle:%d, index:%d\n", handle, index);
	return index;
}
			       ...
			       const char *buf, size_t count)
{
	...
	SITUATION_PR_ERR("situation_store_flush param: handle %d\n", handle);
...
#ifdef CONFIG_LGE_PHONE_GESTURE
	cxt->mdev.minor = ID_PHONE_TILT_GESTURE;/* MISC_DYNAMIC_MINOR; */
#endif

#ifdef CONFIG_LGE_SENSOR_SLOPE_DETECTOR
	cxt->mdev.minor = ID_SLOPE_DETECTOR;
#else
