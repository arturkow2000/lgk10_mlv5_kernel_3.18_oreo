...
#if defined(CONFIG_MTK_AKM09916)
#define AKM_Device_AK09916
#endif

#define DEBUG 0
#if defined(AKM_Device_AK09916)
#define AKM09918_DEV_NAME	"akm09916"
#else
...
#define MAGN_PR_ERR(fmt, args...)	printk(KERN_INFO MAGN_TAG fmt, ##args)
#define MAGN_LOG(fmt, args...)	printk(KERN_ERR MAGN_TAG fmt, ##args)
#else
#define MAGN_TAG
...
static int AKECS_CheckDevice(void)
{
	...
	if ((akm_device == 0x05) || (akm_device == 0x04)) {/* ak09911 & ak09912 */
		ret = AKECS_ReadFuse();
		if (ret < 0) {
			MAGN_PR_ERR("AKM09918 AKECS_CheckDevice: read fuse fail\n");
			return -ENXIO;
		}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		MAGN_PR_ERR("akm09918_i2c_data is null!!\n");
		return 0;
	}
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->hw = hw;
	/* akm_map is different with MTK, so do not use hwmsen_get_convert. fix me*/
	...
	if (err < 0) {
		MAGN_PR_ERR("AKM09918 akm09918_probe: check device connect error\n");
...
static int akm09918_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		MAGN_PR_ERR("akm09918_delete_attr fail: %d\n", err);
...
static int akm09918_local_init(void)
{
	...
	if (i2c_add_driver(&akm09918_i2c_driver)) {
		MAGN_PR_ERR("%s: i2c_add_driver error\n", __func__);
		return -1;
	}
	if (-1 == akm09918_init_flag) {
		MAGN_PR_ERR("%s: akm09918_init_flag = %d\n", __func__, akm09918_init_flag);
		return -1;
	}
	return 0;
}
