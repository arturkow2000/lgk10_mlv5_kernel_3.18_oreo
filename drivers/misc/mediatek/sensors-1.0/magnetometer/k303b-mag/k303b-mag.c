...
static struct mag_init_info k303b_init_info = {
	.name = K303B_DEV_NAME,
...
/***
	if((err = k303b_SetPowerMode(obj->client, 1)))
	{
		SENSOR_ERR("write power control fail!!\n");
		return err;
	}
...
#if 1
	if(get_sensors_status(LSM_ALL_SENSORS) && (k303b_SetPowerMode(obj->client, 1))!=0)
	{
		SENSOR_ERR("write power control fail!!\n");
		return;
	}
...
static int k303b_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}
static int k303b_flush(void)
{
	if(k303b_i2c_client == NULL)
	{
		return -1;
	}
...
static int k303b_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ctl.enable = k303b_enable;
    ctl.set_delay = k303b_set_delay;
    ctl.open_report_data = k303b_open_report_data;
    ctl.batch = k303b_batch;
    ctl.flush = k303b_flush;
    ctl.is_report_input_direct = false;
    ctl.is_support_batch = data->hw->is_batch_supported;
    strlcpy(ctl.libinfo.libname, "daemon", sizeof(ctl.libinfo.libname));
    ctl.libinfo.layout = 0;
    ctl.libinfo.deviceid = 0;
	
    err = mag_register_control_path(&ctl);
    if (err) {
        SENSOR_ERR("register mag control path err\n");
        goto exit_kfree;
    }

    mag_data.div = 1;
    mag_data.get_data = k303b_m_getdata;

    err = mag_register_data_path(&mag_data);
    if (err) {
        SENSOR_ERR("register data control path err\n");
        goto exit_kfree;
    }
