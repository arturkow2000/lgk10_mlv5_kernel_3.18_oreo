...
/*----------------------------------------------------------------------------*/
#define APS_TAG                  "[APDS9930] "
#define APS_FUN(f)               printk(KERN_DEBUG APS_TAG "%s\n", __func__)
#define APS_ERR(fmt, args...)    printk(KERN_ERR APS_TAG fmt, ##args)
...
static struct alsps_init_info APDS9930_init_info __refdata = {
		.name = "apds9930",
...
int APDS9930_i2c_master_operate(struct i2c_client *client, char *buf, int count, int i2c_flag)
{
	...
	case I2C_FLAG_WRITE:
		...
		for(retry=0; retry < I2C_MAX_TRY; retry++){
			...
			if (res < 0) {
				APS_ERR("%s: I2C_FLAG_WRITE i2c_master_send error, retry:%d\n", __func__, retry+1);
				msleep(20);
			} else {
				break;
			}
	...
	case I2C_FLAG_READ:
		...
		for(retry=0; retry < I2C_MAX_TRY; retry++){
			...
			if (res < 0) {
				APS_ERR("%s: I2C_FLAG_READ i2c_master_send error, retry:%d\n", __func__, retry+1);
				msleep(20);
				continue;
			}
			...
			if (res < 0) {
				APS_ERR("%s: I2C_FLAG_READ i2c_master_recv error, retry:%d\n", __func__, retry+1);
				msleep(20);
			} else {
				break;
			}
	...
	default:
		APS_ERR("%s: i2c_flag command not support!\n", __func__);
		break;
	}
	if (res <= 0)
		goto EXIT_ERR;
...
static int APDS9930_get_ps_caldata(struct i2c_client *client)
{
	...
	if (0 == obj->ps_cali) { //if.. doesn't init on lgpserver
		...
		if (prox_cal_val < 0 || 1023 < prox_cal_val) {
			APS_ERR("%s: cali value %d is invalid. use default value %d\n", __func__, prox_cal_val, PS_DEFAULT_CAL);
			obj->ps_cali = PS_DEFAULT_CAL;
		} else {
			APS_LOG("%s: cali value %d\n", __func__, prox_cal_val);
			obj->ps_cali = prox_cal_val;
		}

		obj->hw->ps_threshold_high = obj->ps_cali + obj->local_hw->offset;
		obj->hw->ps_threshold_low = obj->hw->ps_threshold_high - obj->local_hw->hysteresis;
...
struct apds9930_hw *get_apds9930_dts_func(const char *name, struct apds9930_hw *hw)
{
	...
	if (node) {
		...
		ret = of_property_read_u32_array(node, "hysteresis", hysteresis, ARRAY_SIZE(hysteresis));
		if (ret == 0) {
			hw->hysteresis = hysteresis[0];
			APS_LOG("%s: hysteresis = %d\n", __func__, hw->hysteresis);
		}

		ret = of_property_read_u32_array(node, "crosstalk_max", crosstalk_max, ARRAY_SIZE(crosstalk_max));
		if (ret == 0) {
			hw->crosstalk_max = crosstalk_max[0];
			APS_LOG("%s: crosstalk_max = %d\n", __func__, hw->crosstalk_max);
		}
		...
		APS_ERR("%s: can not find alsps node!. Go to use old cust info\n", __func__);
		return NULL;
	}
	return hw;
}
...
//  uint16_t  high_threshold;
	...
	if (event == FB_EVENT_BLANK) {
		if (*blank == FB_BLANK_POWERDOWN) {
			...
			APS_LOG("%s: [IN] LCD Sleep\n", __func__);
		} else if (*blank == FB_BLANK_UNBLANK) {
			atomic_set(&driver_suspend_flag, 0);
			APS_LOG("%s: [OUT] LCD Sleep\n", __func__);
...
static long APDS9930_enable_als(struct i2c_client *client, int enable)
{
	...
	if (enable) {
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		APS_LOG("%s: APDS9930_CMM_ENABLE enable als value = %x\n", __func__, databuf[1]);
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0)
			goto EXIT_ERR;
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		APS_LOG("%s: APDS9930_CMM_ENABLE disable als value = %x\n", __func__, databuf[1]);
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0)
			goto EXIT_ERR;
 ...
 EXIT_ERR:
	APS_ERR("%s: APDS9930_enable_als fail\n", __func__);
	return res;
}
...
static long APDS9930_enable_ps(struct i2c_client *client, int enable)
{
	...
	APS_LOG("%s: enable = %d, ps_cali = %d\n", __func__, enable, obj->ps_cali);
#endif
	...
	if (enable) {
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		APS_LOG("%s: APDS9930_CMM_ENABLE enable ps value = %x\n", __func__, databuf[1]);
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0)
			goto EXIT_ERR;
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		APS_LOG("%s: APDS9930_CMM_ENABLE disable ps value = %x\n", __func__, databuf[1]);
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0)
			goto EXIT_ERR;
...
static int APDS9930_check_intr(struct i2c_client *client)
{
	...
	if (gpio_get_value(alsps_int_gpio_number) == 1) {
		APS_ERR("%s: skip, threre is no irq\n", __func__);
		return 0;
	}
	...
	res = APDS9930_i2c_master_operate(client, buffer, 0x101, I2C_FLAG_READ);
	if (res <= 0) {
		APS_ERR("%s: APDS9930_i2c_master_operate error res = %d\n", __func__, res);
		return -1;
	}
...
int APDS9930_irq_registration(struct i2c_client *client)
{
	...
	if (ret) {
		APS_ERR("%s: alsps request_irq IRQ LINE NOT AVAILABLE!.\n", __func__);
		return ret;
	}
...
int APDS9930_read_als(struct i2c_client *client, u16 *data)
{
	...
	if (client == NULL) {
		APS_ERR("%s: CLIENT CANN'T EQUL NULL\n", __func__);
		return -1;
	}
...
#ifdef APS_DEBUG
	APS_DBG("%s: c0 = %d, c1 = %d, IAC1 = %d, IAC2 = %d\n", __func__, c0_value, c1_value, IAC1, IAC2);
	APS_DBG("%s: als_ga = %d, als_coe_b = %d, als_coe_c = %d, als_coe_d = %d\n", __func__,
			obj->local_hw->als_ga, obj->local_hw->als_coe_b, obj->local_hw->als_coe_c, obj->local_hw->als_coe_d);
#endif
	if(IAC1 > IAC2)
		IAC = IAC1;
	else if(IAC1 <= IAC2)
	...
	if(obj->als_reduce){
		...
		APS_DBG("%d: Dynamic gain lux value = %d\n", __func__, luxValue);
	}
#endif

#ifdef APS_DEBUG
	APS_DBG("%s: als_res_index = %d, als_gain_index = %d, data = %d\n", __func__, obj->als_atime_index, obj->als_again_index, *data);
	APS_DBG("%s: luxValue = %d, obj->als_reduce=%d \n", __func__, luxValue, obj->als_reduce);
#endif
...
static int APDS9930_get_ps_value(struct APDS9930_priv *obj, u16 ps)
{
	...
	APS_LOG("%s: ps = %d psat_enabled = %d\n", __func__, ps, obj->is_psat_enabled);

	if ( obj->is_psat_enabled ) {
		val = 1;	/* forced far */
		forced_far = 1;
	} else if (ps > obj->hw->ps_threshold_high) {
		val = 0;
	} else {
		val = 1;
		forced_far = 0;
	...
	if (atomic_read(&obj->ps_suspend)) {
		...
		/* invalid = 1; */
		APS_ERR("%s: ligh too high will result to failt proximiy\n", __func__);
		return 1;	/*far away */
	}
...
static int APDS9930_do_calibration(struct i2c_client *client)
{
	...
	unsigned int old_disable = 0;
//	unsigned char old_modectl;
	int result, i;

// (2018-01-04) Calibration Structure bug fix
//	old_enable  = obj->enable;
	...
	for (i=0; i< CALB_TIMES; i++){
		...
		if ( calib_data <= obj->local_hw->crosstalk_max /*200*/ /*25*/ /*870*/ ) {
			APS_LOG("%s: ps_cross_talk save : %d\n", __func__, calib_data);
	...
	if (i >= CALB_TIMES) {
		APS_ERR ("%s: failed to calibrate cross talk\n", __func__);
		return -1;
	} else {
#if defined(CALIBRATION_TO_FILE)
		...
		{
			APS_LOG("%s: Calibration factory write END\n", __func__);
		}
		else
		{
			APS_ERR("%s: Fail to write MISC PARTITION\n", __func__);
		}
#endif
#endif
		obj->hw->ps_threshold_high = obj->ps_cali + obj->local_hw->offset;
	...
	if (old_disable) {
		result = APDS9930_enable_ps( client, 0 );
		if (result != APDS9930_SUCCESS) {
			return (result);
		}
	}
...
static int APDS9930_change_gain(u16 als_ch0)
{
	...
	if (als_ch0 >= apds9930_als_res_tb[obj->als_atime_index] * 90 / 100) {
		if (obj->als_again_index != APDS9930_ALS_GAIN_1X) {
			...
			res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
#ifdef APS_DEBUG
			APS_LOG("%s: als_ch0 = %d, als_gain = 0x%0x\n", __func__, als_ch0, databuf[1] & 0x03);
#endif
			if (res >= 0)
				obj->als_reduce = 1;
		}
	...
	if (change_again) {
		APS_LOG("%s: change again to 0x%02x\n", __func__, apds9930_als_again_bit_tb[obj->als_again_index]);

		databuf[0] = APDS9930_CMM_CONTROL;
		databuf[1] = (obj->local_hw->ps_led_current & 0xFC) | apds9930_als_again_bit_tb[obj->als_again_index];
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
#ifdef APS_DEBUG
		APS_LOG("%s: als_ch0 = %d, als_gain = 0x%0x\n", __func__, als_ch0, databuf[1] & 0x03);
#endif
...
{
	struct APDS9930_priv *obj = (struct APDS9930_priv *)container_of(work, struct APDS9930_priv, irq_work);
	//int err;
...
#ifdef APS_DEBUG
		APS_DBG("%s: APDS9930 int top half time = %lld\n", __func__, int_top_time);
#endif
#if 0
		if (obj->als > 40000) {
...
/*signal interrupt function add*/
		if (ps_state) {  //far
			if (forced_far == 1) {
				APS_LOG("%s: pdata = %d,  prox status = forced_far FAR\n", __func__, obj->ps);
				set_psensor_threshold(obj->client, 0, 1022);
			} else {
				APS_LOG("%s: pdata = %d,  prox status = FAR\n", __func__, obj->ps);
				set_psensor_threshold(obj->client, 0, obj->hw->ps_threshold_high);
			}

		} else {
			if (forced_far == 1) {
				APS_LOG("%s: pdata = %d,  prox status = forced_far NEAR\n", __func__, obj->ps);
				set_psensor_threshold(obj->client, 1022, 1023);
			} else {
				APS_LOG("%s: pdata = %d,  prox status = NEAR\n", __func__, obj->ps);
				set_psensor_threshold(obj->client, obj->hw->ps_threshold_low, 1023); // set far threshold
			}
		}
		...
		res = ps_report_interrupt_data(ps_state);
		if (res)
			APS_ERR("%s: apds9930 call ps_report_interrupt_data fail = %d\n", __func__, res);
	...
	if (res == ALS_INT || res == PS_ALS_INT) {
		...
		APS_DBG("%s: als = %d!, ch0 = %d\n", __func__, adata, als_ch0);
		...
		if (atomic_read(&driver_suspend_flag) == 1) {
			als_pocket_threshold =
					obj->local_hw->als_threshold * apds9930_als_again_tb[obj->als_again_index] / apds9930_als_again_tb[ALS_DEFAULT_GAIN];
			APS_LOG("%s: One threshold detect mode\n", __func__);
			if (als_ch0 <= als_pocket_threshold) {
				APS_LOG("%s: als_data(%d)<THRESHOLD(%d), Change bright to dark\n", __func__, als_ch0, als_pocket_threshold);
				set_asensor_threshold(obj->client, 0, als_pocket_threshold);
			} else {
				APS_LOG("%s: als_data(%d)>THRESHOLD(%d), Change dark to bright\n", __func__, als_ch0, als_pocket_threshold);
			...
			set_asensor_threshold(obj->client, 0x0000, 0xFFFF);
		}
	}
 ...
 /*   disable_irq_nosync(alsps_irq);*/
	enable_irq(alsps_irq);
	enable_irq_wake(alsps_irq);
	APS_ERR("%s: i2c_transfer error = %d\n", __func__, res);
#endif
}

static int APDS9930_get_deviceid( struct i2c_client *client, unsigned char *Data )
{
	int ret = 0;
	u8 databuf[2];

	databuf[0] = APDS9930_CMM_ID;
	ret = APDS9930_i2c_master_operate(client, databuf, 0x101, I2C_FLAG_READ);
	if (ret <= 0)
		return ret;
	else {
		*Data = databuf[0];
		APS_LOG("%s: DEVICEID=0x%02x\n", __func__, *Data);
	}
...
static ssize_t APDS9930_show_alsdata ( struct device_driver *dev, char *buf )
{
	...
	{
		APS_ERR("%s: Can't access register of alsdata.\n", __func__);
	}
	return sprintf ( buf, "%d\n", obj->als );
}
...
#ifdef APS_DEBUG
	APS_DBG("%s: CONTROL register = 0x%x, als_gain = 0x%x\n", __func__, buffer[0], als_gain);
#endif
...
static ssize_t APDS9930_show_reg(struct device_driver *dev, char *buf)
{
	struct i2c_client *client = APDS9930_i2c_client;
	struct APDS9930_priv *obj = i2c_get_clientdata ( client );
	...
	for (i = 0; i < 0x1f; i++) {
		...
		if (addr == 0x10
			...
			APS_ERR("%s: addr=0x%02x skip\n", __func__, addr);
			continue;
		}
		databuf[0] = addr + 0x80;

		//msleep(10);
		ret = APDS9930_i2c_master_operate(obj->client, databuf, 0x101, I2C_FLAG_READ);
		if (ret <= 0) {
			APS_ERR("%s: ret = %d\n", __func__, ret);
			count += sprintf(buf+count, "error ret = %d\n", ret);
			//return ret;
		} else {
			char temp = databuf[0] | (databuf[1] << 8);
			APS_LOG("%s: [0x%02x]: 0x%02x\n", __func__, i, temp);
...
static DRIVER_ATTR ( reg, S_IRUGO, APDS9930_show_reg, NULL );
...
*/
	...
	case ALSPS_IOCTL_GET_CALI:
		APS_LOG("%s: CMD = ALSPS_IOCTL_GET_CALI\n", __func__);
	...
	case ALSPS_IOCTL_SET_CALI:
		APS_LOG("%s: CMD = ALSPS_IOCTL_SET_CALI\n", __func__);

#if defined(CALIBRATION_TO_FILE)
		ps_cali = 0;
		err = sensor_calibration_read(ID_PROXIMITY, &ps_cali);
		if (err != 0){
			APS_ERR("%s: Read Cal Fail from file !!!\n", __func__);
		}
#else
	...
	case ALSPS_GET_DEVICEID:
		APS_LOG ( "%s: CMD = ALSPS_GET_DEVICEID\n", __func__ );
		err = APDS9930_get_deviceid ( obj->client, (unsigned char*) &dat );
		if (err)
		{
			goto err_out;
		}
			...
			void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_DELAY:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			APS_ERR("%s: Set delay parameter error!\n", __func__);
			err = -EINVAL;
		}
		/* Do nothing */
		break;
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			APS_ERR("%s: Enable sensor parameter error!\n", __func__);
			err = -EINVAL;
		} else {
			value = *(int *)buff_in;
			if (value) {
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			APS_ERR("%s: get sensor data parameter error!\n", __func__);
			err = -EINVAL;
		} else {
		...
		break;
	default:
		APS_ERR("%s: proxmy sensor operate function no this parameter %d!\n", __func__, command);
		err = -1;
		break;
	}
			 ...
			 void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		...
		break;
	default:
		APS_ERR("%s: light sensor operate function no this parameter %d!\n", __func__, command);
		err = -1;
		break;
	}
...
static int als_enable_nodata(int en)
{
	...
	if (atomic_read(&APDS9930_obj->init_done)) {
		...
		APS_ERR("%s: sensor hub has not been ready!!\n", __func__);
	...
	mutex_unlock(&APDS9930_mutex);
	if (!APDS9930_obj) {
		APS_ERR("%s: APDS9930_obj is null!!\n", __func__);
		return -1;
	}
...
static int als_get_data(int *value, int *status)
{
...
#ifdef CONFIG_LGE_SENSOR_DYNAMIC_AGAIN
	u16 als_ch0 = 0;
#endif /* CONFIG_LGE_SENSOR_DYNAMIC_AGAIN */
	...
	if (err)
		APS_ERR("%s: SCP_sensorHub_req_send fail!\n", __func__);
	else {
	...
	if (atomic_read(&APDS9930_obj->trace) & CMC_TRC_PS_DATA)
		...
		APS_ERR("%s: sensor hub hat not been ready!!\n", __func__);
		err = -1;
	}
#else /* #ifdef CUSTOM_KERNEL_SENSORHUB */
	if (!APDS9930_obj) {
...
static int als_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	/*FIX  ME */
#ifdef USES_ALS_WAKELOCK
	APDS9930_obj->als_wake_delay = value;
#endif
	APS_LOG("%s: apds9930 als set delay = (%d) ok.\n", __func__, value);
...
static int ps_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	APS_LOG("%s: apds9930 ps set delay = (%d) ok.\n", __func__, value);
	return 0;
}
...
static int APDS9930_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		APS_ERR("%s: get_APDS9930_platform_data err = %d\n", __func__, err);
		return -1;
	}
	...
	if (err < 0) {
		APS_ERR("%s: Unable to request gpio int_pin err = %d\n", __func__, err);
		return -1;
	}
	...
	obj->hw->ps_threshold_high = obj->local_hw->offset;
	obj->hw->ps_threshold_low = obj->local_hw->hysteresis;
	obj->enable = 0;
	obj->pending_intr = 0;
	obj->is_psat_enabled = false;
#ifdef CONFIG_LGE_SENSOR_DYNAMIC_AGAIN
...
// (2018-01-04) Driver bug fix : unnecessary settings
//	set_bit(CMC_BIT_ALS, &obj->enable);
//	set_bit(CMC_BIT_PS, &obj->enable);
	...
	if (err) {
		APS_ERR("%s: APDS9930_init_client err = %d\n", __func__, err);
		goto exit_init_failed;
	}
	...
	if (err) {
		APS_ERR("%s: irq registration err = %d\n", __func__, err);
		goto exit_init_failed;
	}

	err = APDS9930_check_and_clear_intr(client);
	if (err) {
		APS_ERR("%s: check/clear intr err = %d\n", __func__, err);
		goto exit_init_failed;
	}

	err = misc_register(&APDS9930_device);
	if (err) {
		APS_ERR("%s: misc_register err = %d\n", __func__, err);
	...
	if (err) {
		APS_ERR("%s: create_attribute err = %d\n", __func__, err);
...
*/
	...
	if (err) {
		APS_ERR("%s: als_register_control_path register err = %d\n", __func__, err);
		goto exit_sensor_obj_attach_fail;
	}
	...
	if (err) {
		APS_ERR("%s: als_register_data_path register err = %d\n", __func__, err);
		goto exit_sensor_obj_attach_fail;
	}
	...
	if (err) {
		APS_ERR("%s: ps_register_control_path err = %d\n", __func__, err);
		goto exit_sensor_obj_attach_fail;
	}
	...
	if (err) {
		APS_ERR("%s: ps_register_data_path err = %d\n", __func__, err);
		goto exit_sensor_obj_attach_fail;
	}

#ifdef USES_ALS_WAKELOCK
	wake_lock_init(&obj->als_wl, WAKE_LOCK_SUSPEND, "apds_als_wl");
#endif
...
static int APDS9930_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		APS_ERR("%s: misc_deregister fail: %d\n", __func__, err);
	...
	if (err)
	{
		APS_ERR("%s: apds9930_delete_attr fail: %d\n", __func__, err);
	}
	gpio_free(alsps_int_gpio_number);
	APDS9930_i2c_client = NULL;
...
/*----------------------------------------------------------------------------*/
static int __init APDS9930_local_init(void)
{
	...
	if (i2c_add_driver(&APDS9930_i2c_driver)) {
		APS_ERR("%s: add driver error\n", __func__);
		return -1;
	}

	APS_LOG("%s: init_flag %d\n", __func__, APDS9930_init_flag);
...
static int of_get_APDS9930_platform_data(struct device *dev)
{
	...
	if (node) {
		...
		APS_ERR("%s: alsps_int_gpio_number = %d, alsps_irq = %d\n", __func__, alsps_int_gpio_number, alsps_irq);
	}
	return 0;
}
/*----------------------------------------------------------------------------*/
static int __init APDS9930_init(void)
{
	...
	if(!proxi_hw)
		APS_ERR("%s: get local_hw dts info fail\n", __func__);
