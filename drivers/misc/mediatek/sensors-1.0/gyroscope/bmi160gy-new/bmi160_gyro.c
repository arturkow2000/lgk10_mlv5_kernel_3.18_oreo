...
static void bmg_ExcuteDynamicGyroBias(struct dynamic_gyro_bias* d_gyro,	s16* data, int* cali);
...
struct bmg_i2c_data {
	...
	/* unmapped axis value */
	int cali_sw[BMG_AXES_NUM+5];
	/* hw offset */
	s8	offset[BMG_AXES_NUM+1];/* +1:for 4-byte alignment */
#ifdef SUPPORT_DYNAMIC_UPDATE_GYRO_BIAS
	int	cali_data[BMG_AXES_NUM+1];
#endif
...
static bool gyro_enable_status = false;
/* 0=OK, -1=fail */
...
static int bmg_set_osr4(struct i2c_client *client)
{
	int err = 0;
	uint8_t databuf[2] = {0};
	struct bmg_i2c_data *obj = obj_i2c_data;
	uint8_t bandwidth = BMI160_GYR_OSR4_AVG1;	// 0x00
	uint8_t gyro_undersampling_parameter = 0;

	mutex_lock(&obj->lock);

	err = bmg_i2c_read_block(client, BMI160_USER_GYR_CONF_ODR__REG, &databuf[0], 1);
	databuf[0] = BMG_SET_BITSLICE(databuf[0], BMI160_USER_GYR_CONF_GYR_BWP, bandwidth);
	databuf[0] = BMG_SET_BITSLICE(databuf[0], BMI160_USER_GYR_CONF_GYR_UNDER_SAMPLING, gyro_undersampling_parameter);
	err += bmg_i2c_write_block(client, BMI160_USER_GYR_CONF_ODR__REG, &databuf[0], 1);

	mdelay(10);

	if(err < 0) {
		GYRO_ERR("set OSR failed.\n");
		mutex_unlock(&obj->lock);
		return -1;
	}

	GYRO_LOG("[%s] gyro_bmp = %d, gyro_us = %d ok.\n", __func__,
				bandwidth, gyro_undersampling_parameter);
...
#ifdef SUPPORT_DYNAMIC_UPDATE_GYRO_BIAS
		bmg_UpdateDynamicGyroBias(d_gyro, (int *)obj->data);
#endif
...
static int bmg_do_calibration(struct i2c_client *client)
{
	...
	/* Set new calibration */
	client_data->cali_sw[BMG_AXIS_X] = -((int)sum[BMG_AXIS_X]/CALIBRATION_DATA_AMOUNT);
	client_data->cali_sw[BMG_AXIS_Y] = -((int)sum[BMG_AXIS_Y]/CALIBRATION_DATA_AMOUNT);
	client_data->cali_sw[BMG_AXIS_Z] = -((int)sum[BMG_AXIS_Z]/CALIBRATION_DATA_AMOUNT);
	...
	err = sensor_calibration_save(ID_GYROSCOPE, client_data->cali_sw);
#endif
	if(err)
			return BMI160_GYRO_CAL_FAIL;
#endif
		...
		const char *buf, size_t count)
{
	...
	int enabled = (obj->power_mode == BMG_NORMAL_MODE);
	...
	if (!enabled)
	{
		GYRO_LOG("%s sensor is disabled. so enable... \n", __func__);
		res = bmg_set_powermode(client, BMG_NORMAL_MODE);

		if (res)
		{
			GYRO_ERR("%s set power mode to normal, failed err=%d\n", __func__, res);
			return res;
		}
		mdelay(100);
	}
	...
	if (!enabled)
	{
		GYRO_LOG("%s sensor is disabled. so disable... \n", __func__);
		res = bmg_set_powermode(client, BMG_SUSPEND_MODE);
		if (res)
		{
			GYRO_ERR("%s set power mode to suspend, failed err=%d\n", __func__, res);
			return res;
		}
	}
		...
		void *buff_out, int size_out, int *actualout)
{
	...
	if ((buff_in == NULL) || (size_in < sizeof(int))) {
		...
		sample_delay = BMI160_GYRO_ODR_200HZ;
	...
	if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
		...
		gyroscope_data->status = SENSOR_STATUS_ACCURACY_HIGH;
...
#ifdef CALIBRATION_TO_FILE
#ifdef CONFIG_CUSTOM_KERNEL_SENSOR_CAL
			err = sensor_calibration_read(ID_GYROSCOPE, obj->cali_sw);
#endif
			if(err) {
...
static int bmg_resume(struct i2c_client *client)
{
	...
	if (gyro_enable_status)
	{
		err = bmg_set_powermode(obj->client, BMG_NORMAL_MODE);
		if (err) {
			GYRO_ERR("bmg_resume set suspend mode failed.\n");
			return err;
		}
	}
...
static int bmi160_gyro_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	int sample_delay = BMI160_GYRO_ODR_200HZ;
	struct bmg_i2c_data *priv = obj_i2c_data;
	err = bmg_set_datarate(priv->client, sample_delay);
	if (err < 0)
		GYRO_ERR("set data rate failed.\n");
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		GYRO_ERR("get dts info fail !! err = %d\n", err);
		err = -EFAULT;
		goto exit;
	}
...
//	ctl.set_delay  = bmi160_gyro_set_delay;
	...
	if (err < 0) {
		GYRO_ERR("register gyro control path err = %d\n", err);
		goto exit_create_attr_failed;
	}
