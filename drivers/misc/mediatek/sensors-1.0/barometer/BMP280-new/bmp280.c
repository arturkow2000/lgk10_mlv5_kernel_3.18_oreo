...
static int bmp_get_calibration_data(struct i2c_client *client)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {
		...
		obj->bmp280_cali.dig_P2 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[9])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[8]);
		obj->bmp280_cali.dig_P3 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[11])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[10]);
		obj->bmp280_cali.dig_P4 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[13])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[12]);
		obj->bmp280_cali.dig_P5 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[15])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[14]);
		obj->bmp280_cali.dig_P6 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[17])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[16]);
		obj->bmp280_cali.dig_P7 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[19])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[18]);
		obj->bmp280_cali.dig_P8 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[21])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[20]);
		obj->bmp280_cali.dig_P9 = (BMP280_S16_t)
				((((BMP280_S16_t) ((signed char)a_data_u8r[23])) << SHIFT_LEFT_8_POSITION) | a_data_u8r[22]);
	}
	return 0;
}
...
static int bmp_get_temperature(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		...
		v_x1_u32r = ((((utemp >> 3) - ((BMP280_S32_t) obj->bmp280_cali.dig_T1 << 1)))
				* ((BMP280_S32_t) obj->bmp280_cali.dig_T2)) >> 11;
		v_x2_u32r = (((((utemp >> 4) - ((BMP280_S32_t) obj->bmp280_cali.dig_T1))
				* ((utemp >> 4) - ((BMP280_S32_t) obj->bmp280_cali.dig_T1)) ) >> 12)
...
*/
static int bmp_get_pressure_int64(struct i2c_client *client, char *buf, int bufsize)
{
	struct bmp_i2c_data *obj;
	int status;
	...
	if (time_before_eq((unsigned long)(obj->last_temp_measurement +
		...
		if (status != 0)
			goto exit;
		if ((err = kstrtos32(temp_buf, 16, &temperature)) < 0)
			BAR_ERR("sscanf parsing fail err = %d\n", err);
	}
...
*unit:10 degrees centigrade
*/
static int bmp_get_pressure_int32(struct i2c_client *client, char *buf, int bufsize)
{
	struct bmp_i2c_data *obj;
	int status;
	s32 temperature = 0, upressure = 0, pressure = 0;
	...
	if (obj->sensor_type == BMP280_TYPE) {
		BMP280_S32_t v_x1_u32r = BMP280_INIT_VALUE;
		BMP280_S32_t v_x2_u32r = BMP280_INIT_VALUE;
		BMP280_U32_t v_pressure_u32 = BMP280_INIT_VALUE;
		/* calculate x1*/
		v_x1_u32r = (((BMP280_S32_t)obj->t_fine)
			>> BMP280_SHIFT_BIT_POSITION_BY_01_BIT) - (BMP280_S32_t)64000;
		/* calculate x2*/
		v_x2_u32r = (((v_x1_u32r >> BMP280_SHIFT_BIT_POSITION_BY_02_BITS)
			...
			* ((BMP280_S32_t)obj->bmp280_cali.dig_P6);
		v_x2_u32r = v_x2_u32r + ((v_x1_u32r *
			((BMP280_S32_t)obj->bmp280_cali.dig_P5))
			<< BMP280_SHIFT_BIT_POSITION_BY_01_BIT);
		v_x2_u32r = (v_x2_u32r >> BMP280_SHIFT_BIT_POSITION_BY_02_BITS)
			+ (((BMP280_S32_t)obj->bmp280_cali.dig_P4)
				<< BMP280_SHIFT_BIT_POSITION_BY_16_BITS);
		/* calculate x1*/
		v_x1_u32r = (((obj->bmp280_cali.dig_P3
			* (((v_x1_u32r
			...
			+ ((((BMP280_S32_t)obj->bmp280_cali.dig_P2)
				* v_x1_u32r)
				>> BMP280_SHIFT_BIT_POSITION_BY_01_BIT))
			>> BMP280_SHIFT_BIT_POSITION_BY_18_BITS;
		v_x1_u32r = ((((32768 + v_x1_u32r))
			* ((BMP280_S32_t)obj->bmp280_cali.dig_P1))
			>> BMP280_SHIFT_BIT_POSITION_BY_15_BITS);
		/* calculate pressure*/
		v_pressure_u32 = (((BMP280_U32_t)(((BMP280_S32_t)1048576) - upressure)
		...
		if (v_pressure_u32 < 0x80000000)
			...
			if (v_x1_u32r != BMP280_INIT_VALUE)
				v_pressure_u32 = (v_pressure_u32
					<< BMP280_SHIFT_BIT_POSITION_BY_01_BIT)
				/ ((BMP280_U32_t)v_x1_u32r);
			else
				return BMP280_INVALID_DATA;
		else
			/* Avoid exception caused by division by zero */
			if (v_x1_u32r != BMP280_INIT_VALUE)
				v_pressure_u32 = (v_pressure_u32 / (BMP280_U32_t)v_x1_u32r) * 2;
			else
				return BMP280_INVALID_DATA;
		/* calculate x1*/
		v_x1_u32r = (((BMP280_S32_t)obj->bmp280_cali.dig_P9) * ((BMP280_S32_t)(
			((v_pressure_u32
				>> BMP280_SHIFT_BIT_POSITION_BY_03_BITS)
				* (v_pressure_u32
					>> BMP280_SHIFT_BIT_POSITION_BY_03_BITS))
		...
		/* calculate x2*/
		v_x2_u32r = (((BMP280_S32_t)(v_pressure_u32 >>
			BMP280_SHIFT_BIT_POSITION_BY_02_BITS))
			* ((BMP280_S32_t)obj->bmp280_cali.dig_P8))
			>> BMP280_SHIFT_BIT_POSITION_BY_13_BITS;
		/* calculate true pressure*/
		v_pressure_u32 = (BMP280_U32_t)((BMP280_S32_t)v_pressure_u32 + ((v_x1_u32r + v_x2_u32r
			+ obj->bmp280_cali.dig_P7)
			>> BMP280_SHIFT_BIT_POSITION_BY_04_BITS));
		pressure = v_pressure_u32;
	}
	sprintf(buf, "%08x", pressure);
	if (atomic_read(&obj->trace) & BAR_TRC_IOCTL) {
		BAR_LOG("pressure: %d\n", pressure);
...
static int bmp_get_pressure(struct i2c_client *client, char *buf, int bufsize)
{
	int status = 0;
#ifdef __LP64__
	status = bmp_get_pressure_int64(client, buf, bufsize);
#else
	status = bmp_get_pressure_int32(client, buf, bufsize);
#endif
	return status;
}
...
static int bmp_check_pt(struct bmp_i2c_data *obj)
{
	...
	if ((err = kstrtoint(t, 16, &temperature)) < 0)
		BAR_ERR("sscanf parsing fail err = %d\n", err);
	...
	if ((err = kstrtoint(p, 16, &pressure)) < 0)
		BAR_ERR("sscanf parsing fail err = %d\n", err);
...
static ssize_t store_power_mode_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if ((err = kstrtoul(buf, 10, &power_mode)) < 0) {
		BAR_ERR("%s: kstrtoul error %d\n", __func__, err);
		return err;
	}

	err = bmp_set_powermode(obj->client, (enum BMP_POWERMODE_ENUM)(!!(power_mode)));
	if (err < 0) {
		BAR_ERR("%s: set power mode error %d\n", __func__, err);
		return err;
	}
	return count;
}
...
static DRIVER_ATTR(powermode, S_IRUGO | S_IWUSR, show_power_mode_value, store_power_mode_value);
			...
			int size_in, void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			if ((err = kstrtoint(buff, 16, &temperature_data->values[0])) < 0)
				BAR_ERR("sscanf parsing fail err = %d\n", err);
...
static long bmp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case BAROMETER_GET_PRESS_DATA:
		...
		if ((err = kstrtou32(strbuf, 16, &dat)) < 0)
			BAR_ERR("sscanf parsing fail err = %d\n", err);
...
static int bmp_get_data(int *value, int *status)
{
	...
	if ((err = kstrtoint(buff, 16, value)) < 0)
		BAR_ERR("sscanf parsing fail err = %d\n", err);
...
static int bmp_local_init(void)
{
	...
	if (-1 == bmp_init_flag) {
		BAR_ERR("%s: bmp_init_flag = %d\n", __func__, bmp_init_flag);
		return -1;
	}
