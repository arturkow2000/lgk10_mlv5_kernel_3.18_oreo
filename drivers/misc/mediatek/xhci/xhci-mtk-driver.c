...
int g_otg_state = OTG_NONE;
void mtk_set_otg_state(int state);
static int iddig_boost_enable = 0;
...
int g_otg_event = EVENT_OFF;

char *xhci_get_event_status(void)
{
	static char udev_event[128];
	snprintf(udev_event, 128, "EPACK_OTG=%s", event_str[g_otg_event]);
	return udev_event;
}

void xhci_send_event(int iddig, int epack)
{
	char udev_event[128];
	char *envp[2] = { udev_event, NULL };
	int ret;
	...
	if (iddig == 0 && epack == 1) {
		if (g_otg_event == EVENT_MAIN || g_otg_event == EVENT_OFF || g_otg_event == EVENT_MAIN_EPACK)
			event = EVENT_MAIN_EPACK;
		else /* if (g_otg_event == EVENT_EPACK || g_otg_event == EVENT_EPACK_MAIN) */
			event = EVENT_EPACK_MAIN;
	} else if (iddig == 0) {
		event = EVENT_MAIN;
	} else if (epack == 1) {
		event = EVENT_EPACK;
	...
	if (g_otg_event != event) {
		g_otg_event = event;
		snprintf(udev_event, 128, "EPACK_OTG=%s", event_str[event]);
		pr_info("epack send %s in %s\n", udev_event, kobject_get_path(&xhci_misc_uevent.this_device->kobj, GFP_KERNEL));
		ret = kobject_uevent_env(&xhci_misc_uevent.this_device->kobj, KOBJ_CHANGE, envp);
		if (ret < 0)
			pr_info("epack failed to send uevent. err( %d )\n", ret);
	}
...
{
	return iddig_boost_enable;
}
EXPORT_SYMBOL(mtk_otg_boost_is_on);
#endif
...
#ifdef CONFIG_INPUT_EPACK
	iddig_boost_enable = 1;
	pr_info("epack OTG_BOOST is ENABLED\n");
#endif
#endif
#ifdef CONFIG_LGE_PM_OTG_BOOST_MODE
	pr_info("[OTG] Charger boost mode : enable\n");
	set_otg_boost_mode(1);
#endif
}
...
#else
#ifdef CONFIG_INPUT_EPACK
	if (iddig_boost_enable == 1) {
		set_chr_enable_otg(0x0);
		iddig_boost_enable = 0;
		pr_info("epack OTG_BOOST is DISABLED\n");
	}
#else
	set_chr_enable_otg(0x0);
#endif
#endif
#if defined(CONFIG_LGE_PM_OTG_BOOST_MODE)
	pr_info("[OTG] Charger boost mode : disable\n");
	set_otg_boost_mode(0);
#endif
}
...
#ifdef CONFIG_INPUT_EPACK
	//epack_usb_otgdev_cnt_enable(true); /* branden.you_20170425 */
#endif
...
#ifdef CONFIG_INPUT_EPACK
	pr_info("epack OTG_DRIVER is LOADED\n");
#endif
...
{
#ifdef CONFIG_INPUT_EPACK
	//epack_usb_otgdev_cnt_enable(false); /* branden.you_20170425 */
#endif
...
#ifdef CONFIG_INPUT_EPACK
	pr_info("epack OTG_DRIVER is UN-LOADED\n");
#endif
}
...
{
	return (g_otg_state == OTG_MAIN) ? true : false;
}

bool mtk_is_otg_working(void)
{
	return (g_otg_state == OTG_MAIN ||g_otg_state == OTG_EPACK) ? true : false;
}

bool mtk_is_epack_only(void)
{
	return (g_otg_state == OTG_EPACK_ONLY) ? true : false;
}
...
void mtk_set_otg_state(int state)
{
	if (g_otg_state != state) {
		g_otg_state = state;
		pr_info("epack OTG_DEVICE is %s\n", otg_str[state]);
	}
}

void update_mtk_xhci_status(void)
{
	cancel_delayed_work(&mtk_xhci_delaywork);
	schedule_delayed_work(&mtk_xhci_delaywork, 0);
}
EXPORT_SYMBOL(update_mtk_xhci_status);
#endif
...
void mtk_xhci_mode_switch(struct work_struct *work)
{
	...
	int iddig = iddig_pin_state;
	int epack = epack_is_ok();

	if (g_bat_init_flag == 0) {
		schedule_delayed_work(&mtk_xhci_delaywork, msecs_to_jiffies(1000));
		return;
	}
	...
	if (get_boot_mode() == CHARGER_BOOT) {
		pr_info("epack BOOT_MODE : CHARGER_LOGO so do nothing in XHCI\n");
		return;
	}
#endif
	...
	if (!usb_cable_connected()) {
		if (iddig == 0 || epack == 1) {
			need_load = true;
			if (epack_is_otg_working() == 0) {
				xhci_send_event(iddig, epack);
				if (iddig == 0) {
					mtk_set_otg_state(OTG_MAIN);
					...
					mtk_set_otg_state(OTG_EPACK_ONLY);
					epack_set_usb_path(EPACK_USB_PATH_EPACK);
					if ( mtk_otg_boost_is_on() ) {
						mtk_disable_otg_mode();
...
static irqreturn_t xhci_eint_iddig_isr(int irqnum, void *data)
{
	...
	iddig_pin_state = 1 - iddig_pin_state;

	if (iddig_pin_state == 0) {
		pr_info("epack ISR = MAIN_OTG is ATTACHED\n");
		irq_set_irq_type(irqnum, IRQF_TRIGGER_HIGH);
	} else {
		pr_info("epack ISR = MAIN_OTG is DETACHED\n");
		irq_set_irq_type(irqnum, IRQF_TRIGGER_LOW);
	}
	...
	if (node) {
		...
		if (node) {
			...
			if (!retval) {
				...
				#ifdef CONFIG_INPUT_EPACK
				gpio_set_debounce(iddig_gpio, 100*1000);
				#endif
...
static int __init xhci_hcd_init(void)
{
	...
	#ifdef CONFIG_INPUT_EPACK
	{
		...
		retval = misc_register(&xhci_misc_uevent);
		if (retval) {
			pr_err("epack create the xhci_uevent_device fail, ret(%d)\n", retval);
		}
	}
	#endif
