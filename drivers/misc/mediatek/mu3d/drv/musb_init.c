...
static struct musb_hdrc_config mtu3d_config __refdata = {
	.multipoint = false,
	/* FIXME:Seems no need */
	/* .soft_con       = true, */
	.dma = true,
...
#ifdef CONFIG_LGE_PM_USB_ID
static struct pinctrl_state *pinctrl_id_adc;
#endif
#endif
...
#if defined CONFIG_USB_MTK_DUALMODE && defined CONFIG_LGE_PM_USB_ID
int mtk_switch_id_adc_mode(void)
{
	int ret = 0;

	if (IS_ERR(pinctrl)|| IS_ERR(pinctrl_id_adc)) {
		pr_err("%s pinctrl or pinctrl_id_adc is invalid\n", __func__);
		return 0;
	}

	if (mtk_is_host_mode()) {
		pr_err("%s mtk is host_mode\n", __func__);
		return -EPERM;
	}

	ret = pinctrl_select_state(pinctrl, pinctrl_id_adc);

	if (ret) {
		pr_err("%s pinctrl_id_adc fails\n", __func__);
		return ret;
	}
...
void mtk_switch_id_dig_mode(void)
{
	if (IS_ERR(pinctrl)|| IS_ERR(pinctrl_iddig)) {
		pr_err("%s pinctrl or pinctrl_iddig is invalid\n", __func__);
		return;
	}

	if (pinctrl_select_state(pinctrl, pinctrl_iddig)) {
		pr_err("%s pinctrl_iddig fails\n", __func__);
	}
}
#endif
...
static int mtu3d_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl))
		...
		#ifdef CONFIG_LGE_PM_USB_ID
		pinctrl_id_adc = pinctrl_lookup_state(pinctrl, "id_adc_init");
		if (IS_ERR(pinctrl_id_adc))
			dev_err(&pdev->dev, "Cannot find usb pinctrl pinctrl_id_adc\n");
		#endif
	}
#endif
