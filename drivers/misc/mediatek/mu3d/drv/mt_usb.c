...
extern bool mtk_is_otg_working(void);
extern bool mtk_is_epack_only(void);
bool usb_is_init = false;
#else
...
void connection_work(struct work_struct *data)
{
	...
	if (!usb_phy_check_in_uart_mode()) {
		...
		if ((is_usb_cable == true) && (connection_work_dev_status != ON)) {
...
#ifdef CONFIG_LGE_USB_G_ANDROID
			/* temp code - for USB wakelock debug */
			if (!wake_lock_active(&musb->usb_wakelock)) {
				wake_lock(&musb->usb_wakelock);
				os_printk(K_ERR, "%s usb_wakelock set\n", __func__);
			}
#else
			if (!wake_lock_active(&musb->usb_wakelock))
				wake_lock(&musb->usb_wakelock);
			...
			#ifdef CONFIG_INPUT_EPACK
			os_printk(K_ERR, "epack USB is CONNECTED\n");
			#endif
			...
			#ifdef CONFIG_INPUT_EPACK
			if (epack_is_ok()) {
				os_printk(K_INFO, "epack usb disconnected so call update_mtk_xhci_status()\n");
				msleep(200);
				update_mtk_xhci_status();
			} else {
				os_printk(K_INFO, "epack usb disconnected but epack is Not OK\n");
			}
			#endif
			...
			if (wake_lock_active(&musb->usb_wakelock)) {
				...
				os_printk(K_ERR, "%s usb_wakelock clear\n", __func__);
			}
#else
			if (wake_lock_active(&musb->usb_wakelock))
				wake_unlock(&musb->usb_wakelock);
			...
			#ifdef CONFIG_INPUT_EPACK
			os_printk(K_ERR, "epack USB is DIS-CONNECTED\n");
			#endif
