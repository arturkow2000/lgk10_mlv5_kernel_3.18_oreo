...
#endif
static int acc_calibration_read(int *cal);
static int acc_calibration_save(int *cal);
...
//    { I2C_BOARD_INFO(K303B_DEV_NAME, 0x3A >> 1) };
...
static int acc_calibration_save(int *cal)
{
	...
	int res;
	char temp_str[5];
	char *fname = SENSOR_CAL_FILENAME;
	...
	GSE_LOG("acc_calibration_save Done.\n");

	return 0;
}

static int acc_calibration_read(int *cal_read)
{
	int fd;
	int i;
	int res;
	...
	for(i = 0; i < 6; i++)
	{
		...
		GSE_LOG("acc_calibration_read : cal_read[%d]=%d\n", i, cal_read[i]);
	}

	sys_close(fd);
	set_fs(old_fs);

	GSE_LOG("acc_calibration_read Done.\n");
...
static int make_cal_data_file(void)
{
	...
	if(fd == -ENOENT)
	{
		GSE_LOG("[%s] Cal File doesn't exist. Need to make file !!!(%d)\n", __func__, fd);
	}
	...
	if(fd < 0)
	{
		...
		if(fd < 0)
		{
			GSE_ERR("[%s] Open or Make Cal File Error !!!(%d)\n", __func__, fd);
...
/*
		if(LGE_FacReadAccelerometerCalibration((unsigned int*)cal_misc) == TRUE)
		{
			...
			GSE_ERR("Read Cal from misc Error !!!\n");
		}
*/
      ...
      /* Sensor stable time after power on  */
       msleep ( 300 ); 
        ...
        for(i = 0; i < CALIBRATION_DATA_AMOUNT; i++)
        {
            ...
            SENSOR_LOG("(%d, %d, %d) (%d, %d, %d)\n", acc_cal_pre[0], acc_cal_pre[1], acc_cal_pre[2], acc_cal[0], acc_cal[1], acc_cal[2]);


            if((abs(acc_cal[0] - acc_cal_pre[0]) > K303B_SHAKING_DETECT_THRESHOLD)
                || (abs(acc_cal[1] - acc_cal_pre[1]) > K303B_SHAKING_DETECT_THRESHOLD)
                || (abs(acc_cal[2] - acc_cal_pre[2]) > K303B_SHAKING_DETECT_THRESHOLD))
            {
            ...
            else
            {
                ...
                sum[K303B_AXIS_X] += acc_cal[0];
                sum[K303B_AXIS_Y] += acc_cal[1];
                sum[K303B_AXIS_Z] += acc_cal[2];

                acc_cal_pre[0] = acc_cal[0];
                acc_cal_pre[1] = acc_cal[1];
                acc_cal_pre[2] = acc_cal[2];
       ...
       unsigned long arg)
{
	...
	switch(cmd)
	{
		case GSENSOR_IOCTL_SET_ENABLE:
			...
			SENSOR_LOG("GSENSOR_IOCTL_SET_ENABLE\n");
