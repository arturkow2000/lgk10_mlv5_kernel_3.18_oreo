...
void pwrap_dump_all_register(void)
{
	...
	PWRAPREG("pwrap_dump_all_register start (before reset)\n");
	pwrap_dump_ap_register();
	pwrap_dump_pmic_register();
	PWRAPREG("pwrap_dump_all_register end (before reset)\n\r");

	PWRAPREG("PMIC Wrap Read Test start (before reset)\n");
	pwrap_read_test();
	PWRAPREG("PMIC Wrap Read Test end   (before reset)\n");

	PWRAPREG("PMIC Wrap Write Test start (before reset)\n");
	pwrap_write_test();
	PWRAPREG("PMIC Wrap Write Test end   (before reset)\n");

	sub_return = pwrap_read_nochk(PMIC_HWCID_ADDR, &rdata);
	if (sub_return != 0) {
		PWRAPREG("PMIC Wrap Read HWCID fail, return = 0x%x\n", sub_return);
	}
	PWRAPREG("HWCID = 0x%x (before reset)\n\r", rdata);

	PWRAPREG("PMIC Wrap WACS2 Reset start\n");
	WRAP_WR32(PMIC_WRAP_WACS2_EN, DISABLE);
	WRAP_WR32(PMIC_WRAP_WACS2_EN, ENABLE);
	PWRAPREG("PMIC Wrap WACS2 Reset end\n\r");

	PWRAPREG("PMIC Wrap Read Test start (after reset)\n");
	pwrap_read_test();
	PWRAPREG("PMIC Wrap Read Test end   (after reset)\n");

	PWRAPREG("PMIC Wrap Write Test start (after reset)\n");
	pwrap_write_test();
	PWRAPREG("PMIC Wrap Write Test end   (after reset)\n");

	sub_return = pwrap_read_nochk(PMIC_HWCID_ADDR, &rdata);
	if (sub_return != 0) {
		PWRAPREG("PMIC Wrap Read HWCID fail, return = 0x%x\n", sub_return);
	}
	PWRAPREG("HWCID = 0x%x (after reset)\n\r", rdata);

	PWRAPREG("pwrap_dump_all_register start (after reset)\n");
	pwrap_dump_ap_register();
	pwrap_dump_pmic_register();
	PWRAPREG("pwrap_dump_all_register end (after reset)\n\r");
