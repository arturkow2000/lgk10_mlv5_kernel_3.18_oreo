...
{
	SEQ_printf(m, "Print %ld lines log in %llu second in last time.\n", print_num, second);
...
static ssize_t mt_log_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	...
	if (sscanf(buf, "%llu %d ", &second, &level) == 2) {
		SEQ_printf(NULL, "will print log in level %d about %llu second.\n", level, second);
	} else {
	...
	pr_err("mt log total write %ld line in %llu second.\n", print_num, second);
	return cnt;
}
#endif
