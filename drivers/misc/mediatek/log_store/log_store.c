...
#ifdef CONFIG_MTK_DRAM_LOG_STORE_LAST_LOG
static void *last_pl_lk;
static void *last_pl_lk_log;
...
struct mirror_log_header *m_log_header;
#endif
...
#ifdef CONFIG_MTK_DRAM_LOG_STORE_LAST_LOG
static int check_magic(void *s)
{
	int ret = 0;
	void *magic_s = s;

	if (!strncmp(magic_s, "PLLKINIT", 8))
		ret = 1;
	else
		ret = 0;
...
static int last_pl_lk_log_show(struct seq_file *m, void *v)
{
	if (!sram_dram_buff) {
		pr_err("%s: log_buff_header is null..\n", __func__);
		return 0;
	}

	if (!(sram_dram_buff->mirror_header_addr)) {
		return 0;
	}

	if (log_buff_header == NULL || pbuff == NULL) {
		seq_puts(m, "log buff is null for last_pl_lk.\n");
		return 0;
	}

	if (!last_pl_lk) {
		last_pl_lk = remap_lowmem(sram_dram_buff->mirror_header_addr, sizeof(struct mirror_log_header));
		m_log_header = (struct mirror_log_header *)(last_pl_lk);
	}

	if (!m_log_header)
		return 0;

	/* sequence of mirror buffer mlog[]
	 * [ ][ ]
	 * [1][ ]
	 * [1][2]
	 * [2][3]
	 ...
	 * ...
	 */
	if (m_log_header->count == 1) { /* warm reset */
		last_pl_lk_log = remap_lowmem(m_log_header->mlog[0].start_addr, (unsigned int)(LOG_STORE_SIZE_MIRROR >> 1));
	} else if (m_log_header->count > 1) { /* warm reset */
		last_pl_lk_log = remap_lowmem(m_log_header->mlog[1].start_addr, (unsigned int)(LOG_STORE_SIZE_MIRROR >> 1));
	} else { /* might be hard reset or status wrong */
		pr_err("%s: m_log_header->count(%d)is empty of wrong..",__func__ ,m_log_header->count);
		return 0;
	}
	if ((dram_log_store_status == BUFF_READY) && sram_header
		&& (sram_header->sig == SRAM_HEADER_SIG)) {
		if (last_pl_lk_log) {
			if (!check_magic(last_pl_lk_log)) {
				pr_err("log_store: magic number is incorrect for last pl_lk log!\n");
				seq_printf(m, "magic number is incorrect for last pl_lk log!\n");
				return 0;
			}
			seq_printf(m, "show buff for last pl_lk log!\n");
			seq_escape(m, last_pl_lk_log, "=END=");
		}
	}
	return 0;
}

static int last_pl_lk_file_open(struct inode *inode, struct file *file)
{
	return single_open(file, last_pl_lk_log_show, inode->i_private);
}

static const struct file_operations last_pl_lk_file_ops = {
	.owner = THIS_MODULE,
	.open = last_pl_lk_file_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#ifdef CONFIG_MTK_DRAM_LOG_STORE_LAST_LOG
	entry = proc_create("last_pl_lk", 0444, NULL, &last_pl_lk_file_ops);
	if (!entry) {
		pr_err("log_store: failed to create proc entry for last_pl_lk\n");
		return 1;
	}
	last_pl_lk = NULL;
#endif
