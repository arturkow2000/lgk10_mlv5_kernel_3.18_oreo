...
#define COMFORT_VIEW_OFF 0
#define COMFORT_VIEW_STEP_CNT 11
#endif
/* --------------------------------------------------------------------------- */
...
#ifdef CONFIG_HQ_SET_LCD_BIAS//PHB add for lcd_bias
	void (*set_lcd_bias_vspn) (unsigned int value);
	void (*set_lcd_bias_vspn_en) (unsigned int en, unsigned int seq);
#endif
...
typedef struct {
	...
	void (*reset_ctrl)(unsigned int enable, unsigned int delay);
#endif

#if defined(CONFIG_LGE_READER_MODE)
	void (*reader_mode)(LCM_READER_MODE mode);
#endif
#ifdef CONFIG_LGE_COMFORT_VIEW
        void (*comfort_view)(unsigned int mode);
#endif
