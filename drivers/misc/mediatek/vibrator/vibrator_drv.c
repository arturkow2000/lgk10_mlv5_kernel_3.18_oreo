...
static ssize_t store_level(struct device *dev,
		...
		const char *buf, size_t size)
{
	...
	vibr_set_power_level((unsigned int)level);
...
static ssize_t show_level(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d", hw->vib_vol);
}

static DEVICE_ATTR(level, 0660, show_level, store_level);
/******************************************************************************
 * vib_mod_init
 *
 * DESCRIPTION:
...
static int vib_mod_init(void)
{
	...
	if (ret)
		VIB_DEBUG("device_create_file level fail!\n");
