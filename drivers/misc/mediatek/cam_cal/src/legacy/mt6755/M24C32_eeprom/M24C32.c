 ...
 *
 */
// LGE_CHANGE_S: [2016-09-26] kyunghun.oh@lge.com, Add M24C32 EEPROM driver
#if 0
#include <linux/i2c.h>
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   EEPROMDB("[M24C32_EEPROM] iWriteData\n" );

   if (ui4_offset + ui4_length >= 0x2000)
   {
      EEPROMDB("[M24C32_EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!! \n" );
      return -1;
   }
   ...
   EEPROMDB("[M24C32_EEPROM] iWriteData u4CurrentOffset is %d \n",u4CurrentOffset);
   ...
   EEPROMDB("[M24C32_EEPROM] iWriteData done\n" );
#endif
   return 0;
}

int iReadDataFromM24C32(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
...
#endif//COMMON_CAM_CAL_DRV
{
    ...
    else
    {
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                EEPROMDB("[M24C32_EEPROM] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:
            EEPROMDB("[M24C32_EEPROM] Write CMD \n");
#ifdef EEPROMGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            EEPROMDB("[M24C32_EEPROM] Read CMD \n");
#ifdef EEPROMGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000CB24C) && (EEPROM_Address[0]==0x0) )
            {
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000CB24C) && (ResetCheck == 0xABCDFEDC) )
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);    
                EEPROMDB("[M24C32_EEPROM] Reset I2C address %x \n", *(u32 *)pWorkingBuff );
            ...
            else
            {
               ...
               {
                   i4RetValue =  iReadDataFromM24C32(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
                   EEPROMDB("[M24C32_EEPROM] error (%d) Read retry (%d) \n", i4RetValue,readTryagain);
                   if(i4RetValue != 0)
                   {
                       readTryagain--;
                   }
            ...
            EEPROMDB("[M24C32_EEPROM] After read Working buffer data  0x%4x \n", *pWorkingBuff);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            EEPROMDB("[M24C32_EEPROM] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    EEPROMDB("[M24C32_EEPROM] EEPROM_Open\n");
...
inline static int RegisterEEPROMCharDrv(void)
{
    ...
    {
        EEPROMDB("[M24C32_EEPROM] Allocate device no failed\n");
    ...
    {
        EEPROMDB("[M24C32_EEPROM] Register device no failed\n");
    ...
    if(NULL == g_pEEPROM_CharDrv)
    {
        ...
        EEPROMDB("[M24C32_EEPROM] Allocate mem for kobject failed\n");
    ...
    {
        EEPROMDB("[M24C32_EEPROM] Attatch file operation failed\n");
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
int i4RetValue = 0;
    EEPROMDB("[M24C32_EEPROM] Attach I2C \n");
//    spin_lock_init(&g_EEPROMLock);

    //get sensor i2c client
    spin_lock(&g_EEPROMLock); //for SMP
    g_pstI2Cclient = client;
    g_pstI2Cclient->addr = M24C32_DEVICE_ID>>1;
    ...
    if(i4RetValue){
        EEPROMDB("[M24C32_EEPROM] register char device failed!\n");
        return i4RetValue;
    }


    EEPROMDB("[M24C32_EEPROM] Attached!! \n");
    return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
    ...
    if(platform_driver_register(&g_stEEPROM_Driver)){
        EEPROMDB("failed to register M24C32_EEPROM driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stEEPROM_Device))
    {
        EEPROMDB("failed to register M24C32_EEPROM driver, 2nd time\n");
        return -ENODEV;
    }
...
void read_moduleID_M24C32(unsigned char * pinputdata)
{
    unsigned int module_vendor_id = 0x700;
    unsigned int offset = 0xCB24C;
    unsigned int module_vendor_id_temp = 0;

    module_vendor_id_temp = (module_vendor_id<<20)&0xfff00000;
    module_vendor_id_temp |= offset;
    iReadDataFromM24C32(module_vendor_id_temp, 2, pinputdata);
	
    return;
}

#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(M24C32_EEPROM_Ioctl);
#else
...
MODULE_LICENSE("GPL");
#endif
// LGE_CHANGE_E: [2016-09-26] kyunghun.oh@lge.com, Add M24C32 EEPROM driver
