...
int iWriteEEPROM_ZC533(u16 a_u2Addr  , u32 a_u4Bytes, u8 * puDataInBytes)
{
    ...
    if (a_u4Bytes + 2 > 8) {
        EEPROMDB("[ZC533EEPROM] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
        return -1;
    }
...
int iReadData_ZC533(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   u32 u4IncOffset = 0;
    u32 u4CurrentOffset;
    u8 *pBuff;
    /* EEPROMDB("[ZC533EEPROM] iReadData\n" ); */
    ...
    do {
        if (i4ResidueDataLength >= 8) {
            ...
            i4RetValue = iReadEEPROM_ZC533((u16)u4CurrentOffset, i4ResidueDataLength, pBuff);
            if (i4RetValue != 0) {
                EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
                return -1;
            }
...
//   EEPROMDB("[ZC533EEPROM] iReadData finial address is %d length is %d buffer address is 0x%x\n",u4CurrentOffset, i4ResidueDataLength, pBuff);
//   EEPROMDB("[ZC533EEPROM] iReadData done\n" );
   return 0;
}
    ...
    case CAM_CALIOC_G_READ:
        ...
        if (ptempbuf->u4Offset == 0x0A04C32A) {
            *(u32 *)pWorkingBuff = 0x1A04C32A;
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0000C533) && (EEPROM_Address[0]==0x0))
            {
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0000C533) && (ResetCheck == 0xABCDFEDC) )
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
            ...
            else
            {
               ...
               {
                   i4RetValue =  iReadDataFromZC533(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
...
static unsigned short force[] = {EEPROM_I2C_GROUP_ID, ZC533_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};
#else
...
EXPORT_SYMBOL(read_moduleID_ZC533); // LGE_CHANGE: [2015-11-09] yonghwan.lym@lge.com, Add Module Vendor ID - bug fix
#else
