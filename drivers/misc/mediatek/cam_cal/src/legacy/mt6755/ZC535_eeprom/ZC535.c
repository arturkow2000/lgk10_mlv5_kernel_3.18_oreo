...
int iWriteEEPROM_ZC535(u16 a_u2Addr  , u32 a_u4Bytes, u8 * puDataInBytes)
{
	u32 u4Index;
	int i4RetValue;
	char puSendCmd[8] = {
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB("[ZC535EEPROM] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		EEPROMDB("[ZC535EEPROM] I2C write  failed!!\n");
		return -1;
	}
...
int iReadEEPROM_ZC535(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
	int  i4RetValue = 0;
	...
	if (ui4_length > 8) {
		EEPROMDB("[ZC535EEPROM] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
int iWriteData_ZC535(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
//   int  i4RetValue = 0;
...
//	u8 *pBuff;
	...
	EEPROMDB("[ZC535EEPROM] iWriteData\n");

#if 0
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[ZC535EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	EEPROMDB("[ZC535EEPROM] iWriteData u4CurrentOffset is %d\n", u4CurrentOffset);
...
#endif
	EEPROMDB("[ZC535EEPROM] iWriteData done\n");
...
int iReadDataFromZC535(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				...
				//i4RetValue = iReadRegI2C(puSendCmd , 2, (u8*)pBuff,16,EEPROM_Address[loopCount]);
				if (i4RetValue != 0) {
...
int iReadData_ZC535(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
   int  i4ResidueDataLength;
   u32 u4IncOffset = 0;
	u32 u4CurrentOffset;
	u8 *pBuff;
	/* EEPROMDB("[ZC535EEPROM] iReadData\n" ); */

	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[ZC535EEPROM] Read Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadEEPROM_ZC535((u16)u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
			...
			i4RetValue = iReadEEPROM_ZC535((u16)u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
...
//   EEPROMDB("[ZC535EEPROM] iReadData finial address is %d length is %d buffer address is 0x%x\n",u4CurrentOffset, i4ResidueDataLength, pBuff);
//   EEPROMDB("[ZC535EEPROM] iReadData done\n" );
   return 0;
}
...
*/
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
			EEPROMDB("[ZC535EEPROM] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff , (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				EEPROMDB("[ZC535EEPROM] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	...
	EEPROMDB("[ZC535EEPROM] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff, (u32)a_u4Command);
	...
	case CAM_CALIOC_S_WRITE:
		EEPROMDB("[ZC535EEPROM] Write CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		i4RetValue = iWriteData_ZC535((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
#ifdef EEPROMGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		...
		EEPROMDB("[ZC535EEPROM] After read Working buffer data  0x%4x\n", *pWorkingBuff);
#else
		...
		if (ptempbuf->u4Offset == 0x0A04C32A) {
			*(u32 *)pWorkingBuff = 0x1A04C32A;
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0000C535) && (EEPROM_Address[0]==0x0))
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
                ...
                EEPROMDB("[ZC535EEPROM] Init I2C address %x \n", *(u32 *)pWorkingBuff );
            }
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0000C535) && (ResetCheck == 0xABCDFEDC) )
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);


                EEPROMDB("[ZC535EEPROM] Reset I2C address %x \n", *(u32 *)pWorkingBuff );
            }
            else
            {
               readTryagain=3;
               ...
               {
                   i4RetValue =  iReadDataFromZC535(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
                   EEPROMDB("[ZC535EEPROM] error (%d) Read retry (%d) \n", i4RetValue,readTryagain);
                   if(i4RetValue != 0)
                   {
                       readTryagain--;
                   }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        EEPROMDB("[ZC535EEPROM] to user length %d \n", ptempbuf->u4Length);
        EEPROMDB("[ZC535EEPROM] to user  Working buffer address 0x%8x \n", (u32)pWorkingBuff);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            EEPROMDB("[ZC535EEPROM] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    EEPROMDB("[ZC535EEPROM] EEPROM_Open\n");
	spin_lock(&g_EEPROMLock);
	if (g_u4Opened) {
		spin_unlock(&g_EEPROMLock);
		return -EBUSY;
...
static inline int RegisterEEPROMCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[ZC535EEPROM] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_EEPROMdevno , 1 , EEPROM_DRVNAME)) {
		EEPROMDB("[ZC535EEPROM] Register device no failed\n");
	...
	if (NULL == g_pEEPROM_CharDrv) {
		...
		EEPROMDB("[ZC535EEPROM] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
		EEPROMDB("[ZC535EEPROM] Attatch file operation failed\n");
...
static unsigned short force[] = {EEPROM_I2C_GROUP_ID, ZC535_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};
#else
//static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, ZC535_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};
#endif
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	EEPROMDB("[ZC535EEPROM] Attach I2C\n");
	/* spin_lock_init(&g_EEPROMLock); */

    //get sensor i2c client
    spin_lock(&g_EEPROMLock); //for SMP
    g_pstI2Cclient = client;
    g_pstI2Cclient->addr = ZC535_DEVICE_ID>>1;
    spin_unlock(&g_EEPROMLock); // for SMP
	...
	if (i4RetValue) {
		EEPROMDB("[ZC535EEPROM] register char device failed!\n");
		return i4RetValue;
	}


	EEPROMDB("[ZC535EEPROM] Attached!!\n");
	return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_Driver)) {
		EEPROMDB("failed to register ZC535EEPROM driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stEEPROM_Device)) {
		EEPROMDB("failed to register ZC535EEPROM driver, 2nd time\n");
		return -ENODEV;
	}
...
void read_moduleID_ZC535(unsigned char * pinputdata)
{
unsigned int module_vendor_id = 0xbe0;
unsigned int offset = 0x0C535;
unsigned int module_vendor_id_temp = 0;

module_vendor_id_temp = (module_vendor_id<<20)&0xfff00000;
module_vendor_id_temp |= offset;
iReadDataFromZC535(module_vendor_id_temp, 2, pinputdata);

return;
}
#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(ZC535_EEPROM_Ioctl);
EXPORT_SYMBOL(read_moduleID_ZC535);
#else
