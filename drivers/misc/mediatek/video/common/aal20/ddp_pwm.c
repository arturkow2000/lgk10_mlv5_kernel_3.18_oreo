...
static int tuning_brightness = 0;
static unsigned int current_brightness;
#endif

#ifdef CONFIG_MTK_AAL_SUPPORT
#if defined(CONFIG_LEDS_DW8767_CV1) || defined(CONFIG_LEDS_DW8767_CV7A)
extern void dw8767_reg_bl_en_ctrl(unsigned int enable);
#endif
#endif

#if defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1) || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A)
#define PWM_MIN_ON_LEVEL 6
#define PWM_MAX_ON_LEVEL 1021
#define PWM_MAX_BRIGHTNESS_LEVEL 1023
#endif
...
static void disp_pwm_set_enabled(cmdqRecHandle cmdq, disp_pwm_id_t id, int enabled)
{
	...
	if (old_en != enabled) {
		if (enabled) {
#if defined(CONFIG_MTK_AAL_SUPPORT)
#ifndef CONFIG_LEDS_DW8767_CV7A
...
static int disp_pwm_level_remap(disp_pwm_id_t id, int level_1024)
{
	...
	if (level_1024 > 1017)
		level_1024 = 1017;
#elif defined (CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
	if (level_1024 > 0 && level_1024 < PWM_MIN_ON_LEVEL)
		level_1024 = PWM_MIN_ON_LEVEL;
	else if (level_1024 > PWM_MAX_ON_LEVEL)
		level_1024 = PWM_MAX_ON_LEVEL;
#else
	/* do nothing */
#endif
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
	...
	if (atomic_read(&g_pwm_is_clock_on) == 0) {
		PWM_ERR("[ERROR] primary display is slept, backlight control is prohibited");
		return -EFAULT;
	}
...
#if defined(CONFIG_LGE_LCM_BACKLIGHT_PWM)
		if (tuning_brightness == 0)
			level_1024 = disp_pwm_level_remap(id, level_1024);
		else
			current_brightness = level_1024;
#else
		level_1024 = disp_pwm_level_remap(id, level_1024);
#endif
...
{
	tuning_brightness = 1;
	disp_pwm_set_backlight(disp_pwm_get_main(), level);
	tuning_brightness = 0;
}
