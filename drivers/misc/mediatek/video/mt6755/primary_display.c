...
#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR) || defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) \
	|| defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) \
	 || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
extern int mfts_lpwg;
extern int esd_recovery_status;
...
extern unsigned int disp_lcm_get_comfort_view(disp_lcm_handle *plcm);
extern int disp_lcm_set_comfort_view(disp_lcm_handle *plcm, unsigned int new_mode);
#endif
...
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
unsigned int primary_display_changing = false;
...
DISP_POWER_STATE primary_set_state(DISP_POWER_STATE new_state)
{
...
#ifdef CONFIG_LGE_VSYNC_SKIP
	static struct vsync_skip_data *vsync_skip;

	vsync_skip = vsync_skip_get_data();
	vsync_skip->fps_cnt++;
#endif
...
static void DC_config_nightlight(struct cmdqRecStruct *cmdq_handle)
{
	...
	if (all_zero)
		DISPERR("Night light backup param is zero matrix\n");
	else
...
int primary_display_suspend(void)
{
	...
	DISPMSG("[POWER]lcm suspend[end]\n");
#else
#if defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
	if (gSetShutdown != STATUS_ON) {
		if(lge_get_mfts_mode() == STATUS_ON && mfts_lpwg != STATUS_ON)
			disp_lcm_suspend_for_mfts(pgc->plcm);
	}
#elif defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5)
	if((lge_get_mfts_mode() == STATUS_ON && mfts_lpwg != STATUS_ON) || (gSetShutdown == STATUS_ON))
		disp_lcm_suspend_for_mfts(pgc->plcm);
#endif
#endif
	...
	|| defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) \
	 || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
	printk("[DISP] esd_recovery_status suspend = %d\n", esd_recovery_status);
	if (esd_recovery_status == false) {
#endif
...
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
	primary_display_changing = false;
#endif

done:
#if defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
	if (gSetShutdown == STATUS_ON) {
		DISPDBG("[POWER]lcm shutdown[begin]\n");
		disp_lcm_shutdown(pgc->plcm);
		DISPMSG("[POWER]lcm shutdown[end]\n");
	}
#elif defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5)
	if((lge_get_mfts_mode() == STATUS_ON && mfts_lpwg != STATUS_ON) || (gSetShutdown == STATUS_ON)){
		DISPDBG("[POWER]lcm shutdown[begin]\n");
...
int primary_display_resume(void)
{
...
#ifdef CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A
	disp_lcm_reset_ctrl(pgc->plcm,0,0);
#endif
    ...
    || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)\
     || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
	printk("[DISP] esd_recovery_status resume = %d\n", esd_recovery_status);
	if (esd_recovery_status == false) {
	...
	primary_display_changing = false;
#endif
#if defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
	mdelay(50);
#endif
...
void primary_display_set_power_mode(unsigned int mode)
{
	DISPMSG("%s start, mode = %d \n", __func__, mode);

	disp_lcm_set_power_mode(pgc->plcm,mode);

	DISPMSG("%s done \n", __func__);
}

#if defined(CONFIG_LGE_READER_MODE)
LCM_READER_MODE primary_display_get_reader_mode(void)
{
    return disp_lcm_get_reader_mode(pgc->plcm);
}
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
int _dsi_set_reader_mode_cmdq_vdo(disp_lcm_handle *plcm, LCM_READER_MODE mode)
{
	int ret = 0;
	int event_ret;
	...
	if (pgc->session_mode == DISP_SESSION_RDMA_MODE
	   || pgc->session_mode == DISP_SESSION_DECOUPLE_MODE) {
		/* switch back to DL mode before stop dsi vdo mode */
		DISPMSG("switch back to DL mode\n");
		do_primary_display_switch_mode(DISP_SESSION_DIRECT_LINK_MODE,
					pgc->session_id, 0, NULL, 1);
	}
	...
	if (ret != 0) {
		DISPERR("fail to create primary cmdq handle for set reader mode\n");
		return -1;
	}
	...
	if (primary_display_is_video_mode()) {
		...
		if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
			event_ret = dpmgr_wait_event_timeout(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_DONE, HZ * 1);
			DISPDBG("[POWER]primary display path is busy now, wait frame done, event_ret=%d\n", event_ret);
		}

		_cmdq_stop_trigger_loop();

		/* 2. stop dsi vdo mode */
		DISPMSG("+ %s, stop dsi vdo mode\n", __func__);
		dpmgr_path_stop(pgc->dpmgr_handle, CMDQ_DISABLE);

		/* 3. write instruction(read from lcm) */
		DISPMSG("VDO=>disp_lcm_set_reader_mode\n");
		disp_lcm_set_reader_mode(plcm, mode);
		/* 4. start dsi vdo mode */
		DISPMSG("- %s, start dsi vdo mode\n", __func__);
		dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);
		...
		dpmgr_path_trigger(pgc->dpmgr_handle, cmdq_handle, CMDQ_ENABLE);
		/* mutex sof wait */
		ddp_mutex_set_sof_wait(dpmgr_path_get_mutex(pgc->dpmgr_handle), cmdq_handle , 0);

		/* 6. flush instruction */
		ret = cmdqRecFlush(cmdq_handle);
	}
	else {
		DISPMSG("primary_display is cmd mode\n");
		return -1;
	}
...
#endif
void primary_display_set_reader_mode(LCM_READER_MODE mode)
{
        _primary_path_lock(__func__);
	if (pgc->state == DISP_SLEPT)
	{
		DISPMSG("primary display path is already slept, skip\n");
                _primary_path_unlock(__func__);
        	return;
	}
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
	_primary_path_unlock(__func__);
	if (primary_display_cmdq_enabled())
		_dsi_set_reader_mode_cmdq_vdo(pgc->plcm, mode);
#else
	disp_lcm_set_reader_mode(pgc->plcm, mode);
        _primary_path_unlock(__func__);
#endif
}
#endif
...
#ifdef CONFIG_LGE_COMFORT_VIEW
#ifdef CONFIG_LGD_INCELL_LG4894_HD_LV5
int set_comfort_view_cmdq_vdo(disp_lcm_handle *plcm, unsigned int mode)
{
	int ret = 0;
	int event_ret;
	struct cmdqRecStruct *cmdq_handle = NULL;
	...
	if (primary_display_is_video_mode()) {
		...
		/* 3. write instruction(read from lcm) */
		DISPMSG("VDO=>disp_lcm_set_comfort_view\n");
		disp_lcm_set_comfort_view(pgc->plcm, mode);
		/* 4. start dsi vdo mode */
		DISPMSG("- %s, start dsi vdo mode\n", __func__);
		dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);
...
unsigned int primary_display_get_comfort_view(void)
{
        return disp_lcm_get_comfort_view(pgc->plcm);
}

void primary_display_set_comfort_view(unsigned int mode)
{
        _primary_path_switch_dst_lock();
        _primary_path_lock(__func__);
        if (pgc->state == DISP_SLEPT) {
                if (pgc->plcm->cur_comfort_view != mode)
                        pgc->plcm->cur_comfort_view = mode;

                DISPMSG("primary display path is already slept, set comfort view skip\n");
                _primary_path_unlock(__func__);
                _primary_path_switch_dst_unlock();
                return;
        }
        ...
        _primary_path_switch_dst_unlock();
        if (primary_display_cmdq_enabled())
			set_comfort_view_cmdq_vdo(pgc->plcm, mode);
#else
        primary_display_idlemgr_kick((char *)__func__, 0);
...
LCM_PARAMS* primary_display_get_lcm_params(void)
{
	LCM_PARAMS *lcm_param = NULL;
	if (pgc->plcm)
		lcm_param = disp_lcm_get_params(pgc->plcm);
...
#endif
#if defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
...
void primary_display_set_touch_osc(int enable)
{
        _primary_path_lock(__func__);
        if(pgc->state == DISP_SLEPT)
        {
        ...
        disp_lcm_set_touch_osc(pgc->plcm, enable);
        if(pgc->state == DISP_SLEPT)
        {
