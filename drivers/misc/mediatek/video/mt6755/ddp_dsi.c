	...
	unsigned char *para_list, unsigned int count)
{
	...
	unsigned char additional_para_list[2] = {0x0f, 0xf0};  //tell esd mechanism that the value 0x0f/0xf0 of 45H is also normal
#endif
...
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
			if ((dsi_params->lcm_esd_check_table[index].cmd == 0x45) && (buffer[i] == additional_para_list[i]))
					continue;  //tell esd mechanism that the value 0x0f/0xf0 of 45H is also normal
#endif
				ret = 1;
...
#ifdef CONFIG_HQ_SET_LCD_BIAS
static void lcm_set_lcd_bias_vspn(unsigned int value)
{
	lcd_bias_set_vspn(value);
}

static void lcm_set_lcd_bias_vspn_en(unsigned int en, unsigned int seq)
{
	lcd_bias_set_vspn_en(en, seq);
}
#endif
...
#else
#ifdef CONFIG_HQ_SET_LCD_BIAS
	utils->set_lcd_bias_vspn = lcm_set_lcd_bias_vspn;
	utils->set_lcd_bias_vspn_en = lcm_set_lcd_bias_vspn_en;
#endif
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			t0.Data_ID =
			    (t0.Data0 <
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
			     0xB0) ? DSI_DCS_READ_PACKET_ID : DSI_DCS_READ_PACKET_ID;
#else
			     0xB0) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;
#endif
			t0.Data1 = 0;
