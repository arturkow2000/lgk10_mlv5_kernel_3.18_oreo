...
static int set_memory_buffer(disp_session_input_config *input)
{
	...
	if (input->config_layer_num == 0
	    || input->config_layer_num > MEMORY_SESSION_INPUT_LAYER_COUNT) {
		DISPPR_ERROR("set_memory_buffer, config_layer_num invalid = %d!\n",
	...
	for (i = 0; i < input->config_layer_num; i++) {
		...
		if (layer_id > DISP_SESSION_TIMELINE_COUNT) {
			DISPPR_ERROR("set_memory_buffer, layer_id invalid = %d!\n",
				layer_id);
			return 0;
		}
...
static int set_external_buffer(disp_session_input_config *input)
{
	...
	if (input->config_layer_num == 0
	    || input->config_layer_num > EXTERNAL_SESSION_INPUT_LAYER_COUNT) {
