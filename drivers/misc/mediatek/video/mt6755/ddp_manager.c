...
int dpmgr_path_pwm_power_on(disp_path_handle dp_handle, CMDQ_SWITCH encmdq)
{
	DDP_MANAGER_CONTEXT *context = _get_context();
	ddp_path_handle handle;
	...
	mutex_lock(&context->mutex_lock);
	path_top_clock_on();
	ddp_modules_driver[DISP_MODULE_PWM0]->power_on(DISP_MODULE_PWM0,
...
int dpmgr_path_pwm_power_off(disp_path_handle dp_handle, CMDQ_SWITCH encmdq)
{
        DDP_MANAGER_CONTEXT *context = _get_context();
        ddp_path_handle handle;
        ...
        mutex_lock(&context->mutex_lock);
        ddp_modules_driver[DISP_MODULE_PWM0]->power_off(DISP_MODULE_PWM0,
                                           encmdq ? handle->cmdqhandle :
                                           NULL);
        handle->power_sate = 0;
        path_top_clock_off();
