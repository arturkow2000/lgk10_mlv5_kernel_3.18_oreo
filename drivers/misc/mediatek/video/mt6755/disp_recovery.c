...
extern DISP_POWER_STATE primary_set_state(DISP_POWER_STATE new_state);
static DEFINE_MUTEX(td4100_esd_lock);
#endif
#if defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5)
extern unsigned int primary_display_esd_checking;
#endif
	...
	while (1) {
		...
		if (esd_check_enable) {
			...
			do {
				...
				primary_display_esd_checking = true;
#endif
...
#if defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
		primary_display_esd_checking = false;
#endif
...
int primary_display_esd_recovery_suspend(void)
{
	...
	disp_lcm_suspend_for_mfts(primary_get_lcm());
#endif
...
int primary_display_esd_recovery_resume(void)
{
	...
	DISPDBG("[ESD]lcm resume[begin]\n");
#if defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5)
	disp_lcm_init(primary_get_lcm(), 1);
#else
	disp_lcm_resume(primary_get_lcm());
#endif
	DISPMSG("[ESD]lcm resume[end]\n");

#if defined(CONFIG_LGE_READER_MODE)
	disp_lcm_set_reader_mode(primary_get_lcm(), primary_get_lcm()->cur_reader_mode);
#endif

#ifdef CONFIG_LGE_COMFORT_VIEW
        disp_lcm_set_comfort_view(primary_get_lcm(), primary_get_lcm()->cur_comfort_view);
#endif
	...
	DISPDBG("[ESD]lcm resume_cmd[begin]\n");
	disp_lcm_resume_cmd_esd(primary_get_lcm());
	DISPMSG("[ESD]lcm resume_cmd[end]\n");
...
void primary_display_check_recovery_init(void)
{
	...
	/* Do not use Display ESD recovery */
	esd_init = 1;
#endif
	if (!esd_init) {
...
int  esd_recovery_status;
extern int store_esd_recovery_suspend(void);
extern int store_esd_recovery_resume(void);
#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR)
extern bool esd_recovery_state;
#endif
...
void LG_ESD_recovery(void)
{
	...
	mutex_lock(&td4100_esd_lock);

	if( get_voice_status() == true || get_voice_md2_status() == true ){
		store_esd_recovery_suspend();
		store_esd_recovery_resume();
	}
	else{
		esd_recovery_status = true;
#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
		primary_display_esd_checking = true;
#endif

		primary_display_manual_lock();
		ret = primary_display_esd_recovery_suspend();

		if(ret == DISP_RECOVERY_SKIP) {
			DISPERR("%s : skip recovery path",__func__);
			primary_display_manual_unlock();
			goto done;
		}
...
#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR)
        esd_recovery_state = false;
#endif

done:
	esd_recovery_status = false;

#if defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
	primary_display_esd_checking = false;
#endif

	mutex_unlock(&td4100_esd_lock);
}
