...
#endif
#if defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_LGE_USE_TOUCH_NOTIFIER) || \
	defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR)
...
#if defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_LGE_USE_TOUCH_NOTIFIER) || \
	defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
enum {
	LCD_MODE_U0 = 0,
	LCD_MODE_U2_UNBLANK,
	LCD_MODE_U2,
...
static int vsync_cnt;
	...
	defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) \
	|| defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
unsigned int gSetShutdown;
int display_status;
#endif
...
static struct class *comfortviewClass = NULL;
static struct device *dev_comfortview = NULL;
#endif
#ifdef CONFIG_OF
...
#ifdef CONFIG_OF
static int __init _parse_tag_videolfb(void);
#endif
#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR) || defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || \
	defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) \
	|| defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
...
{
	if (gSetShutdown != 1 && display_status == 1)
	{
	    LG_ESD_recovery();
	}
	else
...
#if defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A)
static void mtkfb_blank_pre_event_handler(int event)
{
	switch (event) {
		case FB_BLANK_UNBLANK:
		case FB_BLANK_NORMAL:
			touch_notifier_call_chain(LCD_EVENT_LCD_UNBLANK, (void *)&event);
			break;
		case FB_BLANK_POWERDOWN:
			touch_notifier_call_chain(LCD_EVENT_LCD_BLANK, (void *)&event);
			break;
		default:
			break;
	}
...
#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR) || defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) \
    || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SRR) || defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
		display_status = 0;
#endif
		mtkfb_blank_suspend();
#if defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_LGE_USE_TOUCH_NOTIFIER) || \
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
        ...
        set_value = simple_strtoul(buf, last, 0);
...
static DEVICE_ATTR(esd_recovery, S_IRWUGO, NULL, store_esd_recovery);
...
{
	ssize_t r = 0;
	LCM_PARAMS *lcm_param = primary_display_get_lcm_params();
	if(lcm_param != NULL)
...
static ssize_t reader_mode_show(struct device *device, struct device_attribute *attr,
        char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", primary_display_get_reader_mode());
}

static ssize_t reader_mode_store(struct device *device,
        ...
        const char *buf, size_t count)
{
    ...
    if(set_value < READER_MODE_OFF || set_value > READER_MODE_MONO)
        set_value = READER_MODE_OFF;

    primary_display_set_reader_mode(set_value);
...
static DEVICE_ATTR(reader_mode, S_IRUGO|S_IWUSR, reader_mode_show, reader_mode_store);
#endif
#ifdef CONFIG_LGE_COMFORT_VIEW
static ssize_t comfort_view_show(struct device *device, struct device_attribute *attr, char *buf)
{
        return snprintf(buf, PAGE_SIZE, "%d\n", primary_display_get_comfort_view());
}

static ssize_t comfort_view_store(struct device *device, struct device_attribute *attr, const char *buf, size_t count)
{
        char **last = NULL;
        int err = 0;
        unsigned int set_value = 0;
        ...
        if(set_value < COMFORT_VIEW_OFF || set_value >= COMFORT_VIEW_STEP_CNT )
                set_value = COMFORT_VIEW_OFF;

        primary_display_set_comfort_view(set_value);
...
static DEVICE_ATTR(comfort_view, S_IRUGO|S_IWUSR, comfort_view_show, comfort_view_store);
#endif

#if defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR) || defined(CONFIG_SW49106_FHDPLUS_DSI_VDO_LGD_CV5) || \
	defined(CONFIG_SW49107_LGD_FHDPLUS_DSI_CMD_CV7A) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3_SPR) || defined(CONFIG_LGD_INCELL_LG4894_HD_LV5) \
	|| defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1)
int store_esd_recovery_suspend(void)
{
	...
	err = fb_blank(fb_info, FB_BLANK_POWERDOWN);
	fb_info->flags &= ~FBINFO_MISC_USEREVENT;
	console_unlock();
...
int store_esd_recovery_resume(void)
{
	struct fb_info *fb_info = mtkfb_fbi;
	int err = 0;
	...
	console_lock();
	fb_info->flags |= FBINFO_MISC_USEREVENT;
	err |= fb_blank(fb_info, FB_BLANK_UNBLANK);
	fb_info->flags &= ~FBINFO_MISC_USEREVENT;
	console_unlock();
...
#if defined(CONFIG_LGE_VSYNC_SKIP) || defined(CONFIG_LGE_READER_MODE) || defined(CONFIG_TUNING_MIPI_CLOCK)
...
#ifdef CONFIG_LGE_COMFORT_VIEW
void mtkfb_register_comfortview(void)
{
        if(!comfortviewClass) {
                comfortviewClass= class_create(THIS_MODULE, "panel");
                if (IS_ERR(comfortviewClass))
                        pr_err("%s: Failed to create comfortview class\n", __func__);
        }
        if (!dev_comfortview) {
                dev_comfortview = device_create(comfortviewClass, NULL, 0, NULL, "img_tune");
                if (IS_ERR(dev_comfortview)) {
                        pr_err("%s: Failed to create dev(comfortview)!", __func__);
                } else {
                        if (device_create_file(dev_comfortview, &dev_attr_comfort_view) < 0)
...
{
#if defined(CONFIG_LGE_READER_MODE) || defined(CONFIG_LGE_VSYNC_SKIP) || defined(CONFIG_LGE_COMFORT_VIEW)
    int rc;
    struct device* dev = fbdev->fb_info->dev;

#ifdef CONFIG_LGE_COMFORT_VIEW
    mtkfb_register_comfortview();
#endif
    if(!dev)
        return -EFAULT;
...
{
#if defined(CONFIG_LGE_READER_MODE) || defined(CONFIG_LGE_VSYNC_SKIP)  || defined(CONFIG_TUNING_MIPI_CLOCK)
    struct device* dev = fbdev->fb_info->dev;
	if (dev)
...
{
	//_parse_tag_videolfb();
	if(is_videofb_parse_done != 1)
		pr_warn("uninitialized calls from %s\n", __func__);
...
#ifdef CONFIG_OF
early_initcall(_parse_tag_videolfb);
#endif
