...
#if defined(CONFIG_LGE_HANDLE_PANIC) && \
	defined(CONFIG_LGE_HANDLE_PANIC_MTK_EMI_MPU)
			pr_err("[EMI MPU] EMI_MPUS = 0x%x,EMI_MPUT = 0x%x\n"
				"CHKER = 0x%x,CHKER_TYPE = 0x%x, "
				"CHKER_ADR = 0x%x\n MPUA = 0x%x, "
				"MPUI = 0x%x\n%s%s\n EMI MPU violation.\n",
				     dbg_s,
				     dbg_t+emi_physical_offset,
...
#if defined(CONFIG_LGE_HANDLE_PANIC) && \
	defined(CONFIG_LGE_HANDLE_PANIC_MTK_EMI_MPU)
	victim = master_ID & 0x7;
	switch (victim) {
		case MASTER_APMCU:
			pr_err("[EMI MPU] APMCU violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_APMCU);
			break;
		case MASTER_MM:
			pr_err("[EMI MPU] MM violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_MM);
			break;
		case MASTER_PERI:
			pr_err("[EMI MPU] PERI violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_PERI);
			break;
		case MASTER_MDMCU:
			pr_err("[EMI MPU] MDMCU violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_MDMCU);
			break;
		case MASTER_MDHW:
			pr_err("[EMI MPU] MDHW violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_MDHW);
			break;
		case MASTER_MFG:
			pr_err("[EMI MPU] MFG violation trigger BUG\n");
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_MFG);
			break;
		default:
			pr_err("[EMI MPU] Unknown violation trigger BUG\n");
			pr_err("[EMI MPU] victim = 0x%x\n", victim);
			lge_set_reboot_reason(LGE_CRASH_EMI_MPU_UNKNOWN);
			break;
	}
