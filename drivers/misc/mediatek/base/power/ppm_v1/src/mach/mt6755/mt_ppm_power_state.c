...
enum ppm_power_state ppm_judge_state_by_force_limit(enum ppm_power_state cur_state,
			struct ppm_forcelimit_data force_limit)
{
	enum ppm_power_state new_state = PPM_POWER_STATE_NONE;
	int LL_core_min = force_limit.limit[0].min_core_num;
	int LL_core_max = force_limit.limit[0].max_core_num;
	int L_core_min = force_limit.limit[1].min_core_num;
	int L_core_max = force_limit.limit[1].max_core_num;
	int LL_freq_min = force_limit.limit[0].min_freq_idx;
	int L_freq_max = force_limit.limit[1].max_freq_idx;
...
/* modify request to fit cur_state */
void ppm_limit_check_for_force_limit(enum ppm_power_state cur_state, struct ppm_policy_req *req,
			struct ppm_forcelimit_data force_limit)
{
	if (req && cur_state == PPM_POWER_STATE_L_ONLY) {
		unsigned int LL_min_core = req->limit[0].min_cpu_core;
		...
		if (LL_min_core > 0 && L_max_freq >= LL_min_freq) {
			/* user do not set L min so we just move LL core to L */
			if (force_limit.limit[1].min_core_num <= 0) {
				req->limit[0].min_cpu_core = 0;
				req->limit[1].min_cpu_core = LL_min_core;
				ppm_ver("Judge: move LL min core to L = %d\n", LL_min_core);
			} else if (sum <= 4) {
...
enum ppm_power_state ppm_judge_state_by_prm_limit(enum ppm_power_state cur_state,
			struct ppm_prmlimit_data prm_limit)
{
	enum ppm_power_state new_state = PPM_POWER_STATE_NONE;
	int LL_core_min = prm_limit.limit[0].min_core_num;
	int LL_core_max = prm_limit.limit[0].max_core_num;
	int L_core_min = prm_limit.limit[1].min_core_num;
	int L_core_max = prm_limit.limit[1].max_core_num;
	int LL_freq_min = prm_limit.limit[0].min_freq_idx;
	int L_freq_max = prm_limit.limit[1].max_freq_idx;
	int sum;
...
/* modify request to fit cur_state */
void ppm_limit_check_for_prm_limit(enum ppm_power_state cur_state, struct ppm_policy_req *req,
			struct ppm_prmlimit_data prm_limit)
{
	if (req && cur_state == PPM_POWER_STATE_L_ONLY) {
		unsigned int LL_min_core = req->limit[0].min_cpu_core;
		...
		if (LL_min_core > 0 && L_max_freq >= LL_min_freq) {
			/* prm do not set L min so we just move LL core to L */
			if (prm_limit.limit[1].min_core_num <= 0) {
				req->limit[0].min_cpu_core = 0;
				req->limit[1].min_cpu_core = LL_min_core;
				ppm_ver("Judge: move LL min core to L = %d\n", LL_min_core);
			} else if (sum <= 4) {
