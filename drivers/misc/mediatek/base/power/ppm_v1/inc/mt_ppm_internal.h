...
#define PPM_POLICY_PRIO_PRM_SPECIFY_BASE	(0x20)
#endif
...
struct ppm_forcelimit_data {
	bool is_freq_limited_by_user;
	bool is_core_limited_by_user;
	bool is_fixed_for_game;

	struct ppm_force_limit {
		int min_freq_idx;
		int max_freq_idx;
		int min_core_num;
		int max_core_num;
...
#ifdef CONFIG_LGE_PM_MTK_PRM
struct ppm_prmlimit_data {
	bool is_freq_limited_by_prm;
	bool is_core_limited_by_prm;

	struct ppm_prm_limit {
		int min_freq_idx;
		int max_freq_idx;
		int min_core_num;
		int max_core_num;
...
extern void ppm_limit_check_for_user_limit(enum ppm_power_state cur_state, struct ppm_policy_req *req,
			struct ppm_userlimit_data user_limit);
#ifdef CONFIG_LGE_PM_MTK_GAME_MODE
extern enum ppm_power_state ppm_judge_state_by_force_limit(enum ppm_power_state cur_state,
			struct ppm_forcelimit_data force_limit);
extern void ppm_limit_check_for_force_limit(enum ppm_power_state cur_state, struct ppm_policy_req *req,
			struct ppm_forcelimit_data force_limit);
#endif
#ifdef CONFIG_LGE_PM_MTK_PRM
extern enum ppm_power_state ppm_judge_state_by_prm_limit(enum ppm_power_state cur_state,
			struct ppm_prmlimit_data prm_limit);
extern void ppm_limit_check_for_prm_limit(enum ppm_power_state cur_state, struct ppm_policy_req *req,
			struct ppm_prmlimit_data prm_limit);
#endif
