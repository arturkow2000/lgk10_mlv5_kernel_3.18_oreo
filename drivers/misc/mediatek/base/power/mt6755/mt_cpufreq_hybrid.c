...
void dump_infracfg_info(void)
{
	void __iomem *infracfg_base = NULL, *infracfg_ao_base = NULL;
	unsigned int offset = 0x0;

	pr_err("********** dump infracfg_ao_base **********\n");

	infracfg_ao_base = ioremap(0x10001000, PAGE_SIZE);

	for(offset = 0x0; offset < 0x1000; offset += 0x10)
		pr_err("%x(PA) : (+0x0)%08x (+0x4)%08x (+0x8)%08x (+0xC)%08x\n",
				(unsigned int)(0x10001000+offset), (unsigned int)readl(infracfg_ao_base+offset),
				(unsigned int)readl(infracfg_ao_base+offset+0x4), (unsigned int)readl(infracfg_ao_base+offset+0x8),
				(unsigned int)readl(infracfg_ao_base+offset+0xC));

	pr_err("********** dump infracfg_base **********\n");

	infracfg_base = ioremap(0x10201000, PAGE_SIZE);

	for(offset = 0x0; offset < 0x1000; offset += 0x10)
		pr_err("%x(PA) : (+0x0)%08x (+0x4)%08x (+0x8)%08x (+0xC)%08x\n",
				(unsigned int)(0x10201000+offset), (unsigned int)readl(infracfg_base+offset),
				(unsigned int)readl(infracfg_base+offset+0x4), (unsigned int)readl(infracfg_base+offset+0x8),
				(unsigned int)readl(infracfg_base+offset+0xC));
...
void check_emi_status(void)
{
	static void *infracfg_base;
	static void *emi_base;
	unsigned int debug[4];
	unsigned int debug_ctrl;

	/* MTK debugging */
	infracfg_base = ioremap(0x10201000, 0x1000);
	emi_base = ioremap(0x10203000, 0x1000);

	pr_err("********** Check EMI status **********\n");

	if (infracfg_base && emi_base) {
		debug[0] = __raw_readl(infracfg_base + 0x100);
		debug[1] = __raw_readl(infracfg_base + 0x104);
		debug[2] = __raw_readl(infracfg_base + 0x108);
		debug[3] = __raw_readl(infracfg_base + 0x10C);
		debug_ctrl = __raw_readl(emi_base + 0xE8);

		mt_reg_sync_writel(debug_ctrl | (0x1 << 8), emi_base + 0xE8);

		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0x1 << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0x2 << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x3 << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x4 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		pr_err("emi + 0x5FC: 0x%x\n", __raw_readl(emi_base + 0x5FC));

		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0x0 << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0x7 << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x6 << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x5 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		...
		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0xb << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0xa << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x9 << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x8 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		...
		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0x14 << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0x13 << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x12 << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x11 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		...
		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0x18 << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0x17 << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x16 << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x15 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		...
		mt_reg_sync_writel((debug[0] & ~0xFFE0) | (0x10 << 5), infracfg_base + 0x100);
		mt_reg_sync_writel((debug[1] & ~0xFFE0) | (0x1b << 5), infracfg_base + 0x104);
		mt_reg_sync_writel((debug[2] & ~0xFFE0) | (0x1a << 5), infracfg_base + 0x108);
		mt_reg_sync_writel((debug[3] & ~0xFFE0) | (0x19 << 5), infracfg_base + 0x10C);
		pr_err("infracfg + 0x100: 0x%x\n", __raw_readl(infracfg_base + 0x100));
		pr_err("infracfg + 0x104: 0x%x\n", __raw_readl(infracfg_base + 0x104));
		pr_err("infracfg + 0x108: 0x%x\n", __raw_readl(infracfg_base + 0x108));
		pr_err("infracfg + 0x10C: 0x%x\n", __raw_readl(infracfg_base + 0x10C));
		...
		mt_reg_sync_writel(debug[0], infracfg_base + 0x100);
		mt_reg_sync_writel(debug[1], infracfg_base + 0x104);
		mt_reg_sync_writel(debug[2], infracfg_base + 0x108);
		mt_reg_sync_writel(debug[3], infracfg_base + 0x10C);
		mt_reg_sync_writel(debug_ctrl, emi_base + 0xE8);
	} else
		pr_err("infracfg or emi base is not available!\n");
}
...
void check_usb_status(void)
{
	...
	pr_err("********** CHECK USB STATUS **********\n");

	base = ioremap((phys_addr_t)0x112807B0, PAGE_SIZE);

	writel(0x0000000F, base); // 0x112807B0
	writel(0x00070005, base+0x4); // 0x112807B4
	writel(0x00170002, base+0x8); // 0x112807B8
	writel(0x02161515, base+0xC); // 0x112807BC

	pr_err("Read PA:0x112807C4 : %x\n", (unsigned int)readl(base+0x14)); // 0x112807C4

	writel(0x00160008, base+0x4); // 0x112807B4
	writel(0x00170002, base+0x8); // 0x112807B8
	writel(0x02160202, base+0xC); // 0x112807BC

	while(++count <= 10)
		pr_err("%dth read PA:0x112807C4 : %x\n", count, (unsigned int)readl(base+0x14)); // 0x112807C4
...
unsigned int occurred_dvfstmo = 0;
			       ...
			       unsigned int *ret_volt)
{
	...
	if (r >= 0) {	/* DVFS done */
		...
		/* debug purpose */
		WRITE_ONCE(occurred_dvfstmo, 1);

		/* debug purpose for INFRA */
		dump_infracfg_info();

		/* debug purpose for EMI */
		check_emi_status();

		/* debug purpose for MSDC*/
