...
static int hps_sanitize_var(unsigned int *hpsvar, unsigned int newv)
{
	...
	} else if (hpsvar == &hps_ctxt.pa_hps_enabled) {
		if (newv >= 0 && newv < (unsigned int)MAX_PA_HPS)
			rc = 0;
#endif
	} else {
...
#ifdef CONFIG_POWER_AWARENESS_HPS
PROC_FOPS_RW_UINT(pa_hps_enabled, hps_ctxt.pa_hps_enabled, hps_proc_uint_write_with_lock_reset);
#endif
