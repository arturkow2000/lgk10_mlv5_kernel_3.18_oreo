...
static bool slp_dump_regs = 0;
bool pmic_dump_regs = 0;
#endif
...
static int slp_suspend_ops_enter(suspend_state_t state)
{
	...
	if (slp_dump_gpio)
		gpio_dump_regs_func();
	/* CLK */
#if 0 /* LGE_CHANGE 170814 removed by MTK commit 20d05076cd39fb05f4325923c8e2adc00b1f2b6e */
	if (slp_dump_regs)
		mt_clk_dump();
#endif
	/* PMIC Regs*/
	if (pmic_dump_regs) {
		ret = pmic_read_interface(MT6353_EN_STATUS1, &pm_status_reg[0], 0xFFFF, 0);
		slp_emerg("PMIC EN_STATUS1  = 0x%x\n", (unsigned int)pm_status_reg[0]);
		ret = pmic_read_interface(MT6353_EN_STATUS2, &pm_status_reg[1], 0xFFFC, 0);
		slp_emerg("PMIC EN_STATUS2  = 0x%x\n", (unsigned int)pm_status_reg[1]);
	}
#endif
...
module_param(pmic_dump_regs, bool, 0644);
#endif
