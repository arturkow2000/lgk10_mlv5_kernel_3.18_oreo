...
#ifdef CONFIG_LGE_PM
				/* do not print log wakeup reason is only GPT1 */
				if ((wakesta->r12 & WAKE_SRC_R12_APXGPT1_EVENT_B) == 0) {
					so_warn(sodi_flags, "[LGE] wake up by %s, vcore_status = %d, self_refresh = 0x%x, sw_flag = 0x%x, 0x%x, %s, %d, 0x%x, timer_out = %u, r13 = 0x%x, debug_flag = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, %d\n",
