...
static void collect_dump(int md_id)
{
	void __iomem *infracfg_ao_base = NULL, *perisys_debug_base = NULL, *pwm_base = NULL;
	unsigned int offset = 0;

	CCCI_ERROR_LOG(md_id, KERN, "********** collect_dump **********\n");

	infracfg_ao_base = ioremap(0x10001000, PAGE_SIZE);
	perisys_debug_base = ioremap(0x10003500, PAGE_SIZE);
	pwm_base = ioremap(0x11006010, PAGE_SIZE);

	CCCI_ERROR_LOG(md_id, KERN, "********** dump infracfg_ao **********\n");
	CCCI_ERROR_LOG(md_id, KERN, "0x10001000 : 0x%08X\n", readl(infracfg_ao_base));
	CCCI_ERROR_LOG(md_id, KERN, "0x10001070 : 0x%08X\n", readl(infracfg_ao_base + 0x70));
	CCCI_ERROR_LOG(md_id, KERN, "0x10001074 : 0x%08X\n", readl(infracfg_ao_base + 0x74));
	CCCI_ERROR_LOG(md_id, KERN, "0x10001190 : 0x%08X\n", readl(infracfg_ao_base + 0x90));
	CCCI_ERROR_LOG(md_id, KERN, "0x10001094 : 0x%08X\n", readl(infracfg_ao_base + 0x94));
	CCCI_ERROR_LOG(md_id, KERN, "0x10001098 : 0x%08X\n", readl(infracfg_ao_base + 0x98));
	CCCI_ERROR_LOG(md_id, KERN, "0x100010AC : 0x%08X\n", readl(infracfg_ao_base + 0xAC));

	CCCI_ERROR_LOG(md_id, KERN, "********** dump perisys_debug **********\n");
	for (offset = 0; offset < 0x19; offset += 0x4)
	    CCCI_ERROR_LOG(md_id, KERN, "0x%08X : 0x%08X\n", 0x10003500 + offset, readl(perisys_debug_base + offset));

	CCCI_ERROR_LOG(md_id, KERN, "********** dump pwm_reg **********\n");
	for (offset = 0; offset < 0xc1; offset += 0x40)
	    CCCI_ERROR_LOG(md_id, KERN, "0x%08X : 0x%08X\n", 0x11006010 + offset, readl(pwm_base + offset));

	iounmap(infracfg_ao_base);
	iounmap(perisys_debug_base);
	iounmap(pwm_base);
}
...
static void mdee_output_debug_info_to_buf(struct md_ee *mdee, DEBUG_INFO_T *debug_info, char *ex_info)
{
	...
	case MD_EX_DUMP_ASSERT:
		...
		snprintf(ex_info, EE_BUF_LEN_UMOLY,
				...
				debug_info->assert.parameters[0],
				debug_info->assert.parameters[1], debug_info->assert.parameters[2]);
		//if (strcmp(debug_info->assert.file_name, "l1core/modem/mml1/mml1_rf/mml1_rf_bpitop.c") == 0)
		//if (strcmp(debug_info->assert.file_name, "pcore/modem/l4/atci/src/rmmi_validator_em.c") == 0)
			collect_dump(md_id);
		break;
...
#if defined(CONFIG_LGE_HANDLE_PANIC)
	if (debug_info && debug_info->type != 0) {
		lge_set_reboot_reason(LGE_CRASH_MODEM_PANIC);
