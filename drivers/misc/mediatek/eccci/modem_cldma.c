...
static struct kmem_cache *cldma_request_cachep;
...
static void cldma_rx_ring_init(struct ccci_modem *md, struct cldma_ring *ring)
{
	...
	if (ring->type == RING_GPD) {
		for (i = 0; i < ring->length; i++) {
			item = kmem_cache_zalloc(cldma_request_cachep, GFP_KERNEL);
	...
	if (ring->type == RING_GPD_BD) {
		for (i = 0; i < ring->length; i++) {
			...
			for (j = 0; j < MAX_BD_NUM + 1; j++) {	/* extra 1 BD for skb head */
				bd_item = kmem_cache_zalloc(cldma_request_cachep, GFP_KERNEL);
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
	...
	cldma_request_cachep = kmem_cache_create("cldma_request",
							sizeof(struct cldma_request),
							0, SLAB_PANIC, NULL);
...
{
	if (cldma_request_cachep)
		kmem_cache_destroy(cldma_request_cachep);
}
