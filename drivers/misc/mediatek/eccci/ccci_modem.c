...
void ccci_md_config(struct ccci_modem *md)
{
	...
	md->mem_layout.md_region_vir =
		(void __iomem *)vmap_reserved_mem(md->mem_layout.md_region_phy, MD_IMG_DUMP_SIZE, prot);
...
int ccci_md_prepare_runtime_data(struct ccci_modem *md, struct sk_buff *skb)
{
	...
	struct FactoryNetworkCode *temp_buf = NULL;
	...
	for (i = BOOT_INFO; i < FEATURE_COUNT; i++) {
		...
		if (rt_feature.support_info.support_mask == CCCI_FEATURE_MUST_SUPPORT) {
			...
			case MISC_INFO_NTCODE:
				...
				temp_buf = (struct FactoryNetworkCode *)kmalloc(EMMC_BLOCK_SIZE, GFP_KERNEL);

				if (temp_buf == NULL)
				{
					pr_err("temp_buf alloc Fail!\n");
				}
				else
				{
					...
					memcpy((char*)&(rt_f_element.feature[0]), mcc, 4); /* MCC */
					memcpy((char*)&(rt_f_element.feature[1]), mnc, 4); /* MNC */
					memcpy((char*)&(rt_f_element.feature[2]), subset, 4); /* Subset */
				}
