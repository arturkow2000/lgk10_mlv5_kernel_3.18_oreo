...
#ifdef CONFIG_LGE_PM
static unsigned int g_cur_level = 0;
static unsigned int g_max_level = 0;
static unsigned int g_old_cur_level = 0;
static unsigned int g_old_max_level = 0;
//static unsigned int g_thermal_level = 0;
static unsigned int restore_level = 0;

/* Thermal mitigation BL setting value */
static unsigned int case0_level = 255;
static unsigned int case1_level = 200;
static unsigned int case2_level = 151;
static unsigned int case3_level = 100;
#endif

#ifdef CONFIG_LGE_PM
static void mtk_cl_backlight_set_max_brightness_limit_for_lge(void)
{
	if (g_backlight_last_level != g_backlight_level) {
		switch (g_backlight_level) {
		case 0:
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			if (restore_level == 1)
				release_max_brightness_by_user(1);
			pr_info("[JJB] case 0 Set cur: %d(%d) max: %d(%d) \n",
							g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			thermald_status_update(1);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			restore_level = 0;
			pr_info("[JJB] case 0(%d) Final cur: %d(%d) max: %d(%d) \n",
							case0_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			break;
		case 1:
			g_old_cur_level = get_brightness();
			g_old_max_level = get_max_brightness();
			if (case1_level < g_old_cur_level)
				lge_set_max_brightness(case1_level);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			pr_info("[JJB] case 1(%d) Set cur: %d(%d) max: %d(%d) \n",
							case1_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			thermald_status_update(1);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			restore_level = 1;
			pr_info("[JJB] case 1(%d) Final cur: %d(%d) max: %d(%d) \n",
							case1_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			break;
		case 2:
			g_old_cur_level = get_brightness();
			g_old_max_level = get_max_brightness();
			if (case3_level < g_old_cur_level)
				lge_set_max_brightness(case2_level);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			pr_info("[JJB] case 2(%d) Set cur: %d(%d) max: %d(%d) \n",
							case2_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			thermald_status_update(1);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			restore_level = 2;
			pr_info("[JJB] case 2(%d) Final cur: %d(%d) max: %d(%d) \n",
							case2_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			break;
		case 3:
			g_old_cur_level = get_brightness();
			g_old_max_level = get_max_brightness();
			if (case3_level < g_old_cur_level)
				lge_set_max_brightness(case3_level);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			pr_info("[JJB] case 3(%d) Set cur: %d(%d) max: %d(%d) \n",
							case3_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			thermald_status_update(1);
			g_cur_level = get_brightness();
			g_max_level = get_max_brightness();
			restore_level = 3;
			pr_info("[JJB] case 3(%d) Final cur: %d(%d) max: %d(%d) \n",
							case3_level, g_cur_level, g_old_cur_level, g_max_level, g_old_max_level);
			break;
		default:
...
#ifdef CONFIG_LGE_PM
		mtk_cl_backlight_set_max_brightness_limit_for_lge();
#else
