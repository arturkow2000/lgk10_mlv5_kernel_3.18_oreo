...
/*****************************************************************************
 * Baton Undet Int Handler
 ******************************************************************************/
void baton_undet_int_handler(void)
{
	PMICLOG("[baton_undet_int_handler]BATON_UNDET status = %d....\n",
		pmic_get_register_value(PMIC_RGS_BATON_UNDET));

#if defined(CONFIG_MTK_SMART_BATTERY)
	do_baton_undet_int_task();
#endif
	if (pmic_get_register_value(PMIC_RGS_BATON_UNDET))
		pmic_enable_interrupt(43, 0, "PMIC");
}
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	pmic_register_interrupt_callback(43, baton_undet_int_handler);
#endif
